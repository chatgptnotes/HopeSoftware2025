...
class AuditableBehavior extends ModelBehavior {
  ...
  public function afterSave( Model $Model, $created ) {
    ...
    //overwrite model alias with proper english name
    ...
    if(($audit['NewCropAllergies']['is_deleted'] == 1) || ($audit['Person']['is_deleted'] == 1) || 
    		($audit['Patient']['is_deleted'] == 1) || ($audit['Diagnosis']['is_deleted'] == 1) || 
    		($audit['RadiologyTestOrder']['is_deleted'] == 1) || 
    		($audit['LaboratoryTestOrder']['is_deleted'] == 1)) {
	    $data = array(
	      'Audit' => array(
	        'event'     => 'DELETE',
	        'model'     => $Model->alias,
	      ...
	      //  'source_id' => isset( $source['id'] ) ? $source['id'] : null
	      		'source_id' => AuthComponent::user('id')
	      )
	    );
    } else {
    ...
    foreach( $audit[$Model->alias] as $property => $value ) {
      ...
      if( !$created ) {
        if( array_key_exists( $property, $this->_original[$Model->alias] ) && $this->_original[$Model->alias][$property] != $value ) {
           ...
           */
          //in allergies status is N and A as a inactive and active respectively. 
          
          if($Model->alias=='NewCropAllergies'){
          	if($this->_original[$Model->alias][$property]=='N' || $this->_original[$Model->alias][$property]=='A'){
          		$this->_original[$Model->alias][$property] = ($this->_original[$Model->alias][$property]=='N')?'Inactive':'Active' ;
          	}
          	if($value=='N' || $value=='A'){
          		$value = ($value=='N')?'Inactive':'Active' ;
          	} 
          }
    ...
    if( $created || count( $updates ) ) {
      $Model->Audit->create();
      // insert patient unique id to get display in listing and then sorted out patient by unique id //
      if($Model->alias == "Patient") {
      	$getPatientUniqueId = Classregistry::init('Patient')->find('first', array('conditions' => array('Patient.id' => $Model->id), 'fields' => 'Patient.person_id'));
      	$setPatientUniqueId['patient_id'] = $getPatientUniqueId['Patient']['person_id'];
      } 
      if ($Model->alias == "Person") {
      	$setPatientUniqueId['patient_id'] = $audit['Person']['id'];
      }
      if ($Model->alias == "Diagnosis") {
      	$getPatientUniqueId = Classregistry::init('Patient')->find('first', array('conditions' => array('Patient.id' => $audit['Diagnosis']['patient_id']), 'fields' => 'Patient.person_id'));
      	$setPatientUniqueId['patient_id'] = $getPatientUniqueId['Patient']['person_id'];
      } 
      if ($Model->alias == "RadiologyTestOrder") {
      	$getPatientUniqueId = Classregistry::init('Patient')->find('first', array('conditions' => array('Patient.id' => $audit['RadiologyTestOrder']['patient_id']), 'fields' => 'Patient.person_id'));
      	$setPatientUniqueId['patient_id'] = $getPatientUniqueId['Patient']['person_id'];
      } 
 
      if ($Model->alias == "LaboratoryToken") {
      	$getPatientUniqueId = Classregistry::init('Patient')->find('first', array('conditions' => array('Patient.id' => $audit['LaboratoryToken']['patient_id']), 'fields' => 'Patient.person_id'));
      	$setPatientUniqueId['patient_id'] = $getPatientUniqueId['Patient']['person_id'];
      } 
      if ($Model->alias == "Note") { 
      	$getPatientUniqueId = Classregistry::init('Patient')->find('first', array('conditions' => array('Patient.id' => $audit['Note']['patient_id']), 'fields' => 'Patient.person_id'));
      	$setPatientUniqueId['patient_id'] = $getPatientUniqueId['Patient']['person_id'];
      	if($audit['Note']['sign_note']==0 && $audit['Note']['reason_to_unsign'] != ''){
      		$data['Audit']['model'] = 'PowerNote' ; //to fetch between log of unsign and again sign 
      	}
      	 
      }
      if ($Model->alias == "NewCropPrescription") {
      	$setPatientUniqueId['patient_id'] = $audit['NewCropPrescription']['patient_id'];
      }
      if ($Model->alias == "NewCropAllergies") {
      	$setPatientUniqueId['patient_id'] = $audit['NewCropAllergies']['patient_id'];
      }
      
      if ($Model->alias == "NoteDiagnosis") {
      	$getPatientUniqueId = Classregistry::init('Patient')->find('first', array('conditions' => array('Patient.id' => $audit['NoteDiagnosis']['patient_id']), 'fields' => 'Patient.person_id'));
      	$setPatientUniqueId['patient_id'] = $getPatientUniqueId['Patient']['person_id'];
      	 
      }
      
      
      $data['Audit']['patient_id'] = $setPatientUniqueId['patient_id'];
