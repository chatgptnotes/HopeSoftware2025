...
<div class="inner_title">
	...
	<?php
	//debug($diagnosis);
	$strXML ='<chart caption="Weight for Age Chart for Females" subCaption="2 Years to 20 Years" labelStep="365" canvasBgColor="#DDDDDD" numVDivLines="35" bgAlpha="100" adjustDiv="1" toolTipBgColor="DDDDDD"  connectNullData="1"  yAxisMinValue="5" yAxisValuesStep="3" numDivLines="35" drawAnchors="0" xAxisName="AGE(Years)" yAxisName="Weight (Kg)" bgColor="#DDDDDD" showBorder="0"   use3DLighting="0" chartRightMargin="40"  showLegend="0" showValues="0" divLineColor="#F90792" yAxisMaxValue="110"  alternateHGridAlpha="20" showAlternateHGridColor="0" showAlternateVGridColor="0" anchorBgColor="#000"  decimalPrecision="5" baseFontColor="#000" animation="0">';
	...
	for($i=730;$i<=$max;$i++)
	{
		if(array_key_exists($i,$bmi_array))
		{
			foreach($bmi_array as $key=>$value)
			{
				...
				{
			$strPlot.='<set toolText="Weight:'.$bmi.' Kg {br} AGE:'.$year[$i].".".$month[$i].' Years {br}Noted On:'.date('m/d/Y',strtotime($date[$i])).'" value="'.$bmi.'"/>';
