...
<?php
//debug($diagnosis);
$strXML ='<chart caption="Head Circumference chart for infants (Boys)" subCaption="0 to 36 Months" canvasBgColor="#DDDDDD" numVDivLines="35" bgAlpha="100" labelStep="90"   connectNullData="1"  yAxisMinValue="30" yAxisValuesStep="" numDivLines="25" anchorBgColor="#000" toolTipBgColor="DDDDDD" drawAnchors="0" xAxisName="AGE(Years)" yAxisName="HeadCircumference" bgColor="#DDDDDD" showBorder="0"   use3DLighting="0" chartRightMargin="40"  showLegend="0" showValues="0" divLineColor="#0245EF"   alternateHGridAlpha="20" showAlternateHGridColor="0" showAlternateVGridColor="0"  decimalPrecision="5" baseFontColor="#000" animation="0">';
...
$strData7.='</dataset>'; */
$strData8='<dataset color="#0245EF" lineThickness="1">';
for($i=0;$i<=1080;$i++)
{
...
$strData8.='</dataset>';
$strData9='<dataset color="#0245EF" lineThickness="1">';
for($i=0;$i<=1080;$i++)
{
		...
		foreach($bmi_array as $key=>$value)
			{
				foreach($value as $bmi)
				{
						...
						$strPlot.='<set toolText="Head Circumference:'.$bmi.' cm {br} AGE:'.$year[$i].".".$month[$i].' Years {br}Noted On:'.date('m/d/Y',strtotime($date[$i])).'" value="'.$bmi.'"/>';
