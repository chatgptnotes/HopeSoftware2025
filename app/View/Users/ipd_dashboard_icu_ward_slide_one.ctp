...
<table width="100%" cellpadding="0" cellspacing="0" border="0" class="table_format1">
	...
	<tr class="row_title">
		<th valign="top" align="left" class="table_cell" style="padding-left: 10px;">BED</th>
	  	<!--  <th width="28%" valign="top" align="center" class="table_cell"><?php  echo __('REGISTRATION DATE', true);?></th>-->
	  	<!--  <th width="5%" valign="top" class="table_cell">LOS</th>-->
		<th width="2%" valign="top" class="table_cell"></th>
		<th width="35%" valign="top" class="table_cell" align="left"><?php  echo __('PATIENT', true);?></th>
		<!--  <th valign="top" class="table_cell">TARIFF</th>-->
		<th valign="top" align="left" class="table_cell"><span>PHYSICIAN<br></span></th>
	</tr>
	<?php 
	$i=0;
	$currentWard =0;
...
$this->Paginator->options(array('url' =>array("?"=>$queryStr)));
	...
	if($i%2 == 0) {
		...
		<?php echo $wardVal['Patient']['Room']['bed_prefix'].$wardVal['Patient']['Bed']['bedno'];?>
		</td>
		...
		<td align="left" class="td_ht" style="padding-left: 10px;"><?php // if(strtolower($role)==strtolower(Configure::read('adminLabel'))) { ?>
			
			<?php 
				$doctorId = 'doctor_'.$wardVal['Patient']['Patient']['id'];
				$doctorName = $doctors[$wardVal['Patient']['Patient']['doctor_id']];
				$stringCut = substr($doctorName, 0, 16);
				$docName = explode("(",$doctorName); 
				$explodeString = explode(',', $stringCut);
				//$docName = substr($explodeString[0], 0, strrpos($explodeString[0], ' '));
				$doubleExplode = explode(' ',$explodeString[0]);
				
				if(count($doubleExplode)>3){
					$drName = 'Dr.'.$doubleExplode[0].$doubleExplode[1];
				}else{
					$drName = 'Dr.'.$explodeString[0];
				}
				echo 'Dr.'.$docName[0];
