...
<?php }?>
<style>
.admission-id[contenteditable="true"] {
    padding: 2px 5px;
...
.admission-id[contenteditable="true"]:focus {
    outline: none;
    border-color: #66afe9;
...
<td>
    <span class="admission-id" data-patient-id="<?php echo $wardVal['Patient']['Patient']['id']; ?>" contenteditable="true">
        <?php echo $wardVal['Patient']['Patient']['admission_id']; ?>
    </span>
    <br><br>
    <?php echo $wardVal['Patient']['Person']['mobile']; ?>
    <br>
    <button class="btn btn-primary btn-xs save-admission" 
            data-id="<?php echo $wardVal['Patient']['Patient']['id']; ?>" style="display: none;">
        Save
    </button>
</td>
...
$(document).ready(function() {
    // Show save button when admission ID is edited
    $('.admission-id').on('input', function() {
        $(this).nextAll('.save-admission').first().show();
    });
    
    $('.save-admission').click(function() {
        var patientId = $(this).data('id');
        var admissionElement = $(this).prevAll('.admission-id').first();
        var newId = admissionElement.text().trim();
        ...
        $.ajax({
            url: '<?php echo $this->Html->url(['controller' => 'users', 'action' => 'update_admission_id']); ?>',
            type: 'POST',
            data: {
                patient_id: patientId,
                new_admission_id: newId
            },
            success: function(response) {
                try {
                    response = JSON.parse(response);
                    if (response.success) {
                        // Update successful, hide save button
                        saveButton.hide();
                    } else {
                        alert(response.message);
                        // Revert to original value if needed
                        admissionElement.text(response.original_value || newId);
                    }
                } catch(e) {
