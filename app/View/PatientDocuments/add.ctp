...
<div class="inner_title">
	<h3>
		&nbsp;
		<?php echo __('Add Patient Documents', true);?><font font-size="20px" font-family="verdana" color="darkolivegreen"><?php echo "&nbsp;(".$patientData['Patient']['lookup_name'].' - '.$patientData['Patient']['patient_id'].")";
	...
	<span><?php 
	echo $this->Html->link(__('Back'), array('action' => 'index',$patientId,'?'=>array('flagBack'=>'1')), array('escape' => false,'class'=>'blueBtn'));
	?>
	</span>
</div>
<?php 
if(!empty($errors)) {
?>
<table border="0" cellpadding="0" cellspacing="2" width="36%"
	align="center">
	<tr>
...
<script>
	jQuery(document).ready(function(){
		...
		jQuery("#PatientDocumentAddForm").validationEngine();
	});
...
echo $this->Form->create('PatientDocument',array('type' => 'file')); 
 if(!empty($patient_id)){
 	echo $this->Form->hidden('patient_id',array('id'=>'patient_id','value'=>$patient_id));
...
<table border="0" class="table_format" cellpadding="0"
	...
	<tr>
			...
			<td><input type="text" name="PatientDocument[name]"
					class="textBoxExpnd validate[required,custom[mandatory-enter]]" style='' readonly tabindex="1" value="<?php echo $patientData['Patient']['lookup_name']?>"/></td>
	...
	<tr>
		...
		$d= $this->DateFormat->formatDate2Local(date('Y-m-d H:i:s'),Configure::read('date_format'),true);
		echo $this->Form->input('date',array('type'=>'text','id'=>'date','class'=>'textBoxExpnd', 'readonly'=>'readonly','style'=>'width:120px','label'=>false,'value'=>$d));?> 
		</td>
	</tr>
	...
	<tr>
		...
		<table>
		<tr>
		<td><?php echo $this->Form->input('bill_amount',array('type'=>'text','id'=>'bill_amount','class'=>'textBoxExpnd validate[required,custom[onlyNumber]]','style'=>'width:120px','label'=>false,'div'=>false));?>
		</td>
		<td>
		<?php 
		echo $this->Form->input('payment_type',array('type'=>'select','id'=>'pay_type','class'=>'textBoxExpnd ','style'=>'width:120px','label'=>false,'empty'=>'Please Select','options'=>array('cash'=>'Cash','credit'=>'Credit'),'div'=>false));?>
		</td>
		</tr>
		</table>
		</td>		
	...
	<tr>
		...
		<td><textarea name="PatientDocument[document_description]"
				tabindex="3" rows="2" cols="2" class='textBoxExpnd' style='' ></textarea></td>
	</tr>	
	<tr>
		<td>Comments :</td>
...
<table class="" border="0" cellpadding="0" cellspacing="0" width="100%"
	align="center">	
	<!--<tr>
		<td id ='errorwarn' colspan="4" align="center" style="color: RED;font-size:15px;"></td>
	</tr>-->
	<tr id="PackageBreakup">
		<td>
			<table width="100%" border="0" cellspacing="0" cellpadding="0"
				...
				<tr>
					<th align="center"><?php echo __('Service Name');?><font color="red">*</font>
					</th>
					<th align="center"><?php echo __('Image Upload');?>
					</th>
					<th align="center"><?php echo __('Report Upload');?>
					</th>					
			...
			?>
				<?php $key = 0;?>
				<?php  foreach($radiologyNameData as $keyM=>$value){ 
					//debug($value);?>
				<tr id="removePackageData-<?php echo $key;?>">					
					<td width="20%" style="padding-left:10px;"><?php $radiologySrNo = $key+1;
					echo $this->Form->input("fddfg",array('type'=>'text','id'=>"childId-$key",'name'=> 'data[document_name]['.$key.']','value'=>$value['Radiology']['name'],'class' => 'docName validate[required,custom[mandatory-enter]] textBoxExpnd','label'=>false,'div'=>false));
						echo $this->Form->hidden("dddd",array('type'=>'text','id'=>"docId-$key",'class' => 'alertMsg rate textBoxExpnd calculateCost','name'=> 'data[document_id]['.$key.']','autocomplete'=> 'off','label'=>false,'div'=>false,'value'=>$value['Radiology']['id']));
						echo $this->Form->hidden("ytyt",array('type'=>'text','id'=>"radTestOrderID-$key",'class' => 'rate textBoxExpnd calculateCost','name'=> 'data[rad_test_order_id]['.$key.']','autocomplete'=> 'off','label'=>false,'div'=>false,'value'=>$value['RadiologyTestOrder']['id']));?>
					</td>
