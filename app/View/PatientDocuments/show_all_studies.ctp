<div class="inner_title">
	<h3> &nbsp; <?php echo __('Lanuch Dicom Viewer', true); ?></h3> 
</div>
...
<table cellpadding="1" cellspacing="1" width="100%" >
	...
	<tr>
		<td width="30%"><b>Study ID</font></b></td>
	...
	if($_SERVER['HTTP_HOST']=='192.168.8.8:5454')
	{
	   $viewerurl="http://192.168.8.10:8080";
	}
	else
	{
	...
	<tr>
		...
		<?php $otherName=$patientData['Patient']['lookup_name']."_".$patientData['Patient']['age']."_".$data['uuid'];?>
			<a  href="<?php echo $viewerurl?>/weasis-pacs-connector/viewer-applet?studyUID=<?php echo $data['study_iuid']?>" title="Dicom Viewer" onclick="var w = window.open(href, 'webviewPage', 'scrollbars=yes,location=no,menuBar=no,resizable=yes,status=no,toolbar=no'); if(w.blur) w.focus(); return false;" target="_blank"><?php echo $data['study_id']?></a>
		</td>
