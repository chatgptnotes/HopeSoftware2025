...
<?php echo $this->Html->css(array('jquery.weekcalendar_1.9','jquery.weekcalendar','colpick','jquery.autocomplete.css','calendar_datepicker.css','jquery.ui.all.css'/*,'jquery.ui.timepicker.css'*/));?>
<?php echo $this->Html->script(array('jquery.ui.core','jquery.weekcalendar_1.9','colpick','jquery.blockUI'/*,'jquery.autocomplete'*/,'calendar_datepicker','calendar_date','jquery.ui.timepicker')); ?>
...
$(document).ready(function() {
   ...
   $calendar.weekCalendar({
                  ...
                  $.ajax({
                      ...
                      $('#busy-indicator').hide();
                        if($.trim(html) == "save") { 
							...
							html += '';
							if($.trim(html) == '') { html = 'Please enter patient information';/*  style = 'style="width: 111%;"';*/ }
							$("span.ui-dialog-title").html('<span class="error" '+style+'>'+$.trim(html)+'</span>'+'<br>'+mess);
						}
            		}
	 ...
	 var ss = calEvent.body;
            ...
            buttons: {
               Update : function() {
                ...
                //  calEvent.visit_type = visitTypeField.val();
                  ...
                  $('#busy-indicator').show();
                  //alert(editApptId);//gaurav------
                  calEvent.status = (editApptId) ? 'Pending' : calEvent.status;//for cancelled appointment
         ...
         var startField = $dialogContent.find("select[name='start']").val(calEvent.start);
         var endField = $dialogContent.find("select[name='end']").val(calEvent.end);
        // $('#dateofbirth').datePicker({inline:false,clickInput:true});alert('ssss');
   ...
   function getEventData() {
      ...
      freebusys: [
   				/*{"start": new Date(year, month, day+0, 00), "end": new Date(year, month, day+3, 00, 00), "free": true, userId: [0,1,2,3]},
   				...
   				{"start": new Date(year, month, day+0, 08), "end": new Date(year, month, day+0, 18, 00), "free": false, userId: 3}*/
   				<?php /*if($showCalendarDay != 7)*/echo $freeBusys;?>
   				//{"start": new Date(2014, 6, 27, 00), "end": new Date(2014, 6, 28, 00, 00), "free": true, userId: [0]}, 		
   				//{"start": new Date(2014, 6, 27, 07), "end": new Date(2014, 6, 28, 18, 00), "free": false, userId: [0]},
   				//{"start": new Date(2014, 5, 22, 00), "end": new Date(2014, 5, 22, 00, 00), "free": true, userId: [2]},
   				//{"start": new Date(2014, 5, 22, 00), "end": new Date(2014, 5, 22, 00, 00), "free": true, userId: [7]},
   				//{"start": new Date(2014, 5, 23, 00), "end": new Date(2014, 5, 23, 00, 00), "free": true, userId: [2]},
   				//{"start": new Date(2014, 5, 23, 00), "end": new Date(2014, 5, 23, 00, 00), "free": true, userId: [7]},
...
.mobile_width_left{width:15%;float:left;}
.normal_width_left{width:15%;float:left;}
.mobile_right_width{width:85%;float:left;}
.normal_right_width{width:85%;float:left;}
...
-->
	...
	<input type="hidden" name="listflag" id="listflag"
		value="<?php echo $this->params['named']['listflag']; ?>" /> <input
		type="hidden" name="apppatientid" id="apppatientid"
		value="<?php echo $patientAppointmentData['Patient']['id']; ?>" /> <input
		type="hidden" name="appadmissionid" id="appadmissionid"
		value="<?php echo $patientAppointmentData['Patient']['admission_id']; ?>" />
	<input type="hidden" name="apppatientuid" id="apppatientuid"
		value="<?php echo $patientAppointmentData['Patient']['patient_id']; ?>" />
	<input type="hidden" name="apppatientname" id="apppatientname"
		value="<?php echo $patientAppointmentData['Patient']['lookup_name']; ?>" />
	<input type="hidden" name="apppatientdob" id="apppatientdob"
		value="<?php echo $patientAppointmentData['Person']['dob']; ?>" />
	<input type="hidden" name="applocationid" id="applocationid"
	...
	<div>
		...
		<div style="padding-left: 43px; padding-bottom: 5px;">
			<span>Calender Work Hours</span></br> <input type="text"
				...
				type="text" class="timeCalender" readonly="readonly"
				id="timepicker_end" name="data[DoctorProfile][endhours]"
				value="<?php echo $this->data['DoctorProfile']['endhours']?>">
		</div>
		<?php }?>
		<div  class="mobile_width_left">
			<div id="datepicker" style="padding: 0 0 23px 0px;"></div>
			...
			<div style="margin: 0px; padding: 0px">
				...
				<span style="text-align: middle; font-size: 11px !important;"> <?php echo __('Select All'); ?>
				</span>
				<?php }?>
				<?php 
				...
				<span style="text-align: middle; font-size: 11px !important;"> <?php echo __('Just me'); ?>
				</span>
				<?php }?>
			</div>
		</div>

		<div id='calendar' class="mobile_right_width"></div>
	</div>
	...
	<div id="event_edit_container" style="display: none;">
		<form>
			...
			<li><?php echo __('Patient Location'); ?>:<br /> <?php 
				...
				<?php }?>
				<li><span><?php echo __('Date'); ?>: </span><span
					class="date_holder"><input type="text" name="scheduledate"
						readonly="readonly" id="scheduledate" style="width: 86%;" /> </span>
				</li>
				...
				//echo $this->Form->input('',array('name'=>'treatment_type_txt','id'=>'treatment_type_txt1','div'=>false,'label'=>false,'error'=>false));
				...
				<li><?php echo __('Patient Name'); ?>: <input type="text"
				...
				<li><?php echo __('Reason of Visit'); ?>:<textarea name="body"></textarea>
				</li>
...
function saveDoctorColor(doctorId,color){
	...
	var httpRequestSaveColor = $.ajax({
      ...
      success:function(){
    	  ...
    	  removeFlag = (removeFlag != '') ? removeFlag : 'just_me';
...
function loadingLabel(){
	 $('#main-grid').block({ 
    message: '<?php echo $this->Html->image('icons/ajax-loader_dashboard.gif');?>', 
    css: {            
        padding: '5px 0px 5px 18px',
        border: 'none', 
        padding: '15px', 
...
var editApptId = '';
$(function(){ 
	editApptId = '<?php echo $this->request->query['reScheduleAppt'];?>';
	if(editApptId){
		$('div#'+editApptId).trigger('click');
	}
	$('#datepicker').datePicker({inline:true,startDate:new Date('01/01/1975')})
