...
<?php   echo $this->element("corporate_billing_report");?>
 <div class="inner_title" > 
 	<h3>&nbsp; Mahindra & Mahindra Report</h3> 
    <div style="float:right;">
				<span style="float:right;">
					<?php
						echo $this->Form->create('surgeonreport',array('url'=>array('controller'=>'Corporates','action'=>'mahindra_report','admin'=>'TRUE'),'id'=>'surgeonreport','type'=>'get', 'style'=> 'float:left;')); 
	...
	<div cellpadding="0" cellspacing="0" border="0"  align="left" style="float:left; width:100%; ">
	    ...
	    <span>
	   	...
	   	<span id="look_up_name" class="LookUpName">
			...
			<?php
				echo $this->Html->link($this->Html->image('icons/refresh-icon.png'), array('action'=>'admin_mahindra_report'),array('escape'=>false, 'title' => 'refresh'));
				echo $this->Form->end();
			?>
		</div>
    </div>
	...
	<table width="100%" cellpadding="0" cellspacing="2" border="0" class="tabularForm labTable resizable sticky" id="item-row" style="top:0px; overflow: scroll;"> 
		<tr>
		  	<thead>
				...
				<th width="51px" valign="top" align="center" style="text-align:center;">Relation with Emp. </th>
				<th width="60px" valign="top" align="center" style="text-align:center;">Admission Date</th>
				<th width="48px" valign="top" align="center" style="text-align:center;">Bill No.</th>
				<th width="66px" valign="top" align="center" style="text-align:center;">Discharge Date</th>
				<th width="47px" valign="top" align="center" style="text-align:center;">Hospital Invoice Amount</th>
				<th width="37px"  valign="top" align="center" style="text-align:center;">Bank</th>
                <th width="37px"  valign="top" align="center" style="text-align:center;">Amount Recieved</th>
				<th width="37px" valign="top" align="center" style="text-align:center;">Adv Received</th>
                <th width="37px" valign="top" align="center" style="text-align:center;">TDS</th>
				<th width="50px" valign="top" align="center" style="text-align:center;">Other Deduction</th>
				<th width="80px" valign="top" align="center" style="text-align:center;">Amount Received Date</th>
				<th width="50px" valign="top" align="center" style="text-align:center;">Bill Submission Date</th>
				...
				<th width="61px" valign="top" align="center" style="text-align:center;">Bill Due Date</th>
				<th width="21" valign="center" align="center" style="text-align: center; min-width: 21px;">Action</th>
	   		</thead>
      	</tr>
    ...
    ?>
    	<tr id="row_<?php echo $patient_id; ?>" class="rowselected">
			...
			<td width="50px" style="text-align:center;">
				<?php echo $this->Form->input('bill_number', array('id'=>'bill_'.$bill_id,'type'=>'text','style'=>"width:20%",'label'=>false,
						'div'=>false,'style'=>"width: 100%;",'class'=>'add_bill_number','value'=>$result['FinalBilling']['bill_number'])); ?>
			</td>
			...
			<td width="56px" style="text-align:center;">
				 <?php echo $this->Form->input('package_amount', array('id'=>'package_'.$bill_id,'autocomplete'=>'off','type'=>'text','label'=>false,
				 		'div'=>false,'style'=>"width: 90%;",'class'=>'add_package_amount','value'=>'','patient_id'=>$patient_id));?>	
			</td>
			<td width="104px">
		    <?php
			...
			<td width="40px" style="text-align:center;">
				<?php echo $this->Form->input('tds', array('id'=>'tds_'.$bill_id,'autocomplete'=>'off','type'=>'text','label'=>false,
						'div'=>false,'style'=>"width: 90%;",'class'=>'add_tds','value'=>''));?>
			</td>
			...
			<td width="64px" style="text-align:center;">
				<table>
					<tr>
						<td>	
							<?php echo $this->Form->input(' ', array('id'=>'otherDeduction_'.$bill_id,'autocomplete'=>'off','type'=>'text',
									'label'=>false,'div'=>false,'style'=>"width: 90%;",'class'=>'add_other_deduction','value'=>''));
							?>
		 				</td>
				...
				<?php   //InvoiceDate 
					if(isset($result['FinalBilling']['cmp_paid_date'])){
						...
						echo $this->Form->input("cmp_paid_date_$patient_id",array('autocomplete'=>'off','style'=>"width: 65%;",
							'class'=>'textBoxExpnd cmp_paid_date','label'=>false,'value'=>''));
					}
				?>			
			</td>
...
$('.LookUpName').click(function()
		  {
		  	...
		  	//alert(lookup_name);
				...
				var ajaxUrl = "<?php echo $this->Html->url(array("controller" => 'corporates', "action" => "mahindra_report", "admin" => true));?>";
				$.ajax({
...
$('.add_other_deduction').blur(function(){
			...
			$.ajax({
				url : "<?php echo $this->Html->url(array("controller" => 'Corporates', "action" => "getCalculateOtherDeduction", "admin" => false));?>"+"/"+flag+"/"+val+"/"+OtherDeductionId,
...
*/
/*$('.add_tds, .add_package_amount').keyup(function()
		  {
			  var bill = $(this).attr('id') ;
			  splittedId = bill.split("_");
			  tdsId = splittedId[1];
			  ...
			  var val = $(this).val();
				...
				// to get calculated value of other deduction
				
				/*id=$(this).attr('id');
				splitedvar=id.split("_"); 
				
				var invoice=$('#amt_'+splitedvar[1]).text();					// Hospital Amount
				var tds=parseInt($('#tds_'+splitedvar[1]).val());				// Tds
				var packAmout = parseInt($('#package_'+splitedvar[1]).val());   // Amount Recieved

				
				var total=parseInt(invoice)-(parseInt(tds)+parseInt(packAmout));
				
			$.ajax({
			url : "<?php echo $this->Html->url(array("controller" => 'billings', "action" => "getTds", "admin" => false));?>"+"/"+tdsId+"/"+val+"/"+total,
			...
			success: function(data){
						$('#busy-indicator').hide();
						$('#deduction_'+splitedvar[1]).val(total);
			     }
			});
			}
			);*/
	...
	/*		$('.add_tds, .add_package_amount,.adv_rec').blur(function()
					  {
						  ...
						  if($("#bank_"+ID).val()==''){ 
                                                    ...
                                                    var val = $(this).val();
							var invoice=$('#amt_'+ID).text(); 											// Hospital Amount
...
function addRemarks(patientID){ 
    $.fancybox({
        ...
        'transitionOut': 'fade',
        'type': 'iframe',
        'href': "<?php echo $this->Html->url(array("action" => "getRemarkForCorporateReport",'admin'=>false)); ?>"+'/'+patientID+"/Mahindra Remark"
   });
}
</Script>