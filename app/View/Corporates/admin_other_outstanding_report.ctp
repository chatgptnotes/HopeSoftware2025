...
<div class="inner_title">
	<h3>Outstanding Report - <?php echo $tariffData[$tariffStandardID];?></h3>
		...
		<span style="float:right;">
    		<?php	
 	  			echo $this->Form->create('',array('url'=>array('controller'=>'Corporates','action'=>'other_outstanding_report',$tariffStandardID,'admin'=>'TRUE'),'id'=>'otherOutRepFrm','type'=>'get')); 
		...
		<td style="color:#000;">
		 ...
		 <td><?php echo "Delay In Bill Preparation - Aging Report <br>".$this->Form->input('delay_in_bill_preparation',array('class'=>'textBoxExpnd','empty'=>'Please Select',
		  ...
		  <td>
			...
			<?php echo $this->Html->link($this->Html->image('icons/refresh-icon.png'),array('action'=>'other_outstanding_report',$tariffStandardID,'admin'=>'TRUE'),array('escape'=>false, 'title' => 'Refresh'));?>
		  </td>	
...
<table width="100%" cellpadding="0" cellspacing="2" border="0" class="tabularForm labTable resizable sticky" id="container-table" style="top:0px;  overflow: scroll;">
	<thead>
		...
		<th width="10%"  align="center" style="text-align:center;">Package</th>
	...
	<tr >
		...
		<td  align="center" style=" text-align:center;" class="<?php echo $color ;?>"><?php
// 		echo $result['Patient']['corporate_status']; 
		?>
		<?php
                    if (!empty($result['Patient']['corporate_status'])) {
                        if ($result['Patient']['corporate_status'] == 1) {
                            echo "General";} elseif ($result['Patient']['corporate_status'] == 2) {echo "Semi-Private";} elseif ($result['Patient']['corporate_status'] == 3) {echo "Private";} else {echo "Not Enter";}} else {echo "Not Enter"; }?></td>
		...
		<td width="89px"  align="center" style="text-align: center;" class="<?php echo $color ;?>"><?php echo $balAmount=$totalBillAmount-$amntRecievedByPatient-$otherDeduction-$tdsAmnt-$amntRecieved;//bal amount?></td> 
		...
		<td width="21" align="center" style="text-align: center; min-width: 21px;" class="<?php echo $color ;?>">
		   <?php 
				...
				echo '<a   class="fancybox" href="#corporateClaims" style="display:'.$showEdit.'"  total_amount="'.$result['FinalBilling']['total_amount'].'"  advance_received="'.$result['FinalBilling']['package_amount'].'" patient_id="'.$result['Patient']['id'].'" patient_name="'.$result['Patient']['lookup_name'].'" tariff_standard_id="'.$result['Patient']['tariff_standard_id'].'">';
