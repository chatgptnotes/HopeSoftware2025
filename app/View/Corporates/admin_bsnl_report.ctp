...
<div class="inner_title">
	<h3 style="float: left;">BSNL Reports</h3>
     <div style="float:right;">
        <span style="float:right;">
                <?php

        echo $this->Form->create('surgeonreport',array('url'=>array('controller'=>'Corporates','action'=>'bsnl_report','admin'=>'TRUE'),'id'=>'surgeonreport','type'=>'get', 'style'=> 'float:left;')); 
	...
	<table width="" cellpadding="0" cellspacing="0" border="0" class="tdLabel2" style="color: #b9c8ca;">
		<tr>
			<td style="color:#000;">
				...
				<span>
		    		<?php echo "Date From : "."&nbsp;".$this->Form->input('from', array('id'=>'fromDate','value'=>$this->request->query['from'],
		    				'label'=>false,'div'=>false,'error'=>false)); ?>
		   		</span>
			    <span>
			    	<?php echo "To Date : "."&nbsp;".$this->Form->input('to', array('id'=>'toDate','value'=>$this->request->query['to'],
			    			'label'=>false,'div' => false, 'error' => false));?>
			    </span>
				...
				<span>
					<?php
						echo $this->Html->link($this->Html->image('icons/refresh-icon.png'), array('action'=>'admin_bsnl_report'),
...
<table width="100%" cellpadding="0" cellspacing="2" border="0" class="tabularForm labTable resizable sticky" id="item-row" style="top:0px;overflow: scroll;">
	<tr>
        ...
        <th width="100px" align="center" style="text-align:center;">Name Of Patient</th>
        <th width="100px" align="center" style="text-align:center;">Name Of Employee<br />(rank)</th>
        <th width="80px" align="center"	style="text-align:center;">Relation with Employee</th>
        <th width="70px" align="center" style="text-align: center;">Date of Admission</th>
        ...
        <th width="70px" align="center" style="text-align: center;">Amount Received</th>
        ...
        <th width="70px" align="center" style="text-align: center;">Other Deduction</th>
        <th width="80px" align="center" style="text-align:center;">Amount Received Date</th>
        ...
        <th width="5px" align="center" style="text-align: center; min-width: 21px;">#</th>
	</tr>
	<?php 
		$i=0; 
		$advRecevied=0;
	 ...
	 ?>
		<tr id="row_<?php echo $patient_id; ?>" class="rowselected">
			...
			<td width="97px" align="center" style="text-align:center;">
				<?php echo $result['Patient']['patient_id'];     //Patient name
			...
			<td width="91px" align="center" style="text-align: center;">
				<?php echo $form_received_on = $this->DateFormat->formatDate2Local($result['Patient']['form_received_on'],Configure::read('date_format'));?>
	   		</td>
	   
			<td width="101px" align="center" style="text-align: center;">
				<?php echo $this->Form->input('bill_number', array('id'=>'bill_'.$bill_id,'autocomplete'=>false,'type' =>'text',
						'style'=>"width:20%",'label'=>false ,'div'=>false,'style'=>"width: 100%;",'class'=>'add_bill_number',
						'value'=>$result['FinalBilling']['bill_number'])); 
				?>
	    	</td>
			     	
			<td width="92px" align="center"style="text-align: center;">
			...
			<td width="110px" align="center">
                <?php  
                    ...
                    echo $this->Form->input('cmp_amt_paid', array('id'=>'amt_'.$bill_id,'type' => 'text','readonly'=>'readonly',
					'label'=>false ,'div'=>false,'style'=>"width: 100%;",'class'=>'cmp_amt_paid','value'=>$hospitalInvoice)); 		
                ?>
			</td>
                
			 <td width="96px" align="center"style="text-align: center; min-width: 60px;">
		     	<?php echo $this->Form->input('bank', array('id'=>'bank_'.$bill_id,'autocomplete'=>'off','type'=>'select','label'=>false,
		     			'div'=>false,'style'=>"width: 70%;",'class'=>'bank','value'=>'','options'=>$bank,'empty'=>'Please Select'));				
				 ?>
			 </td>
                
			 <td width="96px" align="center"style="text-align: center; min-width: 60px;">
		     	<?php echo $this->Form->input('package_amount', array('id'=>'package_'.$bill_id,'autocomplete'=>'off','type'=>'text',
		     			'label'=>false,'div'=>false,'style'=>"width: 70%;",'class'=>'add_package_amount',
		     			'value'=>$result['FinalBilling']['package_amount'],'patient_id'=>$patient_id));
			...
			<td>
			    <?php echo $this->Form->input('tds', array('id'=>'tds_'.$bill_id,'autocomplete'=>false,'type'=>'text','label'=>false,
			    		'div'=>false,'style'=>"width:48px;",'class'=>'add_tds','value'=>$result['FinalBilling']['tds']));
			...
			<td width="250px"  align="center" style="text-align: center;">
				<table>
					<tr>
						<td>	
							<?php
							  echo $this->Form->input('', array('id'=>'otherDeduction_'.$bill_id,'autocomplete'=>'off','type' => 'text','label'=>false ,
			...
			<td>
				<?php   //InvoiceDate 
					if(isset($result['FinalBilling']['cmp_paid_date'])){
						...
						echo $this->Form->input("cmp_paid_date_$patient_id",array('autocomplete'=>'off','style'=>"width: 65%;",
		   ...
		   <td width="108px"  align="center" style="text-align: center;">     
            ...
            <td align="center" style="text-align: center;">
				<?php echo $this->Form->input('remark',array('id'=>'remark_'.$result['Patient']['id'],'autocomplete'=>false,'type'=>'textarea',
					'label'=>false,'rows'=>'1','cols'=>'1','class'=>'add_remark','value'=>$result['Patient']['remark']));
				?>
			</td>
	   ...
	   var subtract= parseInt($('#amt_'+splittedId[1]).val()); 	// Hospital Amount Recieved 192
		...
		$('#otherDeduction_'+splittedId[1]).val(totalAmnt);
                $.ajax({
                        ...
                        type: 'POST',
                        data : '&tds='+val+'&package_amount='+amtRec+'&other_deduction='+totalAmnt+'&patient_id='+patientID,
                        beforeSend:function(data){
                                $('#busy-indicator').show();
                        },
                        success: function(data){
...
function addRemarks(patientID){ 
    $.fancybox({
        ...
        'transitionOut': 'fade',
        'type': 'iframe',
        'href': "<?php echo $this->Html->url(array("action" => "getRemarkForCorporateReport",'admin'=>false)); ?>"+'/'+patientID+"/BSNL Remark"
   });
}
</script>