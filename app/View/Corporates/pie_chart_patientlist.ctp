	...
	if($this->params->paging['Appointment']['page']==1) {
		...
		$srno= $this->params->paging['Appointment']['limit']*($this->params->paging['Appointment']['page']-1) ;		
			...
			foreach($patientList as $key=>$value){
				...
				<td width="20%"><?php if($value['OptAppointment']['schedule_date'])
				{
					echo $this->DateFormat->formatDate2Local($value['OptAppointment']['schedule_date'],
							Configure::read('date_format'),true);
				}
				
				echo $this->DateFormat->formatDate2Local($value['Appointment']['date'],
