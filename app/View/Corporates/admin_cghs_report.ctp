<?php echo $this->Html->css(array(/*'jquery.fancybox-1.3.4.css',*/'jquery.autocomplete.css','colResizable.css'));  
 echo $this->Html->script(array(/*'jquery.fancybox-1.3.4',*/'jquery.autocomplete.js','colResizable-1.4.min.js')); 
...
<div class="inner_title">
	<h3>CGHS (Ordnance factory Chanda) Reports</h3>
    	<div style="float:right;">
		<span style="float:right;">
    			<?php
						echo $this->Form->create('surgeonreport',array('url'=>array('controller'=>'Corporates','action'=>'cghs_report','admin'=>'TRUE'),'id'=>'surgeonreport','type'=>'get', 'style'=> 'float:left;')); 
...
<div class="clr ht5"></div>
	<table width="50%" cellpadding="0" cellspacing="0" border="0" class="tdLabel2" style="color: #000;">
		<tr>				
			<td style="color:#000;">
	 		...
	 		<td><?php echo __("From :");?></td>
	  		<td>
				<?php echo $this->Form->input('from', array('id'=>'from','label'=>false,'value'=>$this->request->query['from'],'div'=>false,'error'=>false));?>		
	  		</td>
	  		<td><?php echo __("To :");?></td>  
	  		<td>
				<?php echo $this->Form->input('to',array('id'=>'to','label'=>false,'div'=>false,'value'=>$this->params->query['to'],'error'=>false));?>
	  		</td>
	  		<td>
	  			<span id="look_up_name" class="LookUpName">
					...
					<?php echo $this->Html->link($this->Html->image('icons/refresh-icon.png'),array('action'=>'cghs_report'),array('escape'=>false, 'title' => 'refresh'));?>
	 			</span>
	  		</td>			
		</tr>
	</table>
...
<table width="100%" cellpadding="0" cellspacing="2" border="0" class="tabularForm labTable resizable sticky" id="item-row" style="top:0px; overflow: scroll;">
	...
	<thead>
		...
		<th width="70px" align="center"style="text-align: center;">Bill NO</th>
        <th width="70px" align="center"style="text-align: center;">Claim ID</th>
        <th width="70px" align="center"style="text-align: center;">Card No</th>
		<th width="81px" align="center"style="text-align: center;">Date of Discharge</th>
		<th width="70px" align="center"style="text-align: center;">Hospital Invoice Amount</th>
		<th width="70px" align="center"style="text-align: center;">Bank</th>
        <th width="70px" align="center"style="text-align: center;">Amount  <br />Received</th>
		<th width="70px" align="center"style="text-align: center;">Advance <br />Received</th>
		<th width="70px" align="center" style="text-align: center;">TDS </th>
		<th width="70px" align="center" style="text-align: center;"> Other deduct</th>
	...
	<tr id="row_<?php echo $patient_id; ?>" class="rowselected">
		...
		<td width="88px" align="center" style="text-align: center;">
		...
		<td width="91px" align="center" style="text-align: center;">
			<?php echo $this->Form->input('bill_number', array('id'=>'bill_'.$bill_id,'type' => 'text','style'=>"width:20%",
					'label'=>false,'div'=>false,'style'=>"width:95%;",'class'=>'add_bill_number','value'=>$result['FinalBilling']['bill_number'])); ?>	 
		</td>
                
        <td width="87px" style="text-align:center;">
            <?php echo $this->Form->input('claimID', array('id'=>'claimID_'.$patient_id,'type'=>'text','label'=>false,'div'=>false,
            		'style'=>"width: 70%;",'class'=>'claim_id','value'=>$result['Patient']['claim_id'])); ?>
        </td>
		...
		<td width="110px" align="center">
           <?php echo $this->Form->hidden('patientId',array('id'=>'patient_'.$bill_id,'value'=>$patient_id,'class'=>'patient_id'));
        ...
        <td width="88px" align="center"style="text-align: center;">
	     	<?php echo $this->Form->input('bank',array('id'=>"bank_".$bill_id,'type'=>'select','autocomplete'=>false,'label'=>false,
	     			'div'=>false,'style'=>"width: 100%;",'class'=>'bank','value'=>'','empty'=>'Please Select','options'=>$banks));?>
		</td>
		
		 <td width="88px" align="center"style="text-align: center;">
            <?php echo $this->Form->input('package_amount',array('id'=>"package_".$bill_id,'autocomplete'=>false,'type'=>'text','label'=>false,
		...
		<td width="85px" align="center"style=" text-align:center;">
      		<?php echo $this->Form->input('tds', array('id'=>'tds_'.$bill_id,'type' => 'text','autocomplete'=>false,'label'=>false,
      				'div'=>false,'style'=>"width: 70%;",'class'=>'add_tds','value'=>''));?>
   		</td>
        ...
        <td align="center"style="text-align: center;">
			<?php echo $this->Form->input('remark',array('id'=>'remark_'.$result['Patient']['id'],'type'=>'textarea','label'=>false,
					'rows'=>'1','cols'=>'2','class'=>'add_remark','value'=>$result['Patient']['remark']));?>
		</td>
		
		<td width="76px" align="center"style="text-align: center;"> 
			<?php  
		...
		<td width="30" align="center"style="text-align: center; min-width: 30px;">
                <?php 
...
{var bill = $(this).attr('id'); 
	...
	newId = splittedId[1]; 
	var val = $(this).val(); 
        var patientID = $("#patient_"+newId).val();
	/*$.ajax({
		url : "<?php echo $this->Html->url(array("controller" => 'corporates', "action" => "getBillamt", "admin" => false));?>"+"/"+newId+"/"+val+"/"+patientID,
		beforeSend:function(data){
		$('#busy-indicator').show();
		},
		success: function(data){
...
$('.LookUpName').click(function(){
		  	...
		  	//alert(lookup_name);
				...
				var ajaxUrl = "<?php echo $this->Html->url(array("controller" => 'corporates', "action" => "cghs_report", "admin" => true));?>";
				$.ajax({
...
$( ".bill_uploading_date" ).datepicker({
		...
		$('.deletep_').click(function(){
			 var del = $(this).attr('id') ;
			 splittedId = del.split("_");
			  delId = splittedId[1];
	
				$.ajax({
					url : "<?php echo $this->Html->url(array("controller" => 'corporates', "action" => "setDelete", "admin" => false));?>"+"/"+delId,
					beforeSend:function(data){
						$('#busy-indicator').show();
						//inlineMsg(patient,'<?php echo $this->Html->image('/ajax-loader.gif') ?>')	
					},
					 success: function(data){
						 $('#busy-indicator').hide();
						 var bullet= '<?php echo $this->Html->image('icons/cross.png',array('alt'=>'Deleted'));?>';
						 $("#isDeletep_"+delId).html(bullet);
						 }
...
$('.add_tds, .add_package_amount,.adv_rec').blur(function(){
	...
	$.ajax({
		...
		//data : 'package_amount='+val+'&tds='+tds+'&other_deduction='+totalAmnt,
		beforeSend:function(data){
			$('#busy-indicator').show();
		},
		success: function(data){
 ...
 $('.add_card').blur(function()
{
 ...
 $('.claim_id').blur(function()
{
    var patientId = $(this).attr('id').split("_")[1] ;
    var val = $(this).val(); 
      $.ajax({
        url : "<?php echo $this->Html->url(array("controller" => 'Patients', "action" => "getClaimID", "admin" => false));?>"+"/"+patientId+"/"+val,
...
function addRemarks(patientID){ 
    $.fancybox({
        ...
        'transitionOut': 'fade',
        'type': 'iframe',
        'href': "<?php echo $this->Html->url(array("action" => "getRemarkForCorporateReport",'admin'=>false)); ?>"+'/'+patientID+"/CGHS Remark"
   });
}
</script>