...
<?php echo $this->Html->css(array('datePicker.css','jquery-ui-1.8.16.custom','validationEngine.jquery.css','jquery.ui.all.css','internal_style.css'));
	echo "<style>.blueBtn{ display:none;}</style>"; ?>
<div class="inner_title" style="padding:8px;">
    	<h3>FALL ASSESSMENT INTERVENTIONS</h3>
    	<span  style="padding-right:20px;">
		<?php  echo $this->Html->link(__('Print'),'#', array('id'=>'print','escape' => false,'class'=>'blueBtn','style'=>'padding:5px;12px;display: inline;',
					'onclick'=>"var openWin = window.open('".$this->Html->url(array('action'=>'print_intervention',$patient_id,'risk_level'=>$risk_level,
					'row_id'=>$row_id))."', '_blank', 'toolbar=0,scrollbars=1,location=0,status=1,menubar=0,resizable=1,width=900,height=600,left=200,top=200');  return false;"));
		?></span>
</div>
...
<table width="85%" style="padding-left:20px;">
	<tr id="tr_low_heading_small" style="display:none; font-size:13px;color:#7DD7FC;">
		<th align="center"><b>LOW FALLS RISK  (Universal Falls Precautions)</b></th>
	</tr>
	<tr id="tr_low_small" style="display:none;font-size:12px;color:#B5C8FF;">
		<td><b>Maintain safe unit environment </b>
		  <ul>
			<li>Remove excess equipment/supplies/ furniture from rooms & hallways.</li>
			<li>Coil and secure excess electrical and telephone wires/cords.</li>
	...
	<tr id="tr_medium_small" style="display:none;font-size:12px;color:#FFFF8D;">
		<td><b>Maintain safe unit environment</b>
		  <ul>
			<li>Remove excess equipment/supplies/ furniture from rooms & hallways.</li>
			<li>Coil and secure excess electrical and telephone wires/cords.</li>
	...
	var risk_level = <?php echo json_encode($risk_level); ?>;
		...
		if(risk_level == 'Low Risk Level'){			
			// intereventions as per risklevel
			 ...
			 } else if(risk_level == 'Midium Risk Level'){
