...
<table cellspacing="0" cellpadding="0" border="0"
	...
	for($h=$currentHr;$h >=0;$h--){ 
		...
		$timeBar .= "<td class='gray-container doubleClick ' id='".$dateString."_".$h."'><div class='time-area'>";
		if($h==0)
			$timeBar .= "00:00- 00:59" ;
		else
			$timeBar .= $h.":00- ".$h.":59" ;
		
		$timeBar .='<input  id="check_'.$dateString."_".$h.'" type="checkbox"
		...
		<td>
			 <table cellpadding="0" cellspacing="0">
									<tr class="test">
										...
										<td class="gray-container" colspan=<?php echo $currentHr+$shiftCount+1 ;?>><?php echo date('m/d/Y',strtotime($selectedDate));?>
										</td> 
	...
	foreach ($subCatOptions as $optionKey =>$optionValue){
		...
		foreach ($arrayOfOptions as $subOptionKey =>$subOptionValue){
			...
			if($subCatName=='continuous infusion' || $subCatName=='medications'){
			...
			//today			
			for($h=$currentHr;$h >=0;$h--){
				$currentDayT =  $dateArray[$dateString][$h][$optionValue['ReviewSubCategory']['id']][$subOptionValueID]['value'] ; 
				if($currentDayT ==0) continue ; //for 0
				if($dateArray[$dateString][$h][$optionValue['ReviewSubCategory']['id']][$subOptionValueID]['is_deleted']==1) continue ;//dont count deleted entry
				$currentSubCatTotal += $currentDayT  ;
				$datewiseSubCat[$dateString][$h][$str][] = $currentDayT; 
				$datewiseSubCat[$dateString][$h][$optionValue['ReviewSubCategory']['id']][]= $currentDayT ; 
				//calculate shift count  
				if($h > 14 && $h <=23){									 
					$datewiseSubCat[$dateString][$str]['day'][] 		= $currentDayT ;
					$datewiseSubCat[$dateString]['shift_option']['day'][$optionValue['ReviewSubCategory']['id']][] = $currentDayT;
					$datewiseSubCat[$dateString]['shift_sub_option']['day'][$subOptionValueID][] = $currentDayT;
				}else if($h > 7 && $h <=14){				
					$datewiseSubCat[$dateString][$str]['morning'][] 	= $currentDayT ;
					$datewiseSubCat[$dateString]['shift_option']['morning'][$optionValue['ReviewSubCategory']['id']][] = $currentDayT;
					$datewiseSubCat[$dateString]['shift_sub_option']['morning'][$subOptionValueID][] = $currentDayT;
				}else{				 
					$datewiseSubCat[$dateString][$str]['night'][] 	= $currentDayT ;
					$datewiseSubCat[$dateString]['shift_option']['night'][$optionValue['ReviewSubCategory']['id']][] = $currentDayT;
					$datewiseSubCat[$dateString]['shift_sub_option']['night'][$subOptionValueID][] = $currentDayT;
				}
				//EOF shift count 
			}
		...
		$datewiseSubCat[$dateString][$varname] =   $currentSubCatTotal ;  
	...
	//echo "pankaj".$ContinuousInfusion(ml) ;
	?>
	<tr>
		<td><div>
								...
								<tr class="treesubmenu">
									...
									<?php  for($h=$currentHr;$h >=0;$h--){ 
											...
											if($h == 7){
												...
												$total8 =  array_sum($datewiseSubCat[$dateString]['intake']['night']) ; 
												$timeBar .= ($total8>0)?$total8:'' ;
												$timeBar .= "</div></td>" ;
											}else if($h == 14){
												//morning
												$timeBar = "<td  class='size50' ><div class='time-area'>";
												$total9 =  array_sum($datewiseSubCat[$dateString]['intake']['morning']) ; 
												$timeBar .= ($total9>0)?$total9:'' ; 
												$timeBar .= "</div></td>" ;
											}else if($h == 22){
												//afternoon
												$timeBar = "<td class='size50'><div class='time-area'>";
												$total10 =  array_sum($datewiseSubCat[$dateString]['intake']['day']) ;
									...
									<td class="size50">
										<div    >
											<?php $intakeSum  = array_sum($datewiseSubCat[$dateString][$h]['intake']) ;
								...
								<tr class="treesubmenu">
									...
									<?php  for($h=$currentHr;$h >=0;$h--){
										...
										if($h == 7){ 
											...
											$total5 = array_sum($datewiseSubCat[$dateString]['output']['night']) ;
											$timeBar .= ($total5>0)?$total5:'' ;
											$timeBar .= "</div></td>" ;
										}else if($h == 14){
											//morning 
											$timeBar = "<td  class='border' ><div class='time-area'>";
											$total6 = array_sum($datewiseSubCat[$dateString]['output']['morning']) ;
											$timeBar .= ($total6>0)?$total6:'' ;										 
											$timeBar .= "</div></td>" ;
										}else if($h == 22){
											//afternoon
											$timeBar = "<td class='border' ><div class='time-area'>";
											$total7 = array_sum($datewiseSubCat[$dateString]['output']['days']) ;
									...
									<td>
										<div class="size50" >
											<?php $outputTotal  = array_sum($datewiseSubCat[$dateString][$h]['output']) ; 
							...
							<li id="<?php echo clean($optionValue['ReviewSubCategory']['name']); ?>">
								<table class="treesubmenu">
									<tr>
										...
										<?php  
										for($h=$currentHr;$h >=0;$h--){
											$className = $dateString."_".$h;
											$timeBar = '' ;
											if($h == 7){
												//night
												$timeBar = "<td class='border' ><div class='time-area'>";
												$total2 = array_sum($datewiseSubCat[$dateString]['shift_option']['night'][$optionValue['ReviewSubCategory']['id']]) ;
												$timeBar .= ($total2>0)?$total2:'' ; 
												$timeBar .= "</div></td>" ;
											}else if($h == 14){
												//morning
												$timeBar = "<td  class='border'><div class='time-area'>";
												$total3 = array_sum($datewiseSubCat[$dateString]['shift_option']['morning'][$optionValue['ReviewSubCategory']['id']]) ;
												$timeBar .= ($total3>0)?$total3:'' ;
												$timeBar .= "</div></td>" ;
											}else if($h == 22){
												//afternoon
												$timeBar = "<td class='border' ><div class='time-area'>";
												$total4  = array_sum($datewiseSubCat[$dateString]['shift_option']['day'][$optionValue['ReviewSubCategory']['id']]) ;
												$timeBar .= ($total2>0)?$total2:'' ;
												$timeBar .= "</div></td>" ;
											}
											echo $timeBar ;
										...
										<td>
											<div  class="<?php echo $className ?> sub-cat-option size50 ">
												<?php $total1 = array_sum($datewiseSubCat[$dateString][$h][$optionValue['ReviewSubCategory']['id']]) ;
										...
										<div class='total'>
												<?php $varname = clean($optionValue['ReviewSubCategory']['name']) ; 
													  echo   $datewiseSubCat[$dateString][$varname]  ; 
												?>
											</div>
										</td>											 
									</tr>
								...
								<ul>
									...
									foreach ($arrayOfOptions as $subOptionKey =>$subOptionValue){
										if($subCatName=='continuous infusion' || $subCatName=='medications'){ //From mar 
											...
											if(!empty($subOptionValue['NewCropPrescription']['strength']))
												$unit  = " <i style='float:right'>".$subOptionValue['NewCropPrescription']['strength']."</i>" ;
										}else{
											$subOptionValueID =$subOptionValue['id'] ;
											$subOptionValueName  = $subOptionValue['name'] ;
											$isMar= false ;
									...
									<li><table>
											<tr> 
												<td class="container" style="width: 155px;" title="<?php echo $subOptionValue['name'] ;?>">
													<?php 														
												...
												<?php  for($h=$currentHr;$h >=0;$h--){
													$className = $dateString."_".$h;
													$timeBar = '' ; 
												 	$currentDay =  $dateArray[$dateString][$h][$optionValue['ReviewSubCategory']['id']][$subOptionValueID]['value'] ;
													...
													if($h == 7){
														...
														$total11 = array_sum($datewiseSubCat[$dateString]['shift_sub_option']['night'][$subOptionValueID]) ;
														$timeBar .= ($total11>0)?$total11:'' ;
														$timeBar .= "</div></td>" ;
													}else if($h == 14){
														//morning
														$timeBar = "<td   ><div class='time-area'>";
														$total12 = array_sum($datewiseSubCat[$dateString]['shift_sub_option']['morning'][$subOptionValueID]) ;
														$timeBar .= ($total12>0)?$total12:'' ;
														$timeBar .= "</div></td>" ;
													}else if($h == 22){
														//afternoon
														$timeBar = "<td  ><div class='time-area'>";
														$total13 = array_sum($datewiseSubCat[$dateString]['shift_sub_option']['day'][$subOptionValueID]) ;
													...
													if(!empty($currentDay)) {
														...
														$patientDetailID = 'detail-id="'.$dateArray[$dateString][$h][$optionValue['ReviewSubCategory']['id']][$subOptionValueID]['id'].'"' ; // ID of current cell
													}else{
														$marClass = 'cell-color' ;
														$rightClickClasses = '' ;
														$patientDetailID = '' ;
													...
													if($dateArray[$dateString][$h][$optionValue['ReviewSubCategory']['id']][$subOptionValueID]['flag']==1){ //display  flag
														$flagHtml =  $this->Html->image('icons/flag.png',array('class'=>'flag','title'=>$dateArray[$dateString][$h][$optionValue['ReviewSubCategory']['id']][$subOptionValueID]['flag_date']));
													}else{
														$flagHtml = '' ;
													}
													
													if($dateArray[$dateString][$h][$optionValue['ReviewSubCategory']['id']][$subOptionValueID]['is_edited']){ //display triangle
														$triagleHtml =  $this->Html->image('icons/triangle.png',array('style'=>'float:right;','title'=>$$dateArray[$dateString][$h][$optionValue['ReviewSubCategory']['id']][$subOptionValueID]['edited_on']));
													}else{
														$triagleHtml = '' ;
													}
													?>
												<td>
														...
														echo $flagHtml ;
														echo $triagleHtml ; 
														$is_deleted  = $dateArray[$dateString][$h][$optionValue['ReviewSubCategory']['id']][$subOptionValueID]['is_deleted'] ;
														if($currentDay > 0 || $is_deleted==1){
															if($is_deleted==1) echo "In Error" ;
															else echo ($currentDay > 0)?$currentDay:'&nbsp;' ;
														}else{
															echo $marDataArray[$dateString][$h][$optionValue['ReviewSubCategory']['id']][$subOptionValueID].'&nbsp;';
														}  
		...
		<td>
			<table>
				<tr >
					...
					<td class="size50">
						<div>
							<?php   $balanceIntakeSum  = array_sum($datewiseSubCat[$dateString][$h]['intake']) ;
									$balanceOutputSum  = array_sum($datewiseSubCat[$dateString][$h]['output']) ;
