...
echo $this->Html->script(array('jquery.fancybox-1.3.4'));?>
   ...
   <table border="0" class="table_format_body" cellpadding="0"
                ...
                <tr class="row_title">
    <td class="table_cell"><strong><?php echo __(' Name', true); ?></td>    
  ...
  ?><?php  //debug($personval); exit;?>
   ...
   <input type="hidden" name="<?php echo $orderresult['OrderDataMaster']['name'];?>" value="<?php echo $orderresult['OrderDataMaster']['name'];?>" id="<?php echo $key;?>">
   
   <?php echo $this->Html->link(__($orderresult['OrderDataMaster']['name']),'#',
   		array('onclick'=>'ordersentences("'.$patient_id.'","'.$category.'","'.$orderresult['OrderDataMaster']['id'].'","'.$orderresult['PharmacyItem']['drug_id'].'")','class'=>'','div'=>false,'label'=>false,"id"=>"result".$orderresult['OrderDataMaster']['id']));
			?>
			</td>
