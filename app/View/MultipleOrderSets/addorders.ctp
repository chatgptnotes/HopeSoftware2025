...
<h3><font color="red">Find cannot be empty.</font></h3>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0"
		...
		<tr>
			...
			echo  $this->Js->link('<input type="button" value="Search" class="blueBtn" id="search">', array('controller'=>'MultipleOrderSets', 'action'=>'orderresults', 'admin' => false),
...
$(document).ready(function(){
	...
	var categoryOrderId=$("#category option:selected").val();	
	var ajaxUrl = "<?php echo $this->Html->url(array("controller" => "MultipleOrderSets", "action" => "addorders","admin" => false)); ?>"+"/"+p_id+"/"+categoryOrderId;
         $.ajax({	
        	 beforeSend : function() {
         ...
         url: ajaxUrl,
          ...
          success: function(data){
        	  ...
        	  window.location.href = '<?php echo $this->Html->url("/MultipleOrderSets/addorders"); ?>'+"/"+p_id+"/"+categoryOrderId;	
	       		$("#resultorder").html(" ");  
          },
...
function ordersentences(id,category,loinc,lab_id) { 
	...
	$
			.fancybox({
				...
				'transitionOut' : 'fade',
				'type' : 'iframe',
				'href' : "<?php echo $this->Html->url(array("controller" => "MultipleOrderSets", "action" => "ordersentence")); ?>"+"/"+id+"/"+category+"/"+loinc+"/"+lab_id+"/"+toCheckExistingSenctence,
