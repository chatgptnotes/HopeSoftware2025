...
$(function() {
 ...
 $("#to").datepicker({
	...
	jQuery('#consultant_type').change(function() {  
	  if(jQuery('#consultant_type').val() == 1) {
	   jQuery('#showTreatingConsultant').show();
	   jQuery('#showExternalConsultant').hide();
	  } else if(jQuery('#consultant_type').val() == 2) {
	    jQuery('#showExternalConsultant').show();
		jQuery('#showTreatingConsultant').hide();
	  } else {
	   jQuery('#treating_consultant').val('');
	   jQuery('#external_consultant').val('');
	   jQuery('#showTreatingConsultant').hide();
	   jQuery('#showExternalConsultant').hide();
	  }
	});
...
<h3>&nbsp;<?php echo __('Doctor Wise Billing', true); ?></h3>

</div>
 <form name="reportfrm" id="reportfrm" action="<?php echo $this->Html->url(array("controller" => "reports", "action" => "doctorwise_collection")); ?>" method="post" >
 <table align="center">
	...
	<tr id="toDate">
	 ...
	 <tr>
	 <td colspan="8" align="right"><?php echo __('Consultant Type'); ?></td>
	 <td><b>:</b></td>
	 <td colspan="8" align="left">
	  <?php 
             echo $this->Form->input('consultant_type', array('id'=>'consultant_type','label'=> false, 'div' => false, 'error' => false, 'options'=>array(''=>'All','1'=>'Treating Consultant','2'=>'External Consultant')));
          ?>
	 </td>
	</tr>

	 <tr id="showTreatingConsultant" style="display:none">
	 <td colspan="8" align="right"><?php echo __('Treating Consultant'); ?></td>
	 <td><b>:</b></td>
	 <td colspan="8" align="left">
	  <?php 
             echo $this->Form->input('treating_consultant', array('id'=>'treating_consultant','label'=> false, 'div' => false, 'error' => false, 'options'=>$consultantList, 'empty' => 'All'));
          ?>
	 </td>
	</tr>
	<tr id="showExternalConsultant" style="display:none">
	 <td colspan="8" align="right"><?php echo __('External Consultant'); ?></td>
	 <td><b>:</b></td>
	 <td colspan="8" align="left">
	  <?php 
             echo $this->Form->input('external_consultant', array('id'=>'external_consultant','label'=> false, 'div' => false, 'error' => false, 'options'=>$externalConsultantList, 'empty' => 'All'));
          ?>
	 </td>
	</tr>
