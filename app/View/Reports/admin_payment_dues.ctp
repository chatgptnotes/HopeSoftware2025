<?php echo $this->Html->script(array('jquery.autocomplete.js'));
...
<script>
	...
	jQuery('#admission_type').change(function() {  
	  if(jQuery('#admission_type').val() == "OPD") {
	   ...
	   jQuery('#showIpdPatientStatus').reset();
	    ...
	    jQuery('#showSkipRegistration').reset();
	    jQuery('#showOpdPatientStatus').reset();
	  } else {
	   jQuery('#showSkipRegistration').hide();
	   jQuery('#showOpdPatientStatus').hide();
	   jQuery('#showIpdPatientStatus').hide();
...
<h3>&nbsp;<?php echo __('Payment Receivable', true); ?></h3>

</div>
 <form name="reportfrm" id="reportfrm" action="<?php echo $this->Html->url(array("controller" => "reports", "action" => "payment_dues")); ?>" method="post" >
 <table align="center">
	...
	<tr id="showSkipRegistration" style="display:none;">
	 ...
	 <tr>
	 <td colspan="8" align="right"><?php echo __('Sevice Code'); ?><!-- <font color="red">*</font> --></td>
	 <td><b>:</b></td>
	  <td colspan="8" align="left">
	   <?php 
             echo $this->Form->input('sevice_account_code', array('id'=>'sevice_account_code','label'=> false, 'div' => false, 'error' => false));
           ?>
	  </td>
	</tr>
      </table>
 ...
 $(document).ready(function(){
		...
		$("#sevice_account_code").autocomplete("<?php echo $this->Html->url(array("controller" => "app", "action" => "getDualAutoComplete","TariffList","name","service_account_code","admin" => false,"plugin"=>false)); ?>", {
			width: 250,
			selectFirst: true,
			isIdValue:true,
			select: function(e){ }
