<?php
	...
	if(empty($facilityname)) {
	  	$facilitynamewithreport = 'Patient Admissions Report'; 
	  	$facilityname = "";
	}
	else {
	    $headerString = "by ".$facilityname ;
		$facilitynamewithreport = $facilityname.'-Patient Admissions Report'; 
	}
	...
	$tcpdf->SetHeaderData($headerimage, PDF_HEADER_LOGO_WIDTH, $facilitynamewithreport, $headerString, $this->DateFormat->formatDate2Local(date('Y-m-d H:i'),Configure::read('date_format'), true));
	...
	$html ='<div style="margin-top:200px;">&nbsp;<span>Date: '.$this->DateFormat->formatDate2Local(date('Y-m-d'),Configure::read('date_format'), false).'</span><br><br></div>
			<table border="1" class="table_format"  cellpadding="2" cellspacing="0" width="100%" style="text-align:left;padding-top:50px;">				  
				  <tr class="row_title" style="background-color:#A1B6BE;">
					   ...
					   <td height="30px" align="center" valign="middle" width="9%"><strong>Date Of Reg.</strong></td>
					   <td height="30px" align="center" valign="middle" width="10%"><strong>MRN</strong></td>
					   <td height="30px" align="center" valign="middle" width="12%"><strong>Patient Name</strong></td>  
					   <td height="30px" align="center" valign="middle" width="10%"><strong>Blood Group</strong></td>
					   <td height="30px" align="center" valign="middle" width="8%"><strong>Age</strong></td>
					   <td height="30px" align="center" valign="middle" width="8%"><strong>Sex</strong></td>
					   <td height="30px" align="center" valign="middle" width="10%"><strong>Sponsor Name</strong></td>
					   <td height="30px" align="center" valign="middle" width="8%"><strong>Patient Type</strong></td>
					   <td height="30px" align="center" valign="middle" width="12%"><strong>Specilty</strong></td>
					   <td height="30px" align="center" valign="middle" width="15%"><strong>Doctor</strong></td>'; 
	      ...
	      if(count($reports[0]) > 0) {
			   $k = 1; 
	      		foreach($reports as $pdfData){	
					...
					if($pdfData['Corporate']['name']) $patientorg =  $pdfData['Corporate']['name']; 
					elseif($pdfData['InsuranceCompany']['name']) $patientorg = $pdfData['InsuranceCompany']['name'];
					else $patientorg = __('Private');
					...
					$html .= '<tr>
								    ...
								    <td align="center" height="17px">'.$patientorg.'</td>';
									if($patient['is_emergency'] == 1 AND $patient['admission_type'] == 'IPD'){									
										$html .= '<td align="center" height="17px">Emergency</td>';
									} else {
										$html .= '<td align="center" height="17px">'.$patient['admission_type'].'</td>';
									}

									$html.='
									    <td align="center" height="17px">'.$pdfData['Department']['name'].'</td>
...
//pr($html);exit;
	...
	echo $tcpdf->Output('Patient_Admissions_Report'.$this->DateFormat->formatDate2Local(date('Y-m-d H:i:s'),Configure::read('date_format'), true).'.pdf', 'D');
