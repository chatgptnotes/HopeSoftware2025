<?php
	...
	if(empty($facilityname)) {
	  	$facilitynamewithreport = 'ICU Utilization Rate'.' - '.$reportYear; 
	  	$facilityname = "";
	}
	else {
	    $headerString = "by ".$facilityname ;
		$facilitynamewithreport = $facilityname.'-ICU Utilization Rate'.' - '.$reportYear; 
	}
	...
	$html ='<div style="margin-top:200px;">&nbsp;<span>Date: '.$this->DateFormat->formatDate2Local(date('Y-m-d H:i:s'),Configure::read('date_format'), false).'</span><br></div>
		...
		<table border="1" cellpadding="0" cellspacing="0" width="100%" >	
			...
			<tr style="background-color:#A1B6BE;" class="row_title">
				<td style="padding-left:10px;" height="30px"></td>';
			...
			foreach($yaxisArray as $key => $yaxisArrayVal) {
				...
				$html .= '</tr>
						<tr>
							<td style="padding-left:10px;text-align:center;"><strong>ICU Utilization Rate</strong></td>';
			foreach($yaxisArray as $key => $yaxisArrayVal) {
				...
				$html .= '</tr>
						  <tr>
							<td style="text-align:center;"><strong>Total ICU Utilization Rate</strong></td>
							<td>'.$icuUtilizationRate.'</td>
							<td style="text-align:center;"><strong>Out Of</strong></td>
							<td colspan="10">'.$totalHoursIcu.'  ICU Hours</td>
						  </tr>
						<tr>
							<td style="text-align:center;"><strong>Total IPD Hours</strong></td> 
							<td colspan="12">'.$totalHoursIpd.'  Hours</td>
						</tr>
						</table>';
	//pr($html);exit;
	...
	echo $tcpdf->Output('ICU_Utilization_Rate'.$this->DateFormat->formatDate2Local(date('Y-m-d H:i:s'),Configure::read('date_format'), true).'.pdf', 'D');
?>