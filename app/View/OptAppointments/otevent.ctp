           ...
           var DATA_FEED_URL = "<?php echo $this->Html->url('/OptAppointments/datafeed/'); ?>";
            ...
            function cal_afterrequest(type)
            {
                switch(type)
                {
                    ...
                    case 3:
                    case 4:
                        window.location = "<?php echo $this->Html->url(array("controller" => "opt_appointments", "action" => "otevent", $patient_id));?>";
            ...
            function Edit(data)
            {
               var eurl="<?php echo $this->Html->url('/opt_appointments/ot_editevent/'); ?>?id={0}&start={2}&end={3}&isallday={4}&title={1}&patient_id={10}";
                if(data)
                {
                    var url = StrFormat(eurl,data);
                    OpenModelWindow(url,{ width: 1090, height: 620, caption:"Schedule  OR Appointment",onclose:function(){
            ...
            jQuery("#faddbtn").click(function(e) {
                var url ="<?php echo $this->Html->url('/opt_appointments/ot_editevent/?patient_id='.$patient_id); ?>";
