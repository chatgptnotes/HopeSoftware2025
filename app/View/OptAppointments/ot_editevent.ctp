			...
			<form
			 ...
			 ?>
						<input MaxLength="10" class="required date" id="stpartdate"
							...
							type="text" value="<?php echo $sdate; ?>" readonly="readonly" /> <input
	    ...
	    <div class="twoCol">
            ...
            <?php } ?>
				...
				<?php if($this->Session->read('website.instance') == 'vadodara'){?>
					...
					</span> <?php echo $this->Form->input(null,array('name' => 'dummy_surgery', 'type'=>'text','id'=> 'dummy_id', 'label' => false,'style'=>'width:190px;', 'class'=> 'required safe'));?>
					</label>
					</div>
				<?php }?>
				...
				<div class="twoCol" style="height: 20px; width: <?php echo $width?>;">
					...
					if($checked==0)
						...
						$checkboxfinalValue=$selectedServices[$key];
					else
						...
						<?php  if(!isset($otDateTime[0])){
    						...
    						<?php echo $this->Form->input(null, array( 'name' => 'otintime', 'id' => 'otintime', 'label'=> false, 'div' => false,'error' => false,
    								'class'=>$addClass.' allottime editable', 'options'=> $timeOptions, 'default' => isset($otDateTime[1])?$otDateTime[1]:''));
                                        ...
                                        echo $otDateTime[0].' '.$otDateTime[1];
                                    } ?>
					</div>
					<?php if($this->Session->read('website.instance') != 'kanpur'){?>
					<div style="width: 270px; float: left;">
...
$(document).ready(function(){
	...
	if(websiteInstance == 'vadodara'){
		...
		$("#calendarPatientNameText").autocomplete("<?php echo $this->Html->url(array("controller" => "app", "action" => "autocompleteForPatientNameAndDob",0,'Patient.admission_type=IPD','Patient.admission_id',"admin" => false,"plugin"=>false)); ?>", {
			width: 250,
			selectFirst: true,
			valueSelected:true,
			showNoId:true,
