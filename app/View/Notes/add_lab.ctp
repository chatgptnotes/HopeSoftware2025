<?php 
if($ajaxHold!='Yes'){
echo $this->Html->script(array('jquery-1.9.1.js','jquery-ui-1.10.2.js','validationEngine.jquery','jquery.ui.timepicker','jquery.validationEngine2',
	...
	<table width="99%" cellpadding="0" cellspacing="1" border="0"
		...
		<tr>
			<td width="30%" valign="top">
				...
				<div style="float: right; margin-right: 100px; margin-top: 6px;"
					id="">
		...
		<tr>
		 ...
		 -->
					...
					<tr id="appendQuestions_0">
						...
						<td width="19%"><?php  
						echo $this->Form->input('LaboratoryToken.0.icd9_code',array('class'=>'textBoxExpnd icd9_code','options'=>$diagnosesData,'id'=>'icd9_code_0','div'=>false,'label'=>false,'selected'=>$labRad));
	...
	<table width="99%" cellpadding="0" cellspacing="0" border="0"
		...
		<tr>
			<td align='right' valign='bottom' colspan='2'><?php if($flagSbar != 'sbar' ){
				if($noteId=='null'){
			if($returnUrl=='IPD'){
			echo $this->Html->link(__('Cancel'),array("controller"=>'notes',"action"=>'soapNoteIpd',$patientId),array('id'=>'labsubmit1','class'=>'blueBtn'));
			}else{
			echo $this->Html->link(__('Cancel'),array("controller"=>'notes',"action"=>'soapNote',$patientId),array('id'=>'labsubmit1','class'=>'blueBtn'));
					...
					if($returnUrl=='IPD'){
	echo $this->Html->link(__('Cancel'),array("controller"=>'notes',"action"=>'soapNoteIpd',$patientId,$noteId),array('id'=>'labsubmit1','class'=>'blueBtn'));
}else{
...
jQuery(document).ready(function(){
	...
	$("#test_name").autocomplete("<?php echo $this->Html->url(array("controller" => "Laboratories", "action" => "labRadAutocomplete","Laboratory",'id',"dhr_order_code","name","admin" => false,"plugin"=>false)); ?>", {
		width: 250,
		selectFirst: true,
		valueSelected:true,
		showNoId:true,
	...
	if(validateMandatory == false){ 
		return false;
		 ...
		 var ajaxUrl = "<?php echo $this->Html->url(array("controller" => "notes", "action" => "addLab",
		 		$patientId,$noteId,'null',$appointmentId,"admin" => false)); ?>"; 
		   ...
		   $.ajax({
				...
				success: function(data){ 
					...
					var returnUrl="<?php echo $returnUrl;?>"
						if(returnUrl=='IPD'){
							window.location.href='<?php echo $this->Html->url(array("controller"=>'notes',"action" => "soapNoteIpd",$patientId));?>'+'/'+noteId
						}
