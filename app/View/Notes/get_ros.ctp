	...
	<tr width="100%">
		<td><table>
				...
				<tr style="display: none" id='editRosAdd'>
					...
					<td><?php echo $this->Html->link('Cancel','javascript:void(0)',array('type'=>'button','label'=>false,'name'=>'Cancel','id'=>'CancelRosEdit','class'=>'Bluebtn')); ?>
					</td>
				</tr>
				<table id='rosTable'>
...
$('#addRos').click(function(){
	var copyText=$('#searchRos').val();
	$('#rosText').val(copyText);
	$('#showRosAddd').show();
	$('#showRosAdd1').hide();
});
...
$('.editR').click(function(){
	...
	var currentVal=$('#subShowRos').val();
	var currentValClincal=$('#rosTextNew').val();
	$('#subShowRos').val(currentVal+" "+id1);
	$('#rosTextNew').val(currentValClincal+" "+id1);
	$('#subShowRos').focus();
	id1="";
});
//***********************Save Template Text********************************
$('#addRosData').click(function(){
	var contentType="review of system";
...
$('#editRosData').click(function(){
	...
	var contentTypeR="review of system";
...
$('.deleteRos').click(function(){
    ...
    type: 'POST',
    url: ajaxUrl+"/"+spData[0]+"/"+spData[1]+"/"+"review of system",
    success: function(data){
    	$('#busy-indicator').hide('slow');
    	  $('#rosDisplay').html(data);
    },
...
function searchTemplateRos(){
    ...
    type: 'POST',
    url: ajaxUrl+"/"+searchString+'/'+'review of system'+"/"+templateID,
    success: function(data){
    	$('#busy-indicator').hide('slow');
