...
<?php echo $this->Form->create('AddRad',array('action'=>'addLab','id'=>'Radfrm','type'=>'post'));?>
	...
	<!--   	<tr>
		...
		<tr>
			...
			<td width="19%"><?php //if($this->Session->read('role')=='Primary Care Provider'){
						...
						//}	
			echo $this->Form->input('RadiologyTestOrder.primary_care_pro', array('class'=>'textBoxExpnd','id' => 'primary_care_pro','type'=>'text','label'=>false,'value'=>$getDocName )); ?>
			</td>
			<td>&nbsp;</td>
			<td>&nbsp;</td>
		</tr>
		...
		<tr>
			...
			if($noteId=='null'){
if($returnUrl=='IPD'){
			 echo $this->Html->link(__('Cancel'),array("controller"=>'notes',"action"=>'soapNoteIpd',$patientId),array('id'=>'labsubmit1','class'=>'blueBtn')); echo $this->Form->submit(__('Submit'),array('disabled'=>'disabled','id'=>'radsubmit','class'=>'grayBtn','onclick'=>"javascript:save_rad(); return false;",'div'=>false)); 
			}else{
echo $this->Html->link(__('Cancel'),array("controller"=>'notes',"action"=>'soapNote',$patientId),array('id'=>'labsubmit1','class'=>'blueBtn')); echo $this->Form->submit(__('Submit'),array('disabled'=>'disabled','id'=>'radsubmit','class'=>'grayBtn','onclick'=>"javascript:save_rad(); return false;",'div'=>false));
				}
				}
			else{
				echo $this->Html->link(__('Cancel'),array("controller"=>'notes',"action"=>'soapNote',$patientId,$noteId),array('id'=>'labsubmit1','class'=>'blueBtn')); echo $this->Form->submit(__('Submit'),array('disabled'=>'disabled','id'=>'radsubmit','class'=>'grayBtn','onclick'=>"javascript:save_rad(); return false;",'div'=>false));
}?>
			</div>
			</td>
		</tr>
	           ...
	           url: ajaxUrl,
	            ...
	            success: function(data){
		            if($.trim(data)=='sbar'){
			            ...
			            if(returnUrl=='IPD'){
			            	window.location.href='<?php echo $this->Html->url(array("controller"=>'notes',"action" => "soapNoteIpd",$patientId));?>'+'/'+fromNoteId
			            } else if('<?php echo $ajaxHold?>'=='Yes'){
				            alert('Save');
			            }
			            else{
