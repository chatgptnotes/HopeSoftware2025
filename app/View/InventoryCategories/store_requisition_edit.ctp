...
<div class="inner_title">
	<h3>Store Requisition Slip - Edit</h3>
	<span><?php   
		if(!empty($this->params->query['page']) && $this->params->query['page'] == "store_inbox_requistion_list"){
			echo $this->Html->link(__('Back'), array('action' => 'store_inbox_requistion_list'), array('escape' => false,'class'=>"blueBtn"));
		}else{
			echo $this->Html->link(__('Back'), array('action' => 'store_requisition_list'), array('escape' => false,'class'=>"blueBtn"));
		}
	?>
	</span>
</div>
...
<table width="100%" border="0" cellspacing="1" cellpadding="0"
	...
	<tr>
		...
		<td width="18%" colspan=""><?php echo $this->Form->input('StoreRequisition.requisition_for',
				array('label'=> false,'div' => false, 'error' => false,'empty'=>'Select Other Location','value'=>$requsition_for,
                                'options'=>$department,'div'=>false,'class'=>'requisition_selector validate[required]','value'=>$StoreRequisition['StoreRequisition']['requisition_for']));
				echo $this->Form->hidden('StoreRequisition.requisition_for_name',array('id'=>'for_name','value'=>$storeLocation['StoreLocation']['name']));?>
			<!-- <label><input type="radio" value="ward" name="requisition_for" class="requisition_selector" checked = <?php if($selectedRadio == 'ward')echo 'checked'; ?> >Ward</label>
	             <label><input type="radio" value="ot" name="requisition_for" class="requisition_selector" checked = <?php if($selectedRadio == 'ot')echo 'checked'; ?> > OT</label>
	            <label><input type="radio" value="chamber" name="requisition_for" class="requisition_selector" checked = <?php if($selectedRadio == 'chamber')echo 'checked'; ?> > Chamber</label>
	  	       <label><input type="radio" value="other" name="requisition_for" class="requisition_selector" <?php if($selectedRadio == 'other')echo 'checked'; ?> > Other</label> -->
		</td>
...
<table width="100%" border="0" cellspacing="1" cellpadding="0"
	...
	<tr>
		...
		<th width="110" align="center">Sale Price</th>
        <th width="150" align="center">Stock Quantity (MSU)</th>
	...
	<tr id="row<?php echo $i;?>">
		...
		<td><input name="StoreRequisition[slip_detail][qty][]" type="text"
			class="textBoxExpnd validate[required]"
		 ...
		 <td align="center" id="salePrice<?php echo $i;?>"><?php echo number_format($value['ProductRate']['sale_price'],2);?> </td>
		 <td align="center" id="stock<?php echo $i;?>"><?php echo $msu = $value['Product']['quantity'] * (int)$value['Product']['pack'] + $value['Product']['loose_stock']; ?>       
         <td align="center" id="limit<?php echo $i;?>"> <?php $reorderLevel = !empty($value['Product']['reorder_level']) ? $value['Product']['reorder_level']: 0; 
         			echo $msu - (($msu * 0.1) + $reorderLevel); ?> </td>
...
<script>
	...
	jQuery("#storeRequisition").validationEngine();
	});
...
function addFields(){
		   ...
		   field += ' <td><input name="StoreRequisition[slip_detail][qty][]" type="text" class="textBoxExpnd validate[required] quantity" id="qty'+number_of_field+'" fieldNo="'+number_of_field+'" style="width:110px;"/> </td>';
...
function removeRow(){
 ...
 $(document).ready(function(){
	 ...
	 var requisitionTo = $("#storeLocation").val(); 
		$(".requisition_selector").each(function(){
			$('#StoreRequisitionRequisitionFor option').each(function() {
			    if ($(this).val() == requisitionTo ) {
			        $(this).remove();
			    }
			});		
		});
