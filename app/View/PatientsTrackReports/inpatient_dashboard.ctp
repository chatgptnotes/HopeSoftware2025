...
<div class="inner_title">
	<h3><?php echo __('Inpatient Summary'); ?></h3>
</div>
<?php }else{?>
<div class="inner_title">
	<h3><?php echo __('Clinical Summary'); ?></h3>
</div><?php }?>
	...
	switch (strtolower($column['Widget']['title'])) {
		...
		case 'medications':
			if($patient['Patient']['admission_type']=='IPD'){
				...
				$medicationLink = $this->Html->link($this->Html->image('icons/plus_6.png'), array('controller'=>'patients','action' => 'patient_notes',$patientId), array('escape' => false,'title'=>'Add Medication'));
				echo medication($newCropPrescriptions,$boxHtml,$medicationLink);
				break;
			}
		case 'past medical':
		...
		if(!empty($getEkg) ){
			...
			foreach ($getEkg as $dataEkg){
				...
				$dignosticHtml.= '<tr class="light">
					<td width="300px">'.$dataEkg['EKG']['history'].'<span style="color:grey; font-size:13px;">( '.$dataEkg['EkgResult']['result_publish_date'].' ) </td>
									<td>'.$status.' </td>
									</tr>';
			}
			$dignosticHtml.='</table>';
				...
				$dignosticHtml.='<table>
									<tr><td><span style="color:grey; font-size:13px;">No Result Found</td></tr>
									</table>';
			}
...
function past_med_his($getPastMedicals,$boxHtml){
	...
	$medicationHtml.= '<tr><td align="right"><a href="#" id="pres" onclick="getRxHistory()">View Rx History</a></td></tr></table>';
	}else{
