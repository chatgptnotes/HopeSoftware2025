<?php 
	...
	//BOF-Calculate Gross Profit
	#debug($getExpense[1]);
	#debug($getIncome[1]);
	$getGrpSumExp=(int)$getExpense[1]; ////10000-For Opening Stock
	$getGrpSumInc=(int)$getIncome[1];////10000-For Closing Stock
	#debug($getGrpSumExp);
	#debug($getGrpSumInc);
...
<tr> 
<td width="10%" style="font-weight:900;font-size: 15px !important;padding-left:20px;">Total</td>
...
<script>
$(document).ready(function(){	console.log(window.frames);
	$('#tree').tree({
	...
	$(".subchldDiv1").dblclick(function() {		
		...
		$.fancybox({
			...
			'transitionOut' : 'fade',
			'type' : 'iframe',
			'href' : getLedgerTransactionsURL + '/' +"<?php echo Configure::read('profit_loss_statement');?>" + '/' + id + '/' + locationId + '/' + from + '/' + to
		});
	});
});
