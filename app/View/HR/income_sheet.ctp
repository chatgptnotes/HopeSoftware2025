...
<?php echo $this->Form->create('AccountReceipt',array('type' => 'GET',
		'url' => array('controller' => 'HR', 'action' => 'income_sheet'),'class'=>'manage','style'=>array("float"=>"left","width"=>"100%"),'id'=>'HeaderForm','inputDefaults' => array(
																								        'label' => false,
																								        'div' => false,
																								        'error' => false
																								    )
...
$(document).on('click','.checkboxRow', function() {	
	if(confirm("Do you really want to delete this record?")){	
		...
		ID=splitedId['1'];
				...
				var htmlvalue=$("#totalperDayIncome_"+ID).html();							
				var floatVal=parseFloat(htmlvalue);					
				var totalPerDayExpAmt=$("#total_per_day_income").val();
				totalValue=totalPerDayExpAmt-floatVal;
				totalValue=Number(totalValue).toFixed(2);    
				$("#total_per_day_income").val(totalValue);
				$("#totalperDayIncomeval").show().html(totalValue);	
...
$(document).on('click','.ChkBoxMonthlySelectAll', function() { 	 
	...
	//	currentId=$(this).attr('id'); 
		...
		$(".col_"+ID).each(function(){
				...
				$("#totalperDayIncome_"+rowID).show().html(eachPerDayExp);
				$("#monthavgperday_"+rowID).val(eachPerDayExp);	
				//perDayExpArray.push(eachPerDayExp);	
				//totalExpValue=Calc_totals(perDayExpArray);				
				tCount++;	
		...
		$("#totalperDayIncomeval").show().html(totalExpValue);
		$("#total_per_day_income").val(totalExpValue);
		
		$("#showHeading").show().html("Incomes For Last "+calColMonth+" Months");
		...
		//  window.location.href = '<?php echo $this->Html->url("/HR/income"); ?>';
...
////BoF-Calculating total Per Day Income
...
$(document).ready(function(){
	//load all records from here using ajax
		$("#Submit,#refreshBtn").click(function(){ 
			...
			$.ajax({
				url: '<?php echo $this->Html->url(array('controller'=>'HR','action'=>'income_list'));?>',
	...
	$(document).on('blur','.commentTxtCls',function(){
		...
		var htmlData = '';
		$.ajax({
   			url : "<?php echo $this->Html->url(array("controller" => 'HR', "action" => "edit_income", "admin" => false));?>",
   			type: 'POST',
