...
<div class="inner_title">
	<h3>
		<?php echo __('Balance Sheet', true); ?>
	</h3>
	<span>
	<?php   if($flag=='fromAcc'){
...
<?php echo $this->Form->create('',array('type' => 'POST',
		'url' => array('controller' => 'HR', 'action' => 'balanceSheetStatement'),'class'=>'manage','style'=>array("float"=>"left","width"=>"100%"),'id'=>'HeaderForm','inputDefaults' => array(
																								        'label' => false,
																								        'div' => false,
																								        'error' => false
																								    )
...
$(document).ready(function(){
	$("#print").click(function() {
		 ...
		 var url="<?php echo $this->Html->url(array('controller'=>'HR','action'=>'balanceSheetStatement','print')); ?>"+ queryString;
	...
	$("#printGroup").click(function() {
		 ...
		 var tran_date_to=$("#to_date").val();
		 var queryString = "?from_date="+tran_date_from+"&to_date="+tran_date_to;
		 var url="<?php echo $this->Html->url(array('controller'=>'HR','action'=>'balanceSheetStatement','printGroup')); ?>"+ queryString;
		 window.open(url, "_blank","toolbar=0,scrollbars=1,location=0,status=1,menubar=0,resizable=1,width=1200,height=600,left=200,top=200");
	});
	...
	// $(document).on( 'click', '#Submit,#refreshBtn', function() {	
		$("#Submit,#refreshBtn").click(function(){ 
			...
			if(currentId=="refreshBtn"){		
				...
				$.ajax({
						url: '<?php echo $this->Html->url(array('controller'=>'HR','action'=>'balanceSheetReport'));?>',
						type: 'POST',
	...
	//on load render balanceSheetReport
	$.ajax({
		url: '<?php echo $this->Html->url(array('controller'=>'HR','action'=>'balanceSheetReport'));?>',
		type: 'POST',
		data:$('#HeaderForm').serialize(),
