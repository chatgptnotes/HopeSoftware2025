...
.formFull{border-bottom: 1px solid #ccc !important;}
</style>
</head>
<body>
...
<div style="text-align: right;" id="printButton">
    ...
    <tr>
        <td class="no-print"> 
            <button id="printButton1" class="save-btn" style="background: red;">Send Whatsapp</button>
        </td>
        ...
        <?php } else { ?>
        <td class="no-print">
            <?php echo $this->Html->link('Print','#', array('onclick'=>'', 'class'=>'grayBtn', 'escape'=>false));?>
        </td>
        <?php } ?>
    </tr>
</table>
			...
			<thead>
				<tr>
					<td>
						<table width="100%" border="0" align="center" cellspacing="0"
							...
							style="margin: 0px; padding: 0px; border-top: 1px solid #ccc; border-left: 0px solid; border-right: 0px solid; padding-bottom: 0px; margin-bottom: 0px">
							<tr>
								<td valign="top">
				...
				<?php //if($getPanelSubLab[0]['Laboratory']['lab_type'] == '2'){?>
				 <tr>
											...
											echo $this->DateFormat->formatDate2Local($getPanelSubLab[0]['LaboratoryTestOrder']['start_date'],Configure::read('date_format_us'),true);
											?></td>
										</tr>
				<?php //}?>
				<tr>
...
$admisionDate = $patientData['Patient']['form_received_on'];
...
<?php if($patientData['Patient']['tariff_standard_id'] != 7 && strtotime($endDate) > strtotime(date('Y-m-d H:i:s'))){ ?>
...
$initSero++;
    ...
    $filteredMethods = array_filter($labMethod);
    echo implode(" ", $filteredMethods); // Bas space se separate karega
    ?>
</td>
								</tr>
...
<tr>
									<td style="border-top: 1px solid #ccc; line-height: 1px;"
										colspan="3">&nbsp;</td>
								</tr>
								<tr>
								...
								<tr>
									<td style="border-bottom: 1px solid #ccc; line-height: 1px;"
										colspan="3">&nbsp;</td>
								</tr>
	 <?php }?>
	...
	<?php if($subData['LaboratoryHl7Result'][$paraKey]['result']){?>
		...
		<?php
			...
			<td colspan="3"><table>
			<tr>
			<!--						<td width="-10px" style="padding-bottom: 30%;" valign="bottom" class="tdLabel" id=""><b style="padding-bottom: 30%;display:none" ><?php echo __('Method');?>-->
			<!--</b></td>-->
									<td width="" valign="middle" class="tdLabel" id="">
			<?php
			$labMethod = explode("\r\n",$subData ['Laboratory'] ['test_method']);
...
<tr>
									<td 
										colspan="3"><hr style="border: 1px solid #ccc; line-height: 1px;"></td>
								</tr>
				<tr>
					<td>
						<?php if($getPanelSubLab[0]['Laboratory']['lab_type'] == '2'){?>
						...
						<table width="100%" class="labFooter" style="float: right">
							<tr>
								...
								<td align="right" width='47%'>
									<table>
										<tr>		
										<?php //if($this->Session->read('instance')=='hope'){?>	 
											<td width="100%" align="right">
												<p>
													<?php echo $this->Html->image('/img/icons/Arun Agre.jpeg',array('width'=>'100px','height'=>'100px')); ?>
												...
												<p
													style="line-height: 1px; padding-top: 5px; padding-right: 48px;"><?php  echo  strtoupper(Configure::read('pathology_doctor_authority_designation'));?></p>
											</td>
					 					<?php  //} ?>	 
										</tr>
									</table>
...
<script>
document.getElementById("printButton1").addEventListener("click", async function () {
    ...
    // ✅ Optimize Scale to Reduce Size (1.5 instead of 3)
    let scale = 1.5;  
    ...
    // ✅ Convert Image to JPEG (Less Size than PNG)
    let imgData = canvas.toDataURL("image/jpeg", 0.5);  // 50% Quality to Reduce Size

    // 🛠 Show Buttons Again After Capturing PDF
    document.querySelectorAll('.no-print').forEach(el => el.style.visibility = 'visible');

    // ✅ Image Size Fixing for Better PDF Compression
    let imgWidth = 190;
    let pageHeight = 280;
    ...
    // 🛠 Generate File Name with Patient Name & Mobile
    let orderId = "<?php echo $this->request->query['testOrderId']; ?>";
    let patientMobile = "<?php echo $patientData['Person']['mobile']; ?>";
    let patientName = "<?php echo $patientData['Person']['first_name'] . '_' . $patientData['Person']['last_name']; ?>";
    let timestamp = Math.floor(Date.now() / 1000);
    let pdfFileName = patientName + "_" + patientMobile + "_" + orderId + "_" + timestamp + ".pdf";

    // ✅ Reduce PDF Size Before Sending
    let pdfBlob = doc.output('blob');
    
    if (pdfBlob.size > 3 * 1024 * 1024) {  // ✅ Max 3MB Condition
        alert("❌ PDF size is too large. Please reduce content.");
        return;
    }
    ...
    formData.append("patientMobile", patientMobile);
    formData.append("patientName", patientName);

    fetch("<?php echo $this->Html->url(['controller' => 'NewLaboratories', 'action' => 'savePDF']); ?>", {
        method: "POST",
        body: formData
    })
