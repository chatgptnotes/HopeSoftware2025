...
<div style="text-align: right;" id="printButton">
		...
		<table align='right'>
			...
			if ($from != 'Preview') {
				if (in_array ( $this->Session->read ( 'userid' ), $authenticated )) {
					?>
			<td>
			...
			<thead>
				...
				<?php //if($getPanelSubLab[0]['Laboratory']['lab_type'] == '2'){?>
				 <tr>
											...
											<td><?php echo $this->DateFormat->formatDate2Local($getPanelSubLab[0]['LaboratoryTestOrder']['sample_received'],Configure::read('date_format_us'),true);?></td>
										</tr>
				<?php //}?>
				<tr>
...
<tr>
				...
				<tr>
											...
											<td><?php echo $this->DateFormat->formatDate2Local(date("Y-m-d H:i:s"),Configure::read('date_format_us'),true);?></td>
										</tr>
										<tr>
...
$isRegularHeaderPrinrted = $isHistoHeaderPrinted = $isSerologyHeaderPrinted = $isCultureHeaderPrinted = '';
...
<?php
		...
		if (strtoupper ( $subData ['TestGroup'] ['name'] ) == 'SEROLOGY') {
			...
			<?php if (strpos(strtoupper ($subData ['Laboratory'] ['name']),' WIDAL') !== false) {
					$repHeader = ($lastReportDepartmentName) ? $lastReportDepartmentName : $subData ['Laboratory'] ['name'];
					?>
					...
					<table align="center"><tr><td>
						<h2 align="center" style="text-decoration: underline;line-height:5px;padding-top:9px;margin-bottom:16px;<?php echo $stl;?>">
							<?php 
								if(empty($isSerologyHeaderPrinted))
									echo /* __ ( "Report on " ) .  */$repHeader;
								$isSerologyHeaderPrinted = 1;
							?>
						</h2>
						</td></tr>
						<tr>
							<td style="line-height: 5px; padding-top: 2px; padding-bottom: 2px" colspan="3">&nbsp;</td>
						</tr>
					</table>
					<div class="clr"></div>
					<table border="1px" width = "95%" cellspacing="0" cellpadding="1" align="center">
						<tr>
							<td><?php echo ($subData ['Laboratory'] ['name']);?></td>
							<?php foreach(Configure::read('WIDAL') as $key => $value){?>
								<td align="center"><?php echo $value?></td>
							<?php }?>
						</tr>
						<?php $prArray = $subData ['LaboratoryParameter'];
						...
						<tr>
							...
							<?php foreach(Configure::read('WIDAL') as $key => $value){
								...
								if(($subData ['LaboratoryHl7Result'] [$paraKey] ['result']) >= $explodeValue[1]){
									$resultValue = '+';
								}else{
									$resultValue = '-';
								}
								?>
							<td align="center"><?php echo $resultValue?></td>
							<?php }?>
						</tr>
						<?php }?>
					</table>	
...
<?php
		...
		<tr>
									<td width="20%" valign="middle" class="tdLabel" id=""><?php echo $subData['Laboratory']['notes_display_text'];?>
			</td>
...
$initSero++;
			...
			?>
			<tr>
									<td width="20%" valign="middle" class="tdLabel" id=""><?php echo __('Method');?>
			</td>
									<td width="" valign="middle" class="tdLabel" id="">
			<?php
				echo '<span>: </span><span>' . str_replace ( "\r\n", "<BR>", $subData ['Laboratory'] ['test_method'] ).'</span>';
				?>
			</td>
			...
			<tr>
									<td
										style="border-bottom: 1px solid; line-height: 5px; padding-top: 10px; padding-bottom: 2px"
										colspan="3">&nbsp;</td>
								</tr>
	<?php if($subData['LaboratoryResult']['text']){?>
		<tr>
...
<tr>
								...
								<?php if(empty($isRegularHeaderPrinrted)){?>
								<tr>
									<td style="font-weight: bold; width: 37%"><?php echo __("INVESTIGATION");?></td>
									<td style="font-weight: bold; width: 30%"><?php echo __("OBSERVED VALUE");?></td>
									<td style="font-weight: bold; width: 30%"><?php echo __("NORMAL RANGE");?></td>
								</tr>
								<tr>
	...
	<?php if($subData['LaboratoryHl7Result'][$paraKey]['result']){?>
		<?php if($initCount ==0){
			if(count($subData['LaboratoryHl7Result'])  > 1){
				$divPadding = 'padding-left: 10px;';
			?>
		<tr>
									<td width="" valign="middle" class="tdLabel" id=""
										style="width: 37%; font-size: 16px;" colspan="2"><b>
		...
		<tr>
			...
			<?php
							...
							?>
									...
									<td width="" valign="middle" class="tdLabel" id="" style="width:30%; <?php echo $isPrintedFirstTimeStyle;?>">
				<?php
							$unitData = ($optUcums [$value ['unit']]) ? $optUcums [$value ['unit']] : $value ['unit_txt'];
							echo $isPrintedFirstTime . $subData ['LaboratoryHl7Result'] [$paraKey] ['result'];
							
							?>
			</td>
									<td width="" valign="middle" class="tdLabel" id="" style="width:30%;<?php echo $isPrintedFirstTimeStyle;?>">
				<?php
							$defaultRange = trim(trim($defaultRange),"-");
		...
		<?php
			...
			<table>
		<tr>
									<td width="10%" valign="middle" style="padding-left:10px" id=""><b><?php echo ($subData['Laboratory']['notes_display_text'])?$subData['Laboratory']['notes_display_text']:__('Comments:');?></b>
									</td>
									<td width="" valign="middle" class="tdLabel" id="" colspan="2">
			<?php
				echo '' . str_replace ( "\r\n", "<BR>&nbsp;&nbsp;", $subData ['LaboratoryResult'] ['text'] );
...
<!-- END OF Histology -->
	...
	<?php if($getPanelSubLab[0]['Laboratory']['lab_type'] == '2'){?>
		...
		if ($drProfile [0] ['DoctorProfile'] ['id']) {
			foreach ( $drProfile as $key => $valueDr ) {
				if ($valueDr ['DoctorProfile'] ['education']) {
					$education = '(' . $valueDr ['DoctorProfile'] ['education'] . ')';
				} else {
					$education = '';
				}
				?>
							<td>
												<p
													style="line-height: 1px; padding-top: 25px; padding-right: 20px;"><?php echo ('Dr.'.$valueDr['DoctorProfile']['first_name'].' '.$valueDr['DoctorProfile']['last_name']);?></p>
												<p
													style="line-height: 1px; padding-top: 5px; padding-right: 20px;"><?php echo  $education;?></p>
											</td>
			...
			?>
					<td>
												<p
													style="line-height: 1px; padding-top: 25px; padding-right: 20px;"><?php echo  strtoupper(Configure::read('pathology_doctor_authority_name_histo'));?></p>
												<p
													style="line-height: 1px; padding-top: 5px; padding-right: 20px;"><?php echo  strtoupper(Configure::read('pathology_doctor_authority_designation_histo'));?></p>
											</td>
					<?php }?>
				</tr>
									</table>
	...
	?>
								...
								<td align="right" width="<?php echo $tdWidth;?>">
									<table>
										<tr>
				<?php
				...
				if ($drProfile [0] ['DoctorProfile'] ['id']) {
					...
					<td>
												<p
													style="line-height: 1px; padding-top: 25px; padding-right: 20px;"><?php echo  strtoupper(Configure::read('pathology_doctor_authority_name'));?></p>
												<p
