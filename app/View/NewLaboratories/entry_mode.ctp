...
<span>
<?php 
/****************************Sample id for data to be fetched from lab m/c result -Pooja*******************************************************************/
echo $this->Form->create ( 'sample',array ('type'=>'GET','id' => 'labsample') );?>
Lab Sample ID : <?php echo $this->Form->hidden ( '', array (
			'name' => "testOrderId",
			'type' => 'text',
			...
			'label' => false,
			'id' => 'test_sample_id',
			'class' => 'test_sample_id' 
			) );
echo $this->Form->input ( '', array (
			'name' => "lab_sample_id",
			'type' => 'text',
			'value'=>$this->params->query['lab_sample_id'],
			'div' => false,
			'autocomplete'=>'off',
			'label' => false,
			'id' => 'lab_sample_id',
...
echo $this->Html->link ( 'Back', array (
		...
		'class' => 'blueBtn',
		'style'=>"float:right;height: none !important",
		'div' => false,
		'label' => false
) );
echo $this->Form->button('Submit',array('type'=>'submit','class'=>'blueBtn','div'=>false,'label'=>false));
echo $this->Form->end();
/***************************EOF Sample id*************************************************************************************/?>
</span>
<?php echo $this->Form->create ( 'LaboratoryResult', array (
		'type' => 'file',
		'id' => 'labfrm',
		'inputDefaults' => array (
				'label' => false,
					...
					if ($value ['laboratory_categories_id'] == $labCatValue ['id']) {
						...
						if ($value ['type'] == 'text') {
							...
							if ($value ['by_gender_age'] == 'gender') {
								if (strTolower($patientData ['Person'] ['sex'])== 'male') { // if male
		...
		<tr>
			...
			<?php echo $isPrintedFirstTime;?>
				<?php
						...
						if ($isMultiple) {
      						/*************************show result availble from lab m/c ******************************************/
									if($value ['laboratory_categories_id']=='3'){//for 3rd parameter DLC
										$cbcKey=key($cbcLabRes[$value ['laboratory_categories_id']][$d]);//lab m/c data
										if(!empty($cbcLabRes[$value ['laboratory_categories_id']][$d][$cbcKey]))
											$labResultVal=$cbcLabRes[$value ['laboratory_categories_id']][$d][$cbcKey];
										else if($subData ['LaboratoryHl7Result'] [$paraKey] ['result'])
											$labResultVal=$subData ['LaboratoryHl7Result'] [$paraKey] ['result'];
										else $labResultVal=$defaultResultParam;
									
										$d++;
									}else{
										$cbcKey=key($cbcLabRes[$value ['laboratory_categories_id']]);
										if(!empty($cbcLabRes[$value ['laboratory_categories_id']][$cbcKey]))
											$labResultVal=$cbcLabRes[$value ['laboratory_categories_id']][$cbcKey];//lab m/c data
										else if($subData ['LaboratoryHl7Result'] [$paraKey] ['result'])
											$labResultVal=$subData ['LaboratoryHl7Result'] [$paraKey] ['result'];
										else $labResultVal=$defaultResultParam;
									}
							...
							if (($labResultVal < $defaultRangeBck [0]) || ($labResultVal > $defaultRangeBck [1]))
...
<script>
	...
	jQuery(document).ready(function() {
		if('<?php echo empty ($getPanelSubLab[0]['LaboratoryResult']['report_date']) || $getPanelSubLab[0]['LaboratoryResult']['report_date']=='0000-00-00 00:00:00'?>'){
		    ...
		    $(".getBlurId").trigger('blur'); //trigger event onload used while interface data fetch
...
// 	});
            $('#print').click(function(){
                ...
                console.log("Sending Data:", data); // 🔹 Check AJAX request me kya ja raha hai
                ...
                $.ajax({
                    ...
                    success:function(response){
                        $('#busy-indicator').hide();
                        console.log("Server Response:", response); //Check controller se kya response aa raha hai
