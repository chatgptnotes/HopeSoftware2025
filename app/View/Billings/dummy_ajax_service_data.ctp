...
<table width="100%" cellpadding="0" cellspacing="1" border="0" style="clear:both" class="tabularForm" id="serviceGrid">
		...
		<?php if(strtolower($isMandatory)!=Configure::read('mandatoryservices') && $isNursing!='yes'){?>
			<?php 
			/* $recordCount=count($serviceCharge);
			$flag=true;
			if(($recordCount==0 || $recordCount==1) && $serviceCharge['0']['Billing']['tariff_list_id']=='' && $serviceCharge['0']['Billing']['id'] ==''){
				$flag=false;
			}else{
				for($r=0;$r<=$recordCount;$r++){
					if($serviceCharge[$r]['Billing']['tariff_list_id']!=''){
						$flag=false;
			...
			<th class="table_cell"><?php echo __('Amount To Pay');?></th>
	...
	foreach($servicesData as $services){ 
		...
		<tr class="row" id=row_<?php echo $services['ServiceBill']['id']; ?>>			
			...
			?>
			<td class='select' style="<?php echo $display;?>"><input class="checkbox1 <?php echo $billable;?>" type="checkbox" id="<?php echo "optCheck_".$i; ?>" name="check[]" value="<?php echo $services['ServiceBill']['id'];?>" <?php echo $disabled;?> tariffId="<?php echo $services['TariffList']['id'];?>">
			<?php }?>
			<td align="">
				<?php //echo $services['ServiceBill']['date'];
				if($servicesData[0]['ServiceCategory']['name']==Configure::read('mandatoryservices')){
					if(!empty($services['Patient']['form_received_on']))echo $this->DateFormat->formatDate2Local($services['Patient']['form_received_on'],Configure::read('date_format'),true);
				}else{
			...
			<td align="right"><?php 
				echo $this->Form->input("ServiceBill.$i.id",array('type'=>'hidden','id'=>'pay_service_id_'.$services['ServiceBill']['id'],'value'=>$services['ServiceBill']['id'],
					/* 'name'=>'data[ServiceBill][][id]', */'legend'=>false,'label'=>false,'class'=>'serviceAmountToPay'));
				if($totalAmount1==$services['ServiceBill']['paid_amount'] && !empty($serviceCharge)){
					echo ($services['ServiceBill']['paid_amount']);
				}else{
					if(empty($services['ServiceBill']['paid_amount']))$services['ServiceBill']['paid_amount']=$totalAmount1;
					echo $this->Form->input("ServiceBill.$i.paid_amount",array('class' => 'validate[required,custom[onlyNumber]] textBoxExpnd paidAmtService serviceAmountToPay','type'=>'text',
						'legend'=>false,'label'=>false,'id' => 'serviceAmountToPay_'.$services['ServiceBill']['id'],'fieldNo'=>1,
						/* 'name'=>'data[ServiceBill][][amount_to_pay]', */'autocomplete'=>'off','value'=>$services['ServiceBill']['paid_amount']));
				}
			 ?></td>
			...
			<td align="center">
			<?php 
			if(empty($serviceCharge) && $discharge=='no'){
		...
		<tr class="duplicateRow" style="display:none" id=duplicateRow_<?php echo $services['ServiceBill']['id']; ?>>
			...
			<!-- 
			<td align="center"> 
				<?php echo $this->Form->input('',array('type'=>'text','class'=>'getServiceSubGroup textBoxExpnd','id'=>'add-service-sub-group_'.$services['ServiceBill']['id'],
						'label'=>false,'value'=>$services['ServiceSubCategory']['name']));
					  echo $this->Form->hidden('', array('name'=>'data[ServiceBill][sub_service_id]','id'=>'addServiceSubGroupId_'.$services['ServiceBill']['id'],
						'class'=>'addServiceSubGroupId','value'=>$services['ServiceBill']['sub_service_id']));?></td>
			 -->
			<td align="center">
				<?php  
			...
			<?php echo $this->Form->input('amount',array('class' => 'validate[required,custom[onlyNumber]] textBoxExpnd service_amount',
					...
					'name'=>'data[ServiceBill][amount]','autocomplete'=>'off','value'=>$services['ServiceBill']['amount'])); ?></td>
			...
			<?php $no_of_time = $services['ServiceBill']['no_of_times']?$services['ServiceBill']['no_of_times']:1;
				echo $editTotalAmount=$services['ServiceBill']['amount']*$no_of_time;?></td>
				
			<td align="right"><?php echo $editTotalAmount; ?></td>
	...
	<?php $i++; }?>
		...
		if($addmissionType['Patient']['admission_type']=='IPD' && strtolower($isMandatory)==Configure::read('mandatoryservices')){
			$serviceGroup='Mandatory Services';
			if($doctorCharges){?>
			<tr class="row">
				<td align="">
	...
	<td>
		<table width="100%" cellpadding="0" cellspacing="1" border="0" style="clear:both" class="tabularForm">
		   <tbody>
		        ...
		        <tr>
		            <td align="right" ><?php //echo $serviceCharge['Billing']['total_amount']; ?></td>
		            <td align="right" ><?php //echo $totalpaid;?></td>
		            <td align="right" ><?php //echo $pendingAmt=$serviceCharge['Billing']['total_amount']-$totalpaid;?></td>
		        </tr>
		   </tbody>
		</table>	  			
	</td>
...
var keyAry=[];
var assoc = [];
$(".serviceAmountToPay").each(function () {
	  currentId=$(this).attr('id');
	  splitedVar=currentId.split('_');
	  var payAmountId= splitedVar['1'];
	 // paidServiceArray[payAmountId]=$(this).val();
	  keyAry.push(payAmountId);
	  valueAry.push($(this).val());
	  assoc.payAmountId = $(this).val();
	});
 */
...
if(categoryName==mandatoryFlag){
	//$( '#amount', parent.document ).val('<?php //echo $totalAmount+$paidtopatient-$servicePaid;?>');commented to allow partial payment..
	//$( '#totalamountpending', parent.document ).val('<?php //echo '0';?>');
}
 ...
 */
 $(".saveService").click(function(){
	  ...
	  /*  var validatePerson = jQuery("#editLabServices_"+recId).validationEngine('validate'); 
		...
		//paid_amount = $('#editServiceAmountToPay_'+recId).val();
...
/*	 $('#selectall').click(function(event) {  //on click
  ...
  //for printing custom service report.
	...
	/*$('.paidAmtService').keyup(function(){ //commented because need disscussion.. Do not delete.commented code...yashwant
		total=0;
		$(".paidAmtService").each(function () {
			if($(this).val()=='')total=total;
			else total=total+parseInt($(this).val());
		});
		$( '#amount', parent.document ).val(total);
	});*/
