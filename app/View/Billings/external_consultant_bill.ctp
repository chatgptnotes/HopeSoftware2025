                   				...
                   				<table width="100%" cellspacing="0" cellpadding="0" border="0">
                   					...
                   					<tbody><tr>
                                        ...
                                        <td width="230" valign="top">
                                        	<p class="name"><?php echo $patient[0]['lookup_name'];?></p>
                                            <p class="address"></p>
                                      	</td>
                                        <td width="20">&nbsp;</td>
                                        <td valign="middle">
...
<table width="100%" align="right" cellpadding="0" cellspacing="0"
	...
	<tr>
		...
		<td width="25"><input type="radio" name="billtype"
			id="pharmacy-sectionBtn" value="Pharmacy" autocomplete="off" /></td>
	...
	<tr>
		...
		<table width="100%" style="margin-top: 70px;" cellpadding="0"
			...
			<tr>
				...
				<td valign="middle" style="text-align: right;"><?php echo $this->Html->link($this->Html->image('icons/delete-icon.png',array('title'=>'Delete','alt'=>'Delete')), array('action' => 'deleteConsultantCharges', $consultantData['ConsultantBilling']['id'],$consultantData['ConsultantBilling']['patient_id']), array('escape' => false),__('Are you sure?', true));?>
				</td>
			</tr>
			<?php }
			?>
			...
			<tr id="row1">
				<td valign="top" width="260">
					...
					'style'=>'width:88px;','readonly'=>'readonly','fieldNo'=>1,'name'=>'data[ConsultantBilling][date][]')); ?>
				</td>
				...
				<td valign="top">
					...
					'id' => 'category_id1','style'=>'width:100px;','fieldNo'=>1,'name'=>'data[ConsultantBilling][category_id][]',"onchange"=>"categoryChange(this)")) ?>
				</td> 
				<td valign="top" style="text-align: left;">
					<?php echo $this->Form->input('ConsultantBilling.doctor_id', array('class' =>
					 ...
					 'options'=>array(''),'id' => 'doctor_id1','style'=>'width:100px;','fieldNo'=>1,'name'=>'data[ConsultantBilling][doctor_id][]',
				...
				<td valign="top" style="text-align: left;">
					...
					<br />
					<select id="service-sub-group1" name="data[ConsultantBilling][service_sub_category_id][]" style="width:100px;" 
				...
				<td valign="top" style="text-align: left;">
					<?php  echo $this->Form->input('', array('class' => 'validate[required,custom[mandatory-select]] textBoxExpnd consultant_service_id',
								...
								'style'=>'width:100px;','fieldNo'=>1,'name'=>'data[ConsultantBilling][consultant_service_id][]' ,
				...
				<td valign="top" style="text-align: center;">
					<?php  echo $this->Form->input('', array('class' => 'textBoxExpnd','type'=>'select','options'=>array('private'=>'Private','cghs'=>'CGHS','other'=>'Other'),
								...
								'style'=>'width:100px;','name'=>'data[ConsultantBilling][hospital_cost][]' ,  ));
					?>
				...
				<td valign="top" style="text-align: center;"><?php echo $this->Form->input('amount',array('class' => 'validate[required,custom[onlyNumber]] amount','legend'=>false,'label'=>false,'id' => 'amount1','style'=>'width:80px;','fieldNo'=>1,'name'=>'data[ConsultantBilling][amount][]')); 
				?></td>
...
<table class="tabularForm" style="position: relative; width: 100%"
	...
	<?php if(!empty($lab)){?>
		...
		<?php }else{?>
		<tr>
			<td align="center" colspan="4">No Record in Laboratory for <?php echo $patient[0]['lookup_name'];?></td>
		</tr>
		<?php }?>
	</tbody>
</table>
...
<table class="tabularForm" style="position: relative; width: 100%"
	...
	<?php if(!empty($rad)){?>
		...
		<?php }else{?>
		<tr>
			<td align="center" colspan="4">No Record in Radiology for <?php echo $patient[0]['lookup_name'];?></td>
		</tr>
		<?php }?>
	</tbody>
</table>
...
<table class="tabularForm" style="position: relative; width: 100%"
	...
	<tbody>
		<tr>
			<td align="center" colspan="4">No Record In Pharmacy For <?php echo $patient[0]['lookup_name'];?></td>
		</tr>
	</tbody>
</table>
	<?php }?></div>
...
<table width="100%" cellpadding="0" cellspacing="1" border="0"
	...
	<?php }else{?>
	<tr>
		<td align="center">No Record in Other Services for <?php echo $patient[0]['lookup_name'];?></td>
	</tr>
	<?php }?>
</table>
...
<table width="55%" cellpadding="0" cellspacing="0" border="0">
	<tr>
		...
		<td align="left" width="80"><?php echo $this->Form->input('', array('name'=>'service_name','type'=>'text','id' => 'search_service_name','style'=>'width:150px;','autocomplete'=>'off')); ?></td>
		<td align="left" width="80"><?php echo $this->Form->submit('Search', array('div'=>false,'label'=>false,'error'=>false,'class'=>'blueBtn'));?></td>
		<?php echo $this->Form->end();?>
		<?php       echo $this->Form->create('billings',array('controller'=>'billings','action'=>'servicesBilling','type' => 'file','id'=>'servicefrm','inputDefaults' => array(
																								        'label' => false,
																								        'div' => false,
																								        'error' => false
		)));
		...
		?>
		<td align="right" width="150">Date:&nbsp;&nbsp;&nbsp;<?php echo $this->Form->input('date', array('value'=>$serviceDate,'type'=>'text','id' => 'billDate','class' => 'validate[required,custom[mandatory-date]]','style'=>'width:150px;','readonly'=>'readonly')); ?></td>
		...
		<table width="100%" cellpadding="0" cellspacing="1" border="0"
			...
			foreach($services as $service){
				...
				if($service['ServiceBill']['evening'] ==1){#echo 'here';exit;
					$evening='checked';
				}
			...
			<tr>
				...
				<?php //echo $this->Form->checkbox('', array('name'=>"data[Nursing][$temp][morning]",'id' => 'morning'.$temp,'value'=>1,'checked'=>$morning,"onchange"=>"checkApplyInADay('".$service['TariffList']['id']."','".$patient['Patient']['tariff_standard_id']."','".$temp."','".$service['TariffList']['apply_in_a_day']."');")); ?>
				<?php if($morning=='checked'){?> <input type="checkbox"
					...
					onchange="defaultNoOfTimes('morning<?php echo $service['TariffList']['id'];?>','<?php echo $service['TariffList']['id'];?>');checkApplyInADay('<?php echo $service['TariffList']['id'];?>','<?php echo $patient['Patient']['tariff_standard_id'];?>','<?php echo $temp;?>','<?php echo $service['TariffList']['apply_in_a_day'];?>')">
					<?php }else{?> <input type="checkbox"
					name="data[Nursing][<?php echo $temp;?>][morning]"
					id="morning<?php echo $temp;?>" value="1"
					onchange="defaultNoOfTimes('morning<?php echo $service['TariffList']['id'];?>','<?php echo $service['TariffList']['id'];?>');checkApplyInADay('<?php echo $service['TariffList']['id'];?>','<?php echo $patient['Patient']['tariff_standard_id'];?>','<?php echo $temp;?>','<?php echo $service['TariffList']['apply_in_a_day'];?>')">
					<?php }?></td>
				<td width="70" style="text-align: center;"><?php //echo $this->Form->checkbox('', array('name'=>"data[Nursing][$temp][evening]",'id' => 'evening'.$temp,'value'=>1,'checked'=>$evening,"onchange"=>"checkApplyInADay('".$service['TariffList']['id']."','".$patient['Patient']['tariff_standard_id']."','".$temp."','".$service['TariffList']['apply_in_a_day']."');")); ?>
				<?php if($evening=='checked'){?> <input type="checkbox"
					...
					onchange="defaultNoOfTimes('evening<?php echo $service['TariffList']['id'];?>','<?php echo $service['TariffList']['id'];?>');checkApplyInADay('<?php echo $service['TariffList']['id'];?>','<?php echo $patient['Patient']['tariff_standard_id'];?>','<?php echo $temp;?>','<?php echo $service['TariffList']['apply_in_a_day'];?>')">
					<?php }else{?> <input type="checkbox"
					name="data[Nursing][<?php echo $temp;?>][evening]"
					id="evening<?php echo $temp;?>" value="1"
					onchange="defaultNoOfTimes('evening<?php echo $service['TariffList']['id'];?>','<?php echo $service['TariffList']['id'];?>');checkApplyInADay('<?php echo $service['TariffList']['id'];?>','<?php echo $patient['Patient']['tariff_standard_id'];?>','<?php echo $temp;?>','<?php echo $service['TariffList']['apply_in_a_day'];?>')">
					<?php }?></td>
				<td width="70" style="text-align: center;"><?php //echo $this->Form->checkbox('', array('name'=>"data[Nursing][$temp][night]",'id' => 'night'.$temp,'value'=>1,'checked'=>$night,"onchange"=>"checkApplyInADay('".$service['TariffList']['id']."','".$patient['Patient']['tariff_standard_id']."','".$temp."','".$service['TariffList']['apply_in_a_day']."');")); ?>
				<?php if($night=='checked'){?> <input type="checkbox"
					...
					onchange="defaultNoOfTimes('night<?php echo $service['TariffList']['id'];?>','<?php echo $service['TariffList']['id'];?>');checkApplyInADay('<?php echo $service['TariffList']['id'];?>','<?php echo $patient['Patient']['tariff_standard_id'];?>','<?php echo $temp;?>','<?php echo $service['TariffList']['apply_in_a_day'];?>')">
					<?php }else{?> <input type="checkbox"
					name="data[Nursing][<?php echo $temp;?>][night]"
					id="night<?php echo $temp;?>" value="1"
					onchange="defaultNoOfTimes('night<?php echo $service['TariffList']['id'];?>','<?php echo $service['TariffList']['id'];?>');checkApplyInADay('<?php echo $service['TariffList']['id'];?>','<?php echo $patient['Patient']['tariff_standard_id'];?>','<?php echo $temp;?>','<?php echo $service['TariffList']['apply_in_a_day'];?>')">
				...
				<script>
		                            checkApplyInADay('<?php echo $service['TariffList']['id'];?>','<?php echo $patient['Patient']['tariff_standard_id'];?>','<?php echo $temp;?>','<?php echo $service['TariffList']['apply_in_a_day'];?>');
		                            </script>
			</tr>
			<?php
	...
	<div align="center" style="border: 1px solid #3E474A; padding: 5px;">No
			Record in services <?php //echo $patient[0]['lookup_name'];?></div>
	<?php }?>
...
?></div>
<?php }?> <?php echo $this->Form->end();?></div>
<?php }?>
<!--  Service section end-->
