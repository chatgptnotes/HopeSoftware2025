...
<table width="100%" cellpadding="0" cellspacing="1" border="0" style="clear:both" class="tabularForm" id="serviceGridLab">
		...
		<?php if($isNursing!='yes'){?>
			...
			<?php if($isNursing!='yes'){?>
            <th class="table_cell" >
                <strong><?php echo __('Print'); ?></strong><br/>
                <input type="checkbox" id="selectAllPrint" title="Select All Print" />
            </th>
            <?php }?>
		</tr>
...
$(document).on('change', '#selectAllPrint', function() {
    var checked = $(this).is(':checked');
    // Clear and update labToPrint array
    labToPrint = [];
...
// Also update labToPrint when individual print checkboxes are changed
$(document).on('change', '.customCheckbox', function() {
    var val = $(this).val();
    if($(this).prop('checked')) {
        if(labToPrint.indexOf(val) === -1) labToPrint.push(val);
    } else {
        var idx = labToPrint.indexOf(val);
        if(idx !== -1) labToPrint.splice(idx, 1);
    }
});
