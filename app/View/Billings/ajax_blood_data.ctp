...
<table width="100%" cellpadding="0" cellspacing="1" border="0" style="clear:both" class="tabularForm" id="serviceGrid">
		...
		<?php if($isNursing!='yes'){?>
			...
			<th class="table_cell"><?php echo __('Blood Bank');?></th>
	...
	<?php $totalAmount=$paid_amount_blood=$allDiscount_blood=0;
	 $i=0;
	foreach($servicesData as $services){ 
		if($services['ServiceBill']['paid_amount']){
			$paid_amount_blood=$paid_amount_blood+$services['ServiceBill']['paid_amount'];
		}
		if($services['ServiceBill']['discount']){
			$allDiscount_blood=$allDiscount_blood+$services['ServiceBill']['discount'];
		}
		...
		<tr class="row" id=row_<?php echo $services['ServiceBill']['id']; ?>>
			...
			// Comparing for already paid services and disabling the chkboxes-- Pooja
				$servicepaidAmt=($services['ServiceBill']['amount']*$no_of_time)-$services['ServiceBill']['paid_amount']-$services['ServiceBill']['discount'];
				$disabled='';
			...
			<td class="<?php echo $background;?>"><?php echo $bloodBanks[$services['ServiceBill']['blood_bank']];?></td>
			...
			if($isNursing!='yes'){
				if(($discharge=='no' && strtolower($tariffStanderdName)!='private') || strtolower($tariffStanderdName)!='private'){
		...
		<tr class="duplicateRow" style="display:none" id=duplicateRow_<?php echo $services['ServiceBill']['id']; ?>>
			...
			<td align="center">
				<?php echo $this->Form->input('blood_bank',array('class'=>'validate[required,custom[mandatory-select]] textBoxExpnd bloodBank',
					'id'=>'bloodBank_'.$services['ServiceBill']['id'],'name'=>'data[ServiceBill][blood_bank]','empty'=>'Please select',
					'options'=>$bloodBanks,'label'=>false,'div'=>false,'style'=>'width:100px;','value'=>$services['ServiceBill']['blood_bank']));?></td>
...
<table width="100%" cellpadding="0" cellspacing="0" border="0" style="clear:both">
	<tr><td >&nbsp;</td></tr>
	<tr>
  		<td style="text-align:center" colspan="5"><strong>Payment Received For Blood</strong></td>
  	</tr>
	...
	<?php if($addmissionType['Patient']['tariff_standard_id']==$getTariffRgjayId && $addmissionType['Patient']['admission_type']=='IPD'){
			}else{?>
            <th >Action</th>
			<?php }?>
            <th >Print Receipt</th>
...
$( '#paymentDetail', parent.document ).trigger('reset');//$allDiscount_blood
...
if(tariffStanderdName!='private'){
	...
	$( '#totaladvancepaid', parent.document ).val('<?php echo $paid_amount_blood;?>');
	$( '#maintainRefund', parent.document ).val('<?php echo $paidtopatient;?>');
	$( '#maintainDiscount', parent.document ).val('<?php echo round($allDiscount_blood);?>');
	$( '#tariff_list_id', parent.document ).val(chk1Array);
	$( '#service_amt', parent.document ).val(chk2Array);
	$( '#amount', parent.document ).val('<?php echo $totalAmount-$paid_amount_blood-round($allDiscount_blood);?>');
	$( '#prevDiscount', parent.document ).html('<?php echo ($allDiscount_blood)?round($allDiscount_blood):0;?>');//for showing previous discount
	...
	if(confirm("Do you really want to delete this record?")){
		$.ajax({
			  ...
			  success: function(data){ 
					  parent.getBloodData(patient_id,groupID);
 ...
 */
 $(".saveService").click(function(){
	  ...
	  /*  var validatePerson = jQuery("#editLabServices_"+recId).validationEngine('validate'); 
		...
		bloodBank = $('#bloodBank_'+recId).val();

		$.ajax({
  			type : "POST",
  			data: "date="+date+"&amount="+amount+"&subGroupID="+subGroupID+"&serviceID="+serviceID+"&noOfTimes="+noOfTimes+"&description="+description+"&bloodBank="+bloodBank,
  ...
  */
    ...
    $('#selectall').click(function(event) {  //on click
        if(this.checked) { // check select status
            ...
            $( '#service_amt', parent.document ).val(chk2Array);
            $( '#totalamountpending', parent.document ).val('0');
          //  $( '#amount', parent.document ).val('<?php //echo $totalAmount-$paid_amount_blood;?>');//to show total in amount field
