...
<tr>
    ...
    <td valign="top">:</td>
    <td valign="top"><?php echo substr($patient['Patient']['admission_id'], -5); ?></td>
</tr>
  <?php }?>
   ...
   <?php }?>             
                ...
                foreach($surgeriesData as $surg){
                    	if($b==1 && $surg['TariffList'][$nameField]!=''){ 
                    		...
                    		echo '&nbsp;&nbsp;&nbsp;&nbsp;(' . $surg['OptAppointment']['implant_description'] . ')'; //by ashwin dahikar
  ...
  <tr>
    <td width="100%" align="left" valign="top">
       <table width="100%" border="0" cellspacing="0" cellpadding="5" class="tdBorderTpBt">
          ...
          foreach ($cCArray as $cBilling){ 
	          ...
	          <tr>
	            ...
	            <td align="right" valign="top" class="tdBorderRt"><?php 
	            	...
	            	?></td>
	            <td align="center" valign="top" class="tdBorderRt"><?php 
	            $totalCost = $totalCost + ($consultantBilling[0]['ConsultantBilling']['amount1']*count($consultantBilling));
          ...
          foreach($wardServicesDataNew as $uniqueSlot){ //debug($uniqueSlot);exit;
          	$srNo++;
				 ...
				 ?>
		          	<tr>
			            ...
			            <td class="tdBorderRt" style="padding-left:10px;"><?php  
				            ...
				            // echo  h($surgeriesData[0]['OptAppointment']['implant_description']);
				...
				<?php }else{    ?>
		            ...
		            <td class="tdBorderRt" style="padding-left:10px;"><?php 
			         ...
			         //   echo '<p>' . h($surgeriesData[0]['OptAppointment']['implant_description']) . '</p>'; //surgery description by @7387737062
...
<?php
if((trim($patientd['TariffStandard']['name'])!=trim(Configure::read("Raymonds"))) && (trim($patientd['TariffStandard']['name'])!=trim(Configure::read("Mahindra")))){?>       
          <?php
            if(count($radRate)>0){
          	$v++;
...
<?php echo $this->Form->hidden('',array('name'=>"data[Billing][$v][amount]",'value' => $this->Number->format($rCost,array('places'=>2,'decimal'=>'.','before'=>false,'thousands'=>false)),'legend'=>false,'label'=>false,'style'=>'text-align:right'));
 ...
 <?php
      if(count($radRate)>0){
      $v++;  ?>	
 <?php  foreach ($radRate as $key=> $radiologySubData) {
	   	$returnSubCategoryArray[$radiologySubData['ServiceSubCategory']['name']][]=$radiologySubData;
	   }
	   $amt=0;
	   $keyMy;
	   $radSubGrpName= array();
	   $blankSubGrpName= array();
	   foreach($returnSubCategoryArray as $key=>$headData){
			if($key==''){
				$blankSubGrpName[]=$headData;
			}
			else{
				foreach($headData as $subData){
					if($subData['RadiologyTestOrder']['amount']> 0)
						$amt=$amt+$subData['RadiologyTestOrder']['amount'];
					else
						$amt=$amt+$subData['TariffAmount']['nabh_charges'];
				}	
				$radSubGrpName[$key]=$amt;
				$amt=0;
			}
		}
		foreach($blankSubGrpName as $blankData){
			foreach($blankData as $subDataKey=> $subData){
				$amount+=$subData['RadiologyTestOrder']['amount'];
				$radSubGrpName[$subData['Radiology']['name']]=$amount;
			}
		}
	   $srNo++;
   ?>	 
...
<?php 
foreach ($radSubGrpName as $subKey=>$subValue){?>
   <tr>
     <td align="center" class="tdBorderRt"></td>
	 <td class="tdBorderRt"><?php echo $subKey ;?>
     </td>
     <td align="center" class="tdBorderRt" style="display:<?php echo $hideCGHSCol ;?>">&nbsp;</td>
     <td align="right" valign="top" class="tdBorderRt">
		<?php echo $this->Form->hidden('',array('name'=>"data[Billing][$v][rate]",'value' => $this->Number->format($subValue,array('places'=>2,'decimal'=>'.','before'=>false,'thousands'=>false)),'legend'=>false,'label'=>false,'id' => 'registration_rate','style'=>'text-align:right;'));
			  echo $this->Number->format($subValue);
		?>
    </td>
            <td align="center" valign="top" class="tdBorderRt">
	<?php echo $this->Form->hidden('',array('name'=>"data[Billing][$v][unit]",'value' => '--','legend'=>false,'label'=>false,'style'=>'text-align:center'));
		  echo '--';
	?>
    </td>
    <td align="right" valign="top">
		<?php echo $this->Form->hidden('',array('name'=>"data[Billing][$v][amount]",'value' => $this->Number->format($subValue,array('places'=>2,'decimal'=>'.','before'=>false,'thousands'=>false)),'legend'=>false,'label'=>false,'style'=>'text-align:right'));
		echo $this->Number->format($subValue);
		?>
	</td>
 </tr>
...
<?php echo $this->Form->hidden('',array('name'=>"data[Billing][$v][amount]",'value' => $this->Number->format(($otherServiceD['OtherService']['service_amount']),array('places'=>2,'decimal'=>'.','before'=>false,'thousands'=>false)),'legend'=>false,'label'=>false,'style'=>'text-align:right'));
    ...
    <?php }?>     
          ...
          <tr>
            ...
            <td align="center" class="tdBorderTpRt" style="display:<?php 
            echo $hideCGHSCol ;
            ?>">&nbsp;</td>
            <td align="right" valign="top" class="tdBorderTpRt">&nbsp;</td>
            <td align="right" valign="top" class="tdBorderTpRt">&nbsp;</td>
