...
<table width="100%" cellpadding="0" cellspacing="0" border="0" style="clear:both">
	<tr><td >&nbsp;</td></tr>
	<tr>
  		<td style="text-align:center" colspan="5"><strong>Payment Received For Other Services</strong></td>
  	</tr>
	...
	if(confirm("Do you really want to delete this record?")){
		$.ajax({
			  ...
			  success: function(data){ 
					  parent.getOtherServiceData(patient_id,groupID);
 ...
 */
 $(".saveService").click(function(){
	  ...
	  recId=splitedVar[1];
		...
		$.ajax({
  			type : "POST",
  			data: "date="+date+"&amount="+amount+"&subGroupID="+subGroupID+"&serviceID="+serviceID+"&noOfTimes="+noOfTimes+"&description="+description,
  ...
  */
  ...
  //for printing custom service report.
	...
	/*$(document).on('keypress','.other_service', function(event) {
