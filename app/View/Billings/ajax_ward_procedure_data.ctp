...
<table width="100%" cellpadding="0" cellspacing="1" border="0" style="clear:both" class="tabularForm" id="serviceGrid">
	...
	<?php $totalAmount=$paid_amount_ward=$allDiscount_ward=0;
	 $i=0;
	foreach($servicesData as $services){ 
		if($services['ServiceBill']['paid_amount']){
			$paid_amount_ward=$paid_amount_ward+$services['ServiceBill']['paid_amount'];
			$allDiscount_ward=$allDiscount_ward+$services['ServiceBill']['discount'];
		}
		...
		<tr class="row" id=row_<?php echo $services['ServiceBill']['id']; ?>>
			...
			<td align="center">
			<?php 
			if(empty($serviceCharge) /*&& strtolower($this->Session->read('role'))=='admin'*/){
...
$( '#totaladvancepaid', parent.document ).val('<?php echo $paid_amount_ward;?>');
...
$( '#totalamountpending', parent.document ).val('<?php echo $totalAmount-$paid_amount_ward;?>');
