...
<table width="100%" cellpadding="0" cellspacing="1" border="0" style="clear:both" class="tabularForm" id="serviceGrid">
		...
		<?php $totalAmt= $nonBillable =$allDiscount_rad=0;
		...
		if($billedRadId){//for rad wise refund  --yashwant
			$billingId=$billedRadId[$radData['RadiologyTestOrder']['id']];
		}
		if($radData['RadiologyTestOrder']['paid_amount']){
			$paid_amount=$paid_amount+$radData['RadiologyTestOrder']['paid_amount'];
		}
		if($radData['RadiologyTestOrder']['discount']){
			$allDiscount_rad=$allDiscount_rad+$radData['RadiologyTestOrder']['discount'];
		}?>
		<tr class="row" id=row_<?php echo $radData['RadiologyTestOrder']['id']; ?>>
		<?php 
		$radPaidAmt=($radData['RadiologyTestOrder']['amount']-$radData['RadiologyTestOrder']['paid_amount']-$radData['RadiologyTestOrder']['discount']);
		
		//added by pankaj
		$readonly = '';   
		$disabled='';
		
		if($radPaidAmt<=0 && $radData['RadiologyTestOrder']['amount']>0){
		...
		?>
			...
			<input class="checkbox1 <?php echo $billable;?>" type="checkbox" id="<?php echo "optCheck_".$i; ?>" name="check[]" value="<?php echo $radData['RadiologyTestOrder']['id'];?>"
			amount="<?php echo ($radData['RadiologyTestOrder']['amount']);?>" discount="<?php echo round($radData['RadiologyTestOrder']['discount']);?>"
			paid_amount="<?php echo round($radData['RadiologyTestOrder']['paid_amount']);?>" tariffId="<?php echo  $radData['RadiologyTestOrder']['id'];?>" <?php echo $disabled;?>>	
			<?php }?>
			<td class="<?php echo $background;?>" valign="middle"> <?php echo $this->DateFormat->formatDate2LocalForReport($radData['RadiologyTestOrder']['radiology_order_date'],Configure::read('date_format'),true); ?></td>
			<td class="<?php echo $background;?>" valign="middle"> <?php echo $radData['Radiology']['name']; ?></td>
			<td class="<?php echo $background;?>" valign="middle"> <?php echo $radData['ServiceProvider']['name']; ?></td>
			<td class="<?php echo $background;?>" valign="middle" style="text-align: right;" id="<?php echo "amountBill_".$i;?>">
			...
			?></td>
			<?php
			if(/*$radData['RadiologyTestOrder']['id']== $paidRad[$radData['RadiologyTestOrder']['id']]*/ $radData['RadiologyTestOrder']['paid_amount']){
				//$paidAmt=$totalAmount1;
				$paidAmt=$radData['RadiologyTestOrder']['paid_amount'];
			}else{
				$paidAmt='0';
			}
			echo $this->Form->input('partial_paid_amt',array('type'=>'hidden','id'=>'partial_amt_'.$i,'value'=>$paidAmt));?>
			<td class="<?php echo $background;?>" valign="middle"> <?php echo $radData['RadiologyTestOrder']['description']; ?></td>
			<?php //if($patient_details['Patient']['is_discharge']!=1){?>
			<td class="<?php echo $background;?>" valign="middle" style="text-align: center;">
				<?php 
				if(/*empty($radCharge)*/$radPaidAmt>0 /*&& strtolower($this->Session->read('role'))=='admin'*/){
					/*echo $this->Html->image('icons/delete-icon.png',array('title'=>'Delete','alt'=>'Delete','class'=>'deleteRad','id'=>'deleteRad_'.$radData['RadiologyTestOrder']['id']),
					 array('escape' => false));*/
					echo $this->Html->link($this->Html->image('icons/delete-icon.png',array('title'=>'Delete','alt'=>'Delete','class'=>'deleteRad','id'=>'deleteRad_'.$radData['RadiologyTestOrder']['id']),
					 array('escape' => false)),array('controller'=>'billings','action'=>'deleteRadCharges'),array('escape' => false,'class'=>'billingServicesAction'));
				}
				if($isNursing!='yes'){
					if(($patient_details['Patient']['is_discharge']!=1 && strtolower($tariffStanderdName)!='private') || strtolower($tariffStanderdName)!='private'){//edit for non private discharge patients- pooja 18/06/2016
						echo $this->Html->link($this->Html->image('icons/edit-icon.png',array(/*'style'=>'display:'.$displayStatus,*/'title'=>'Edit','alt'=>'Edit','class'=>'editRad','id'=>'editRad_'.$radData['RadiologyTestOrder']['id']),
							array('escape' => false)),array('controller'=>'billings','action'=>'updateService'),array('escape' => false,'class'=>'billingServicesAction'));
 					}
				if($website != 'vadodara'){
				if($radPaidAmt<=0 && $patient_details['Patient']['is_discharge']!=1){?>
					<input class="refundCheck" type="checkbox" title="Refund" id="<?php echo "refundCheck_".$radData['RadiologyTestOrder']['amount']."_".$radData['RadiologyTestOrder']['id']."_".$billingId."_".$radData['RadiologyTestOrder']['discount'];?>" name="refundCheckbox[]" <?php echo $customDiasbled;?>  value="">
				<?php } } } 
				if(strtolower($this->Session->read('website.instance')) != 'vadodara' && strtolower($this->Session->read('website.instance')) != 'kanpur'){
                                    if(!empty($radData['ServiceProvider']['name']))
					echo $this->Html->link($this->Html->image('icons/print.png',array('title'=>'print','alt'=>'print','class'=>'printRad','id'=>'printRad_'.$radData['RadiologyTestOrder']['id']."_".$tariffStandardId),
						array('escape' => false)),'javascript:void(0);',array('escape' => false,'class'=>'billingServicesAction'));
				}
				?>
			</td>
			<?php //}?>
		...
		<tr class="duplicateRow" style="display:none" id=duplicateRow_<?php echo $radData['RadiologyTestOrder']['id']; ?>>
			...
			echo $this->Form->input('', array('id' => 'radiologyname_'.$radData['RadiologyTestOrder']['id'],'type'=>'text', 'label'=> false, 'div' => false,'value'=>$radData['Radiology']['name'],
					 'error' => false,'autocomplete'=>false,'class'=>'validate[required,custom[mandatory-enter]] radiology_name textBoxExpnd','name'=>'data[RadiologyTestOrder][rad_name][]','readonly'=>$readonly));
			...
			<td><?php echo $this->Form->input('', array('style'=>'text-align:right','name'=>'data[RadiologyTestOrder][amount][]','type'=>'text','readonly'=>$readonly,
					'value'=>$radData['RadiologyTestOrder']['amount'],'label'=>false,'id' => 'radAomunt_'.$radData['RadiologyTestOrder']['id'],'class'=> 'textBoxExpnd radAomunt',
					'div'=>false/*,'readonly'=>'readonly'*/));
			echo $this->Form->input('', array('name'=>'data[RadiologyTestOrder][fix_discount][]','type'=>'hidden',
					'value'=>$radData['RadiologyTestOrder']['fix_discount'],'label'=>false,'id' => 'rfix_discount_'.$radData['RadiologyTestOrder']['id'],'class'=> 'fix_discount',
					'div'=>false/*,'readonly'=>'readonly'*/));?></td>
  	...
  	<td>
		...
		foreach($radCharge as $radCharge){
			...
			if($radCharge['Billing']['refund']=='1'){
				...
				$totalpaidDiscount=$totalpaidDiscount+$radCharge['Billing']['discount'];// to maintain discount in payment detail block  --yashwant
				continue;
			}else{
			...
			if(strtolower($this->Session->read('website.instance'))=='kanpur'){
				if(strtolower($patient_details['Patient']['admission_type'])!='opd'){ 
                    ...
                    echo $this->Html->link($this->Html->image('icons/printer_mono.png',array('title'=>'Print Receipt without Header')),'#',
		                array('escape' => false,'onclick'=>"var openWin = window.open('".$this->Html->url(array('action'=>'printAdvanceReceipt',
				         $radCharge['Billing']['id'],'?'=>'flag=without_header'))."', '_blank',
						 'toolbar=0,scrollbars=1,location=0,status=1,menubar=0,resizable=1,width=900,height=600,left=200,top=200');  return false;"));
 ...
 */
			...
			<td height="30px"><?php  
			 if($website == 'vadodara'){
					echo $this->Html->link('Print Receipt','javascript:void(0)',
				 		...
				 		$radCharge['Billing']['id'],'?'=>array('flag'=>'Radiology','recID'=>$radCharge['Billing']['id'])))."', '_blank','toolbar=0,scrollbars=1,location=0,status=1,menubar=0,resizable=1,width=900,height=600,left=200,top=200');  return false;"));
			 }else{
			 	echo $this->Html->link('Print Receipt','javascript:void(0)',
            ...
            <td height="30px"><?php 
            	echo $this->Html->link('Print without header','javascript:void(0)',
            			array('class'=>'blueBtn','escape' => false,'onclick'=>"var openWin = window.open('".$this->Html->url(array('action'=>'billReportLab',
            					$patientID,'?'=>array('flag'=>'Radiology','header'=>'without','recID'=>$radCharge['Billing']['id'])))."', '_blank','toolbar=0,scrollbars=1,location=0,status=1,menubar=0,resizable=1,width=900,height=600,left=200,top=200');  return false;"));
				?></td>
           <?php }?> 
		</tr>
		<?php }?> 
...
$( '#totalamountpending', parent.document ).val('0');// to allow partial payment..
...
if(tariffStanderdName!='private'){
	...
	$( '#amount', parent.document ).val('<?php echo $totalAmt-$paid_amount-round($allDiscount_rad);?>');
	$( '#tariff_list_id', parent.document ).val(chk1Array);//service id
	$( '#maintainDiscount', parent.document ).val('<?php echo round($allDiscount_rad);?>');
	$( '#prevDiscount', parent.document ).html('<?php echo ($allDiscount_rad)?round($allDiscount_rad):0;?>');//for showing previous discount
 ...
 */
 $(document).on('focus','.radiology_name', function() {
	...
	$(this).autocomplete({
		 source: "<?php echo $this->Html->url(array("controller" => "Billings", "action" => "radChargesAutocomplete","admin" => false,"plugin"=>false)); ?>"+'/'+patientID+"&admission_type="+"<?php echo $patient_details['Patient']['admission_type'] ;?>"+"&room_type="+"<?php echo $patient_details['Room']['room_type']?>",
			 minLength: 1,
...
$(".saveRad").click(function(){
	  ...
	  /*  var validatePerson = jQuery("#editLabServices_"+recId).validationEngine('validate'); 
		...
		if(isOk){
			...
			discount=$('#rfix_discount_'+ID).val();
  ...
  $('#selectall').click(function(event) {  //on click
	...
	$(".printRad").click(function(){
		...
		$.fancybox({
		    ...
		    'href' : "<?php echo $this->Html->url(array("controller" =>"Billings","action" =>"getExternalRequisitionPrint","admin"=>false)); ?>/"+idd[1]+"/"+idd[2],
		});
