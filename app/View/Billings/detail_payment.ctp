...
?>
 ...
 <?php if(!empty($finalBillingData['FinalBillingOption'])){
  ...
  ?>
  <tr><td><table width="100%" cellpadding="5" cellspacing="0" border="0" style="">
  <?php if($diagnosisData['Diagnosis']['final_diagnosis']!=''){?>
            	<tr>
                	<td valign="top">
                	Diagnosis<br />
...
<table class="" border="0" class="table_format" cellpadding="0"
	...
	//BOF radiology
	usort($radRate, function($a, $b) {
    return strtotime($a['RadiologyTestOrder']['create_time']) - strtotime($b['RadiologyTestOrder']['create_time']);
});
	$j=1;
...
<?php
    ...
    //     $surgeryServicesCost += $surgery['TariffList']['cost']; // Assuming 'cost' is the key for surgery cost
?>
<!--<tr>-->
<!--    <td valign="top" class="tdBorderTp"><?php echo $j++; ?></td>-->
...
<?php 
//  echo $this->DateFormat->formatDate2Local($surgery['TariffList']['date'], Configure::read('date_format')); 
 ?>
</td>-->
<!--    <td align="center" valign="top" class="tdBorderRt tdBorderTp">&nbsp;</td>-->
<!--    <td align="center" valign="top" class="tdBorderRt tdBorderTp">1</td>-->
<!--    <td align="right" valign="top" class="tdBorderTp"><?php echo $this->Number->format($surgery['TariffList']['cost'], array('places' => 2, 'decimal' => '.', 'before' => false, 'thousands' => false)); ?></td>-->
<!--</tr>-->
<?php 
    // } 
?>
...
?>
	...
	<tr>
		<td colspan="5" align="center" style="padding:4px !important"><b>In Words:</b>&nbsp;<?php echo $this->RupeesToWords->no_to_words(round($rCost+$pCost+$lCost+$raCost+$dCost+$sCost+$otherServicesCost));?></td>
	</tr>
	<tr>
		<td colspan="5">&nbsp;</td>
	</tr>
