...
<?php } ?>
<form name="GroupSmsFrm" id="GroupSmsFrm" action="<?php echo $this->Html->url(array("action" => "groupAdd")); ?>" method="post" onSubmit="return Validate(this);" >       
	...
	<td><?php echo $this->Form->input('GroupSms.group_name', array('class' => 'validate[required,custom[mandatory-enter]]','id' => 'group_name', 'label'=> false, 'div' => false, 'error' => false));
	   ?></td>
	</tr>
	   <tr>
	<td class="form_lables">
	...
	<td><?php echo $this->Form->input('GroupSms.manager_mobile_no', array('class' => 'validate[required,custom[phone,minSize[10],onlyNumber]]','id' => 'manager_mobile_no', 'label'=> false, 'div' => false, 'error' => false,'maxlength'=>'10'));
	  ...
	  <td>
	   <?php 
	          echo $this->Form->checkbox('GroupSms.is_active', array('checked'=>'checked','id' => 'is_active', 'label'=> false, 'div' => false, 'error' => false));
	   ?>
	  </td>
	 </tr>
	 <tr>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0"
						...
						<tr id="first">
							...
							?>
							 ...
							 <div id="citySection-<?php echo $key?>" style="display:none;"><?php echo $this->Form->input("vbv",array('type'=>'select','id'=>"city_id-$key",'class' => 'textBoxExpnd ','name'=> 'data[city_id]['.$key.']','label'=>false,'div'=>false,'empty'=>'Select City','options'=>$corporateSublocationData,'style'=>'width:150px;'));
							?>
							</div>
							</td>
...
<script>
 //For Sort array in ascending Order
		...
		function addSublocationDropdownStatic(IdSubLoc,corporateId){
		 	$('#busy-indicator').show('fast');	  		
...
//BOF-click on city to fetch Branch
			...
			//$('.corporateCls').change(function (){
				...
				if(privateTariff!=corporateId){
					...
					if(privateTariff==corporateId){
						addSublocationDropdownStatic(IdSubLocSplit[1],corporateId);//changes done by mahalaxmi
...
var packageSrNo = isNaN(parseInt('<?php echo $packageSrNo;?>')) ? 1 : parseInt('<?php echo $packageSrNo;?>'); 
var key =packageSrNo;
function addMoretr() {
    var wardOrderNo = 1;
 //$("#sublocation_id-"+packageSrNo+" option").remove();
    var appendOption= "<option value=''>Please Select</option>";

	 $('#packageData')
     .append($('<tr>').attr('id','removeRow-'+packageSrNo)  
     				 .append($('<td style="padding-left:10px;">').append($('<select>').attr({'id':'initial_id-'+packageSrNo,'class':'textBoxExpnd ','type':'select','name':'data[initial_id]['+packageSrNo+']'})))			          
                     .append($('<td  style="padding-left:10px;">')
                             .append($('<input>').attr({'name':'data[contact_name]['+packageSrNo+']','type':'text','class': 'docName validate[required,custom[mandatory-enter]] textBoxExpnd','autocomplete': 'off','id' : 'childId-'+packageSrNo,'style':'width:150px;'})))
                     .append($('<td>')
                                     .append($('<input>').attr({'name':'data[mobile]['+packageSrNo+']','type':'text','class' :'rate calculateCost textBoxExpnd validate[required,custom[phone,minSize[10],onlyNumber]]','autocomplete': 'off', 'id' : 'childImages-'+packageSrNo,'maxlength':'10','style':'width:150px;'})))  
                     .append($('<td  style="padding-left:10px;">')
                             .append($('<select>').attr({'name':'data[corporate_id]['+packageSrNo+']','type':'text','class': 'corporateCls textBoxExpnd','autocomplete': 'off','id' : 'corporate_id-'+packageSrNo,'style':'width:150px;'}).append(appendOption)))        
                     .append($('<td  style="padding-left:10px;">')                     	
                             .append($('<select>').attr({'name':'data[sublocation_id]['+packageSrNo+']','type':'text','class': 'subLocCls textBoxExpnd','autocomplete': 'off','id' : 'sublocation_id-'+packageSrNo,'style':'width:150px;'}).append(appendOption))
                             .append($('<div>').attr({'id':'citySection-'+packageSrNo,'style':'display:none;'})  
                             .append($('<select>').attr({'name':'data[city_id]['+packageSrNo+']','type':'text','class': 'textBoxExpnd','autocomplete': 'off','id' : 'city_id-'+packageSrNo,'style':'width:150px;'}).append(appendOption))))               
                     .append($('<td  style="padding-left:10px;">')
                             .append($('<input>').attr({'name':'data[other_info]['+packageSrNo+']','type':'text','class': 'docName textBoxExpnd','style':'width:250px;','autocomplete': 'off','id' : 'other_info-'+packageSrNo})))                        
					 .append($('<td class="text">').attr('id','Td-'+packageSrNo).append($('<span>').attr({'id' : 'Add-'+packageSrNo})
                     .append('<?php echo $this->Html->image('icons/plus_6.png', array('title'=> __('Add', true),'alt'=> __('Add',true),'style'=>'float:none;','class'=>'addMoretr','onClick'=>'addMoretr();$(this).parent().hide()'));?>'))
                      .append($('<span>').attr({'class':'removePackageData','id' : 'Remove-'+packageSrNo/*,'onclick' : "$('#Add-"+packageSrNo-1+").show()"*/})
     ...
     var selectInitials = <?php echo json_encode($initials);?>;
			 ...
			 var selectCorporateSublocationData = <?php echo json_encode($corporateSublocationData);?>;
	   		$.each(selectCorporateSublocationData, function (key, value) {
	   			$('#sublocation_id-'+packageSrNo).append( new Option(value, key) );
			});
