...
<div
	...
	<table border="0" cellspacing="0" cellpadding="0"
		...
		<tr>
			...
			<td valign="top"><?php echo $this->Form->input('Patient.date_of_referral', array('type'=>'text','class' => '','id' => 'date_of_referral')); ?>
				...
				<tr><td class="tdLabel" valign="top">
        <?php echo __('Relationship Manager'); ?><font color="red">*</font>
    </td>
    <td>
            <?php 
			echo $this->Form->input('Patient.market_team', [
				'type' => 'text',
				'id' => 'market_team_agent',
				'class' => 'textBoxExpnd',
				'div' => false,
				'placeholder' => 'Market Team Name',
				'style' => 'display: none;',
				'readonly' => true
    			]); 
    		?>
    ...
    <!--code add by dinesh tawade-->
		...
		<tr>
		<td valign="middle" class="tdLabel" id="">Diagnosis 
		<!--<font-->
		<!--	color="red">*</font>-->
			</td>
			<td><?php echo $this->Form->input('diagnosis_txt', array('class' => 'textBoxExpnd ','id'=>'diagnoses')); ?>
			</td>
			<td>&nbsp;</td>
		...
		<tr>
			<td valign="middle" class="tdLabel" id="">Surgery
			<!--<font-->
			<!--color="red">*</font>-->
			</td>
			<td><?php echo $this->Form->input('surgery_text', array('class' => 'textBoxExpnd ','id'=>'sugery_text')); ?> 
			</td>
			<!--//validate[required,custom[mandatory-enter]]-->
		</tr>
 ...
 function refferalAutocomplete(category) {
    $('#searchDoctor').autocomplete({
        ...
        select: function(event, ui) {
            var doctorName = ui.item.value; // Doctor ka naam
            var doctorId = ui.item.id; // Doctor ki ID
            var marketTeam = ui.item.market_team; // Market Team ka naam

            $('.service-box').each(function() { // Duplicate check
            ...
            inputVar = '<input class="service-box" type="hidden" name="data[Patient][consultant_id][]" value=' + doctorId + '>';
            ...
            // **Market Team ka input field update karein**
            $('#market_team_agent').val(marketTeam).show(); // Market Team ka naam show kare

            // Validation agar referral doctor select nahi kiya
