...
echo $this->Html->script(array('jquery.fancybox-1.3.4'));?>
   ...
   <table border="0" class="table_format_body" cellpadding="0"
    ...
    <td class="table_cell"><strong><?php echo __('RXNORM Code', true); ?></th>
      </tr>
  <?php 
   $toggle =0;
      if(count($data) > 0) {
  ...
  ?><?php  //debug($personval); exit;?>
   <td class="row_format rxcuiClass" >
   <input type="hidden" name="<?php echo $orderresult['PharmacyItem']['name'];?>" value="<?php echo $orderresult['PharmacyItem']['name'];?>" id="<?php echo $key;?>">
   
   <?php echo $this->Html->link(__($orderresult['PharmacyItem']['name']),'#',
   		array('onclick'=>'ordersentences("'.$patient_id.'","'.$category.'","'.$orderresult['PharmacyItem']['rxcui'].'","'.$orderresult['PharmacyItem']['drug_id'].'")','class'=>'','div'=>false,'label'=>false,"id"=>"result".$orderresult['Rxnatomarchive']['rxcui']));
			?>
			</td>
   <td class="row_format"><?php echo $orderresult['PharmacyItem']['rxcui']; ?> </td>
   </tr>
  <?php endforeach;  ?>
   <tr>
