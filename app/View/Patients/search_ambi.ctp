...
<?php }
	...
	}else if($this->params->query['type']=='OPD'){
		$urlType= 'IPD' ;
		$serachStr ='IPD&is_emergency=0' ;
		$searchStrArr = array('type'=>'OPD','is_emergency'=>0);
	}
...
<div class="inner_title">
	<h3>&nbsp; <?php
		if($this->params->query['mod']=='assessment'){
			...
			echo strtoupper($urlType).' - ';?><?php if($this->params->query['patientstatus']=='processed') echo __('Process Done Patient Enquiry', true); else if ($this->params->query['patientstatus']=='discharged') echo __('Discharged Patient Enquiry', true); else echo __('Patient Enquiry', true); 
		} 
	?></h3>
	<span><?php 
...
<?php if(isset($data) && !empty($data)){
	...
	?>
				  ...
				  <tr class="row_title">
                       ...
                       <?php if($this->params->query['patientstatus']=='processed'){ ?>
                                           <td class="table_cell"><strong><?php echo $this->Paginator->sort('Patient.discharge_date', __('Process Done Date', true)); ?></strong></td>
                       <?php } elseif($this->params->query['patientstatus']=='discharged'){ ?>
                                           <td class="table_cell"><strong><?php echo $this->Paginator->sort('Patient.discharge_date', __('Discharged Date', true)); ?></strong></td>
                       <?php } else { ?>
				  ...
				  <?php 
				  	  $toggle =0;
debug( $patients['Patient']['id'] ."here" );
				      if(count($data) > 0) {
				      		foreach($data as $patients){
								  ...
								  ?>								  
								   ...
								   <?php if($this->params->query['patientstatus']=='processed' || $this->params->query['patientstatus']=='discharged'){ ?>
								           <td class="row_format"><?php echo $this->DateFormat->formatDate2Local($patients['Patient']['discharge_date'],Configure::read('date_format'),true); ?> </td>
								   <?php } else { ?>
								   ...
								   <td >
								   		<?php
								   			...
								   			if($this->params->query['mod']=='assessment'){
								   				echo $this->Html->link($this->Html->image('icons/uerInfo.png',array('style'=>'height:20px;width:18px;')), array('controller'=>'diagnoses','action' => 'add_ambi', $patients['Patient']['id'],"?"=>array('return'=>'assessment')), array('escape' => false,'title'=>'Patient Info.'));
								   			}else{ 
