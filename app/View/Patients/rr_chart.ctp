...
<div class="inner_title">
	...
	<h3><font style="color:#000">
		<?php echo __('Respiratory Rate Chart', true)." For - ".$patient;?>
	</font></h3>
</div>
<?php  if($temp_datas) {?>
<center>
...
<div>
	...
	$strXML = '<chart caption="Respiration" legendBgColor="#DDDDDD" canvasBgColor="#FFFFFF" bgColor="##FFFFFF" baseFontColor="#000000" toolTipBgColor="#DDDDDD" showBorder="0" borderColor="1B1B1B" bgAlpha="100" alternateHGridColor="#DDDDD" divLineColor="#4285F4" divLineAlpha="80"  divLineIsDashed="1" divLineThickness="1" xaxisname="Noted On" rotateLabels="1"  lineColor ="#4285F4"  anchorBgColor="#000"  anchorRadius="4"  adjustDiv="0" yaxisname="Respiration Rate (br/min)"  showValues="0" decimalPrecision="0" labelDisplay="WRAP"  yAxisMaxValue="5"  >';
	...
	foreach ($temp_datas as $key=>$arSubData) {
		...
		$strDataSSI .= '<set toolText="'.$arSubData.' br/min {br}'.$date1.'" label="' . $date1. '" value="' . $arSubData . '" />';
