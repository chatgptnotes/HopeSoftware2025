<div class="inner_title">
	<h3>Laboratory Test Payment Receipts</h3>
	<span>
<?php
	...
	<tr class="row_title">
	  ...
	  <?php
	   ...
	   <tr <?php if($cnt%2 == 0) echo "class='row_gray'"; ?>>
		...
		<td class="row_format"><?php echo $this->Number->format($data['LabTestPayment']['total_amount'],array('places'=>2,'decimal'=>'.','before'=>false)); ?> </td>
		<td class="row_format"><?php echo $this->Number->format($data['LabTestPayment']['paid_amount'],array('places'=>2,'decimal'=>'.','before'=>false)); ?> </td>
		<td class="row_format"><?php
					if ($currentID != $data ['LabTestPayment'] ['batch_identifier'])
						$paid = 0;
					$paid += $data ['LabTestPayment'] ['paid_amount'];
					$payAmt = ( int ) $data ['LabTestPayment'] ['total_amount'] - ( int ) $paid;
					echo $this->Number->format ( $payAmt, array (
							'places' => 2,
							'decimal' => '.',
							'before' => false 
					) );
					?> </td>
		<td class="row_format"><?php echo $currentID = $data['LabTestPayment']['batch_identifier'] ; ?> </td>
		<td>
			    <?php
					echo $this->Html->link ( $this->Html->image ( 'icons/print.png', array (
							'title' => 'Print Receipt' 
					) ), '#', array (
							'escape' => false,
							'onclick' => "var openWin = window.open('" . $this->Html->url ( array (
									'action' => 'lab_test_payment_receipt_print',
									$data ['LabTestPayment'] ['id'] 
							) ) . "', '_blank',
					...
					echo $this->Html->link ( $this->Html->image ( 'icons/view-icon.png', array (
							...
							'onclick' => "var openWin = window.open('" . $this->Html->url ( array (
									...
									$data ['LabTestPayment'] ['id'],
									'?' => array (
											'identifier' => $data ['LabTestPayment'] ['batch_identifier'] 
									) 
							) ) . "', '_blank',
					           'toolbar=0,scrollbars=1,location=0,status=1,menubar=0,resizable=1,width=900,height=600,left=200,top=200');  return false;" 
					) );
