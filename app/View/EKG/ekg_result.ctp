<style>.row_action img{float:right;}</style>
<p class="ht5"></p>
<?php echo  $this->Form->create('EkgResult',array('id'=>'ekgManagerfrm','type'=>'file','url'=>array('controller'=>'EKG','action'=>'ekg_result',$ekgData[0]['EKG']['patient_id']),
...
<table width="100%" cellpadding="0" cellspacing="1" border="0"
	...
	<tr>
		...
		$patient_id=$ekgData[0]['EKG']['patient_id'];
		$ekg_id=$ekgData[0]['EKG']['id'];

		?>
			<div class="ht5"></div>
			<table width="100%" cellpadding="0" cellspacing="0" border="0">
				<tr>
					<td style="padding: 0 10px 0 0;"><?php echo __('Upload file/record ')?>
					</td>
							...
							<tr>
								...
								?>
									<div id='TextBoxesGroup'>
										<div id="TextBoxDiv0">
											<?php 
											echo $this->Form->input('',array('name'=>'data[EkgResult][file_name][]','type'=>'file','class' => 'validate[required,custom[mandatory-select]] textBoxExpnd'));
									...
									<div id="icdSlc"   style="display:<?php echo $display ;?>;">
										...
										foreach($data as $temData){
													              	  					if($temData['EkgResult']['file_name']){
													              	  						...
													              	  						echo $this->Html->link($this->Html->image('/img/icons/cross.png'),array('action'=>'delete_report',$patient_id),array('escape'=>false,"align"=>"right","id"=>"$id" ,"title"=>"Remove"
								...
								<td valign="top" width="100%">
									...
									<table width="100%" cellpadding="0" cellspacing="0" border="0">
										...
										<tr>
											...
											$note = isset($doctorNote['note'])?$doctorNote['note']:'' ;
...
<p class="ht5"></p>
<p class="ht5"></p>
<div class="row_action" style='float: left;' >
	<?php 
	...
	if($data[0]['EkgResult']['confirm_result']==1){
								...
								echo $this->Form->input('EkgResult.result_publish_date',array('id'=>'result_publish_date','type'=>'text','autocomplete'=>false,'readonly'=>'readonly','autocomplete'=>'off','class' => 'validate[required,custom[mandatory-enter]]'));
							}
...
<script><!--
       ...
       $(function() {	
			...
			$("#ekgManagerfrm").validationEngine();
