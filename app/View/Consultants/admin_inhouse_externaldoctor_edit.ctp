...
<h3><?php echo __('Edit External Consultant', true); ?></h3>
</div>
<?php 
  if(!empty($errors)) {
?>
...
<?php } ?>
<form name="consultantsfrm" id="consultantsfrm" action="<?php echo $this->Html->url(array("controller" => "consultants", "action" => "inhouse_externaldoctor_edit", 'admin'=> true)); ?>" method="post" >
        <?php 
             echo $this->Form->input('Consultant.id', array('type' => 'hidden')); 
             // this id is for external consultant //
	...
	<td>
        <?php 
        echo $this->Form->textarea('Consultant.address1', array('cols' => '35', 'rows' => '10', 'id' => 'customaddress1', 'label'=> false, 'div' => false, 'error' => false));
        ?>
        </td>
	</tr>
        <tr>
	...
	<td>
        ...
        <tr>
	<td class="form_lables" align="right">
	<?php echo __('Enterprise Name',true); ?>
	</td>
	<td>
        <?php 
        echo $this->Form->input('Consultant.hospital_name', array('class' => '', 'id' => 'customhospitalname', 'label'=> false, 'div' => false, 'error' => false));
        ?>
	</td>
	</tr>
	<!--<tr>
	...
	<td>
        <?php 
        echo $this->Form->input('Consultant.education', array('class' => '', 'id' => 'customeducation', 'label'=> false, 'div' => false, 'error' => false));
        ?>
	</td>
	</tr>
        <tr>
	...
	<td>
        <?php 
        echo $this->Form->textarea('Consultant.specility_keyword', array('class' => '', 'cols' => '35', 'rows' => '10', 'id' => 'customspecility_keyword', 'label'=> false, 'div' => false, 'error' => false));
        ?>
        </td>
	</tr>
        <tr>
	...
	<td>
        <?php 
        if(!empty($this->data['Consultant']['dateofbirth']) || $this->data['Consultant']['dateofbirth'] !="") { 
           $dob = $this->DateFormat->formatDate2Local($this->data['Consultant']['dateofbirth'],Configure::read('date_format'),true);
        }
		 echo $this->Form->input('Consultant.dateofbirth', array('type' => 'text','readonly'=>'readonly', 'id' => 'customdateofbirth', 'label'=> false, 'div' => false,'class' => 'textBoxExpnd', 'error' => false, 'value'=> $dob));
	...
	<tr>
		...
		<td>
	        <?php 
	        if(empty($this->data['Consultant']['accounting_group_id'])){
			$Id = $groupId['AccountingGroup']['id'];
			}else{
			$Id = $this->data['Consultant']['accounting_group_id'];
			}
	        echo $this->Form->input('Consultant.accounting_group_id', array('type'=>'select','options'=>$group,'id' => 'group_id','class'=>'' ,'label'=> false, 'div' => false, 'error' => false,'empty'=>'Please Select','value'=>$Id));
	        ?>
		</td>
	</tr>
	<?php if($this->Session->read('website.instance')=='kanpur'){?>
	<!-- for accounting sharing doctor percentage by amit jain -->
	<?php $externalData = unserialize($this->data['Consultant']['doctor_commision']); ?>
	<tr>
		<td class="form_lables" align="right"><?php echo __('Doctor Commission',true); ?></td>
		<td>
			<span style="float:left;"><?php echo __('External Charges (%) '); ?></span>
			<?php echo $this->Form->input('Service.external_charges', array('value'=>$externalData['external_charges'],'type' => 'text','class' => 'textBoxExpnd','label' => false,'legend' => false ,'id'=>'externalCharges','style'=>"width:50px;",'maxlength'=>'3'));?>
			<span style="float:left;"><?php echo __('Hospital Charges (%) '); ?></span>
			<?php echo $this->Form->input('Service.hospital_charges', array('value'=>$externalData['hospital_charges'],'type' => 'text','class' => 'textBoxExpnd','label' => false,'legend' => false ,'id'=>'hospitalCharges','style'=>"width:50px;",'maxlength'=>'3'));?>
			<span style="float:left;"><?php echo __('From:'); ?></span>
			<?php echo $this->Form->input('Service.from', array('value'=>$externalData['from'],'class'=>'validate[required,custom[mandatory-enter]] textBoxExpnd','style'=>'width:70px','id'=>'from','label'=> false, 'div' => false, 'error' => false,'placeholder'=>'From','autocomplete'=>'off'));?>
		</td>	
	</tr>
	 <?php }?>
    <!--     <tr>
	...
	<tr>
	<td colspan="2" align="center">
        <?php echo $this->Html->link('Cancel',array("action" => "inhouse_externaldoctor", 'admin'=> true), array('style'=>'margin-left:83px;','class' => 'blueBtn','escape' => false)); ?>
