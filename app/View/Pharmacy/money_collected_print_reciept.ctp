	...
	<table border="0" class="table_format" cellpadding="0" cellspacing="0" width="100%" style="padding-top:<?php echo $paddingTop;?>; padding-left:<?php echo $paddingLeft;?>;">
		  ...
		  <tr>
		  	...
		  	<td> <?php  /* if($billingData['Billing']['refund']==1){
				  			...
				  			echo $this->RupeesToWords->no_to_words($billingData['PharmacySalesBill']['paid_amnt']); 
						//}?></td>
		  </tr>
		  <?php if($website!='vadodara'){?>
		  <tr>
		 ...
		 <?php if($website =='kanpur'){
		  ...
		  <tr style="display:<?php echo $display; ?>">
		  	<td>Remarks :</td>
		  	<td> <?php echo $billingData['Billing']['remark']; ?><?php if($website == 'kanpur') echo "&nbsp;".$billingData['Billing']['id']; ?></td>
		  </tr>
		<?php }?>
		  <tr>
		  	<td>Date :</td>
		  	<td> <?php echo $this->DateFormat->formatDate2Local($billingData['Billing']['date'],Configure::read('date_format'),false); ?></td>
		  </tr>
		<?php   if($website =='vadodara'){?>
		  <tr>
		  ...
		  <tr>
		  	<td><?php 
					if($billingData['PharmacySalesBill']['discount']/*  && $billingData['Billing']['refund']!=1 */)
		  			echo __("Discount : ") .$billingData['PharmacySalesBill']['discount'];?>
		  	</td>
		  </tr>
		  	...
		  	<td><?php 
		  		/* if($billingData['Billing']['refund']==1){
					echo $billingData['Billing']['paid_to_patient']."/-";
		  			...
		  			echo $billingData['PharmacySalesBill']['paid_amnt']."/-"; 
				//}
		  	...
		  	<?php echo $this->Html->image('icons/rupee_symbol.png',array('style'=>'float:none; vertical-align:top;'));?>&nbsp;<?php 
		  		/* if($billingData['Billing']['refund']==1){
					echo $billingData['Billing']['paid_to_patient']."/-";
		  			...
		  			echo $billingData['PharmacySalesBill']['paid_amnt'] -$billingData['PatientCard']['amount']."/-"; 
				//}?>
			</td>
			<td></td>
		  </tr>
