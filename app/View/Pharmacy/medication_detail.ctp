...
<?php
	...
	// Sort the detail array by create_time in ascending order (date-wise)
	if (!empty($detail)) {
		usort($detail, function($a, $b) {
			// Get create_time from either PharmacySalesBill or PharmacyDuplicateSalesBill
			$timeA = '';
			$timeB = '';

			if (isset($a['PharmacySalesBill']['create_time'])) {
				$timeA = $a['PharmacySalesBill']['create_time'];
			} elseif (isset($a['PharmacyDuplicateSalesBill']['create_time'])) {
				$timeA = $a['PharmacyDuplicateSalesBill']['create_time'];
			}

			if (isset($b['PharmacySalesBill']['create_time'])) {
				$timeB = $b['PharmacySalesBill']['create_time'];
			} elseif (isset($b['PharmacyDuplicateSalesBill']['create_time'])) {
				$timeB = $b['PharmacyDuplicateSalesBill']['create_time'];
			}
			...
			// Return comparison result for ascending order (earliest date first)
