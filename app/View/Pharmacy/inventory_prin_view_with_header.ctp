...
<?php	  
		...
		if($section == "PurchaseReceipt"){
			...
			$saleHeader = $this->render('print_header_kanpur');
			$heading = "Sales Bill";
		...
		if($section == "InventoryPharmacySalesReturnsDetail")
		{
			$discount  = $data['InventoryPharmacySalesReturn']['discount'];
	...
	<!-- 	<table width="90%" border="0" cellspacing="0" cellpadding="0" align="" > -->
				...
				<table width="90%" align="center" border="0" cellspacing="0" cellpadding="0" style="padding-left: 5px">
					     <tr>
						  ...
						  <td width="7%" class="boxBorderBot boxBorderRight boxBorderTop" style="text-align: center;">RATE</td>
						   <?php if($section == "InventoryPharmacyDirectSalesReturnsDetail" && $this->Session->read('website.instance')=='kanpur'){ ?> 
			...
			<tr>
				...
				<td valign="top" class="boxBorderRight" style="text-align: center; ">
					...
					<?php
						...
						if($section == "InventoryPharmacySalesReturnsDetail")
						{
							$getDiscountAmt=$grandtotal*($discount/100);
				...
				<?php if($section == "InventoryPharmacyDirectSalesReturnsDetail" && $this->Session->read('website.instance')=='kanpur'){ 	
				     	$actualAmnt=$pharItem['PharmacyItemRate']['sale_price'];
				     	$actualQty=$item['InventoryPharmacySalesReturnsDetail']['qty'];
				     	$actualTotal = $actualAmnt*$actualQty;
				     	if($itemType == "Tab"){
				     		$actualTotal =  ($actualAmnt/$packOFproduct)*$actualQty;
				     	}
				     	
				     	$grandActTotal= (!empty($actualQty)) ? $grandActTotal+$actualTotal : $grandActTotal;
					?> 
				<td  valign="top" class="boxBorderRight" style="font-size: 14px; text-align: center;"><?php echo !empty($actualQty)?number_format($actualTotal,2):'';unset($actualQty); ?></td>
				 <?php }?>
		...
		<table align="center" width="90%" class="footer" id="footer" style="padding-top:5px;">
	  	<tr>
		    <td width="90%">
		     ...
		     <tr>
		         <td width="350px">PHARMACIST : <?php echo $userName?></td>
		          <?php if(!empty($discount)){ ?>
		           <td style="text-align: right">DISCOUNT : <?php 
		           if($section == "InventoryPharmacySalesReturnsDetail"){
		           		echo number_format(round($getDiscountAmt),2);
		           }else{
		           echo number_format($discount,2);
						}?></td>
		          <?php } else if($section == "DirectSalesReturn" || $section == "InventoryPharmacyDirectSalesReturnsDetail"){?>
		          <td style="text-align: right">DISCOUNT : <?php
		           ...
		           echo number_format($returnTotalDiscount,2);?></td>
		          <?php }?>
		     </tr>
			...
			?>
				...
				//number_format((float)round($refundTotal),2,'.', '')

				if($section == "InventoryPharmacySalesReturnsDetail"){
					echo number_format(round($grandtotal-$getDiscountAmt),2);
				}else if($section == "DirectSalesReturn" || $section == "InventoryPharmacyDirectSalesReturnsDetail"){
	 ...
	 <!-- <table align="center"  width="100%"  border="0" cellspacing="0" cellpadding="0" style="padding-left: 5px;" id="footerRoman" > --> 
	 <table width="90%" align="center" border="0" cellspacing="0" cellpadding="0" id="footerRoman" >      
	       ...
	       <tr>
	         <td colspan="4"  width="90px" height="3px" style="background-color: red" ></td>
	       </tr>
	       <tr >
	         <td>
	         <table>
	         	<tr>
	         		<td><ul type="disc"><li>HERBAL MEDICINES</li></ul></td>
	         		<td><ul type="disc"><li>VACCINES</li></ul></td>
	         		<td><ul type="disc"><li>IMPORTED MEDICINES</li></ul></td>
	         	</tr>
	         </table>
	         </td>
