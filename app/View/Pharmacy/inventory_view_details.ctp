...
<?php
 ...
 foreach($allData as $data) {
     $date = '';
     if(isset($data['PharmacyDuplicateSalesBill']['modified_date']) && !empty($data['PharmacyDuplicateSalesBill']['modified_date'])){
         $date = $data['PharmacyDuplicateSalesBill']['modified_date'];
     }elseif(isset($data['PharmacyDuplicateSalesBill']['create_time'])){
         $date = $this->DateFormat->formatDate2Local($data['PharmacyDuplicateSalesBill']['create_time'],Configure::read('date_format'),false);
     }elseif(isset($data['PharmacySalesBill']['modified_date']) && !empty($data['PharmacySalesBill']['modified_date'])){
         $date = $data['PharmacySalesBill']['modified_date'];
     }elseif(isset($data['PharmacySalesBill']['create_time'])){
         $date = $this->DateFormat->formatDate2Local($data['PharmacySalesBill']['create_time'],Configure::read('date_format'),false);
     }
 ...
 foreach($saleReturn as $data) {
     $date = $this->DateFormat->formatDate2Local($data['InventoryPharmacySalesReturn']['create_time'],Configure::read('date_format'),false);
     $allTransactions[] = array(
         'date' => $date,
         'type' => 'return',
         'data' => $data
 ...
 usort($allTransactions, function($a, $b) {
     // Convert date format from d/m/Y to Y-m-d for proper sorting
     $dateA = '';
     $dateB = '';
     ...
     if(!empty($a['date'])) {
         ...
         if(count($partsA) == 3) {
             $dateA = $partsA[2] . '-' . $partsA[1] . '-' . $partsA[0];
         }
     }
     ...
     if(!empty($b['date'])) {
         ...
         if(count($partsB) == 3) {
             $dateB = $partsB[2] . '-' . $partsB[1] . '-' . $partsB[0];
         }
     }
 ...
 foreach($allTransactions as $transaction) { $count++;
