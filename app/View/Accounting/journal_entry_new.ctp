...
<table width="100%">
	<tr>
		<td style="float :left">
			<?php echo __('Journal No. :'); echo $creditData['VoucherEntry']['id'];?>
		</td>
		<td style="float :right">
			<?php echo $this->Form->input('VoucherEntry.date', array('label'=>false,'type'=>'text','value'=>$this->DateFormat->formatDate2Local($creditData['VoucherEntry']['date'],Configure::read('date_format'),true),
				...
				<table cellspacing="1" cellpadding="0" border="0" width="100%" class="tabularForm">	
					<tr>
						...
						<?php echo $this->Form->input('Entry.credit_amount',array('placeholder'=>'Credit Amount','type'=>'text',
							'id'=>'credit_amount','class'=>'validate[optional,custom[onlyNumber]] inputBox cost','value'=>round($creditData['VoucherEntry']['credit_amount'])));?>
						</td>
...
<!-- Credit fields -->
		<tr id="cRow1">
			<td colspan="3">
				<table cellspacing="1" cellpadding="0" border="0" width="100%" class="tabularForm">
				<?php foreach ($dataDetail as $key=>$data){ ?>
				<tr>
					...
					<?php if(!empty($data['VoucherEntry']['debit_amount'])){ 
								 echo $this->Form->input("VoucherEntry.$key.debit_amount",array('placeholder'=>'Debit Amount','type'=>'text',
									'id'=>'debit_amount','value'=>$data['VoucherEntry']['debit_amount'],'class'=>'debit_amount inputBox cost')); 
							}else{
								 echo $this->Form->input("VoucherEntry.$key.debit_amount",array('placeholder'=>'Debit Amount','type'=>'text',
									'id'=>'debit_amount' ,'class'=>'debit_amount inputBox cost'));
							}?>
					</td>
				</tr>
				...
				<tr>
					...
					<?php if($data['AccountAlias']['balance'] < '0'){
						...
						<b><span id="to_current_balance-1" ><font color="red"><?php echo $amount;?></font></span></b>
					</td>
