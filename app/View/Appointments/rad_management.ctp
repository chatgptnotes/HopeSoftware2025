...
?>
		<?php  		echo $this->Form->create('Appointment',array('action'=>'rad_patient_dashboard','default'=>false,'id'=>'content-form')); ?>
<table width="100%">
	<tr>
			...
			$('#patient-filter').click(function(){
				...
				<?php 
					...
					$this->Js->event(
							'change',
							$this->Js->request(
									array('action' => 'rad_patient_dashboard',),
		...
		<div style="float:left; margin:10px 0px 0px;" class="tdLabel"><?php 
			...
			$this->Js->event('click',
							$this->Js->request(
							array('action' => 'rad_patient_dashboard','future'),
							array('method'=>'POST','dataExpression'=>true,'data'=> $this->Js->serializeForm(
									array(
											'isForm' => false,
											'inline' => true
		...
		<?php 
			...
			<?php $this->Js->get('#closed-filter');
				$this->Js->event('click',
								$this->Js->request(
								array('action' => 'rad_patient_dashboard','closed'),
								array('method'=>'POST','dataExpression'=>true,'data'=> $this->Js->serializeForm(
										array(
												'isForm' => false,
												'inline' => true
		...
		<?php 
			...
			<?php $this->Js->get('#myPatient-filter');
				$this->Js->event('click',
								$this->Js->request(
								array('action' => 'rad_patient_dashboard',$role),
								array('method'=>'POST','dataExpression'=>true,'data'=> $this->Js->serializeForm(
										array(
												'isForm' => false,
												'inline' => true
    ...
    <?php }?>
		...
		if($this->Session->read('website.instance')!='vadodara'){
			...
			/*echo $this->Html->link(__('Display Token no.'),
			 	array("action"=>'patientQueque'),array('escape'=>false,'class'=>'blueBtn')); */
		}?></div>
...
<!--   <td width="17%" class="tdLabel" style=" padding-left:0px !important;">
	...
	<tr><td colspan="4" style="padding-bottom: 10px" ><?php /*$options  = array('OPD'=>'<b>OPD Patients</b>','Lab'=>'<b>Laboratory Patients</b>');
	...
	<div style="padding-left: 16px">			
				...
				<input id="radio2" name="radios" value="Lab" type="radio" class='typeSelected'>
				   <label for="radio2"><font color="#3185AC">Laboratory</font></label>
				<input id="radio3" name="radios" value="Rad" checked type="radio" class='typeSelected'>
				   <label for="radio3"><font color="#3185AC">Radiology</font></label> 
				   
		 <?php if($this->Session->read('website.instance')=='vadodara'){?>
		    <span class="" style="margin: 0;float: right;">
		     <?php echo __(' Not Done');
		           echo $this->Html->image('icons/small_red.png',array("title"=>"Done","alt"=>"DONE","label"=>true));?>
			</span>
			<span class="" style="margin: 0; float: right;">
		    <?php
		    echo __(' Done');
		
		    echo  $this->Html->image('icons/small_green.png',array("title"=>"Done","alt"=>"DONE","label"=>true));?>
			</span>
			<span class="" style="margin: 0; float: right;">
		    <?php 
		    echo __('Partially Done');
		    echo $this->Html->image('icons/small_orange.png',array("title"=>"Done","alt"=>"DONE","label"=>true));?>
			</span>
			<?php }?>
		</div>
</td></tr>
...
<script>	 
	...
	function load_list(refresh){
		...
		url="<?php echo $this->Html->url(array("controller" => 'appointments', "action" => "rad_patient_dashboard","admin" => false)); ?>";
	...
	function refresh_list(){
		...
		setInterval(function(){
			...
			$.ajax({
			   type : "POST",
		       url: "<?php echo $this->Html->url(array("controller" => "Appointments", "action" => "rad_count", "admin" => false)); ?>",
		       context: document.body,
		       success: function( data ){
	...
	$(document).ready(function (){ 
		...
		$("#patient-filter").autocomplete({
			source: "<?php echo $this->Html->url(array("controller" => "Appointments", "action" => "testcomplete",'RAD',"admin" => false,"plugin"=>false)); ?>",
...
$(document).on('change',".nurse",function(){
	...
	$.ajax({
		  type : "POST",
		  url: "<?php echo $this->Html->url(array("controller" => "Appointments", "action" => "rad_patient_dashboard", "admin" => false)); ?>"+"/"+null+"/"+ptId+"/"+opt+"/"+aptId,
		  context: document.body,	
		  data : "value="+value,
		  beforeSend:function(){
			  loading();
...
$(document).on('change',".room",function(){
	...
	$.ajax({
		  type : "POST",
		  url: "<?php echo $this->Html->url(array("controller" => "Appointments", "action" => "rad_patient_dashboard", "admin" => false)); ?>"+"/"+null+"/"+aptId+"/"+opt,
		  context: document.body,	
		  data : "value="+value,
		  beforeSend:function(){
			  loading();
