...
<div class="">
	...
	<table border="0" cellpadding="0" cellspacing="0" width="100%"
		...
		<tr class="row_title">
			...
			<td width="5%" style="text-align: center;" valign="top"
				class="table_cell"><?php echo __("Initials");?></td>
			<td width="2%" style="text-align: center;" valign="top"
			...
			if($this->Session->read("website.instance")!='lifespring'){ ?>
				...
				<?php } //}  if(($role==Configure::read('doctorLabel')||$role==Configure::read('nurseLabel') || $role==Configure::read('medicalAssistantLabel') || $role==Configure::read('residentLabel')) && $future!='future'){ ?>
			<?php if(($role==Configure::read('doctorLabel')||$role==Configure::read('nurseLabel') || $role == Configure::read('adminLabel'))){?>
			<td width="1%" style="text-align: center;" valign="top"
				class="table_cell"><?php echo __("Epen");?></td>
			<?php }?>
			<?php if($this->Session->read("website.instance")!='lifespring'){ 
				if(($role==Configure::read('doctorLabel')||$role==Configure::read('nurseLabel') || $role == Configure::read('adminLabel'))){ ?>
							<td width="1%" style="text-align: center;" valign="top"
	...
	if($future != 'future'){
		...
		<td style="text-align: center;" class="tdLabel tooltip" id="boxSpace"
			title="<?php echo $tooltip?>"><strong><?php //BOF-Mahalaxmi
			$tooltip = '';
			$getPatientFullName=explode(" ",$appointment['Patient']['lookup_name']);
			$getInitalName=substr($getPatientFullName[0], 0, 1).substr($getPatientFullName[1], 0, 1);
			$getInitalUpperName=strtoupper($getInitalName);
			echo $getInitalUpperName;
			//Pic
		
		/*if(file_exists(WWW_ROOT."/uploads/patient_images/thumbnail/".$appointment['Person']['photo']) && !empty($appointment['Person']['photo'])){  echo			$this->Html->link($this->Html->image("/uploads/patient_images/thumbnail/".$appointment['Person']['photo'], array('width'=>'50','height'=>'50','class'=>'pateintpic','title'=>'Edit EMPI','alt'=>'Edit EMPI')),array('controller'=>'Persons','action'=>'edit',$appointment['Person']['id'],"?"=>array('from'=>"appointments_management")), array('escape' => false)); 
			}else{
		...
		<td
			...
			$name=trim($appointment['Patient']['lookup_name']).$admission_id;
		...
		$disabled ="" ;
		/* By pankaj no need of other options
		 if(!empty($appointment['Appointment']['status']) && ($appointment['Appointment']['status']=='Arrived') &&
		 		$appointment['Appointment']['is_future_app']==0){
		echo"1";
		$class = "seen-status textBoxExpnd" ;
		$options= $seen_status;
		$value= "" ;
		}else if(!empty($appointment['Appointment']['seen_status']) && empty($appointment['Appointment']['chamber_id'])){
						echo"2";
		$class = "seen-status textBoxExpnd" ;
		$options= $seen_status;
		$value= $appointment['Appointment']['seen_status'] ;
		}else if(!empty($appointment['Appointment']['seen_status']) && !empty($appointment['Appointment']['chamber_id'])){
						echo"3";
		$class = "seen-status textBoxExpnd" ;
		$options=   array('In Room'=>'In Room :'.$appointment['Chamber']['name'],'Seen'=>'Seen') ;
		$value= $appointment['Appointment']['seen_status'] ;
		if($value=='Seen')$disabled = "disabled" ;
		}else{
						echo"4";
		$class = "status textBoxExpnd" ;
		$options= $status;
	...
	/*	if($role==Configure::read('doctorLabel')){
		...
		<!--  <td style="text-align: center;" class="tdLabel" id="boxSpace"><?php //eligibility check
			...
			/*if($appointment['NewInsurance']['is_eligible']=='0' || $appointment['NewInsurance']['is_eligible']==''){
			    echo $this->Html->link($this->Html->image('icons/red.png',array()),
					 array('controller'=>'Patients','action'=>'insuranceindex',$appointment['Appointment']['patient_id'],'?'=>array('person_id'=>$appointment['Appointment']['person_id'])), array('escape' => false));
			}else */

			if($appointment['Person']['payment_category']=='Insurance'){
				echo $this->Html->link($this->Html->image('icons/green.png',array()),
					array('controller'=>'Patients','action'=>'insuranceindex',$appointment['Appointment']['patient_id']), array('escape' => false));
			}else{
				echo $this->Html->link($this->Html->image('icons/red.png',array()),
				array('controller'=>'Patients','action'=>'insuranceindex',$appointment['Appointment']['patient_id'],'?'=>array('person_id'=>$appointment['Appointment']['person_id'])), array('escape' => false));
			}?>
		...
		<?php if(($role==Configure::read('doctorLabel') || $role==Configure::read('nurseLabel') || $role==Configure::read('medicalAssistantLabel')  || $role==Configure::read('residentLabel'))&& $future!='future'){
			...
			$medTDid = 'med_'.$appointment['Appointment']['patient_id'].'_'.$appointment['Appointment']['id'].'_'.$appointment['Patient']['patient_id'];
			if(!empty($appointment['NewCropPrescriptionAlias']['id'])||!empty($appointment['NewCropPrescription']['id'])){
					if($appointment['NewCropPrescription']['is_med_administered']!='1'|| !empty($appointment['VerifyMedicationOrder']['id'])){
				         $medClass=  "med greenBulb MEDUNIQUECLASS_".$appointment['Appointment']['patient_id'];
			         }else{
				         $medClass=  "med redBulb MEDUNIQUECLASS_".$appointment['Appointment']['patient_id'];
			         }
		        }else{
			        $medClass=  "med greyBulb MEDUNIQUECLASS_".$appointment['Appointment']['patient_id'];
		             }?>
		<td>
		...
		<td style="image-align: center;" class="tdLabel" id="boxSpace"><?php // Patient credentials
		             $pat_uid=trim($appointment['Appointment']['patient_id']);
		...
		if (in_array($appointment['Appointment']['patient_id'], $criticalArray)) {
					if($appointment['Patient']['is_dr_chk'] != 0){
						...
						if($appointment['Appointment']['status']=='' || $appointment['Appointment']['status'] == 'Scheduled' || $appointment['Appointment']['status']=='Pending' || $appointment['Appointment']['status']=='Confirmed with Patient'){
							...
							echo $this->Html->link($this->Html->image('icons/red.png',array()),array("controller" => "Messages", "action" => "compose",$appointment['Appointment']['patient_id'],'null','alert',$appointment['Appointment']['id'],'arrive_time='.$appointment['Appointment']['arrived_time']), array('escape' => false,'class'=>'manage_alert','id'=>'alert_'.$appointment['Appointment']['patient_id']));
						}
					}
				}else{
		...
		<td style="text-align: left;" class="tdLabel" id="boxSpace"><?php //Critical results
				/** lab Counting - gaurav*/
				$labOrder=count($labCount[$appointment['Appointment']['patient_id']]);
				$countOfResult = 0;
				$isAbnormal=false;
				$showFlag=false;
				foreach($labCount[$appointment['Appointment']['patient_id']] as $resultCount){
		   ...
		   if($labOrder > 0)
		   	//$labResUrl = array('controller'=>'laboratories','action'=>'labTestHl7List',$appointment['Appointment']['patient_id'],'?'=>array('return'=>'laboratories')) ;
		   $labResUrl = array('controller'=>'Notes','action'=>'clinicalNote',$appointment['Appointment']['patient_id'],$appointment['Appointment']['id']) ;
		   else
		   	$labResUrl = "#" ;

		   $flag = ($showFlag)?$this->Html->image('icons/context/flag.png',array('style'=>'float: right;','id'=>'labOverDue_'.$appointment['Appointment']['patient_id'],'class'=>'labRadOverDue')) : '';
		   if($appointment['Appointment']['status']=='' || $appointment['Appointment']['status'] == 'Scheduled' || $appointment['Appointment']['status']=='Pending' || $appointment['Appointment']['status']=='Confirmed with Patient'|| $appointment['Appointment']['status']=='Cancelled' ||$appointment['Appointment']['status']=='No-Show'){
		   ...
		   if($role==Configure::read('doctorLabel') || $role==Configure::read('nurseLabel')){
             ...
             /** End of lab */
             /** Rad Counting */
             $radOrder=count($radCount[$appointment['Appointment']['patient_id']]);
             $countOfResult = 0;
             $showFlag=false;
             $isAbnormal=false;
             foreach($radCount[$appointment['Appointment']['patient_id']] as $resultCount){
				$countOfResult = ($resultCount['is_Resulted'] == 1) ? $countOfResult+1 : $countOfResult;
				$showFlag = ($showFlag)? true : $resultCount['showFlag'];
				$isAbnormal = ($isAbnormal)? true : $resultCount['abnormalFlag'];
		    }
		    $flag = ($showFlag)?$this->Html->image('icons/context/flag.png',array('style'=>'float: right;','id'=>'radOverDue_'.$appointment['Appointment']['patient_id'],'class'=>'labRadOverDue')) : '';
		    ...
		    if($radOrder > 0)
		    	$radResUrl = array('controller'=>'radiologies','action'=>'radiology_test_list',$appointment['Appointment']['patient_id'],'?'=>array('return'=>'radiologies')) ;
		    else
		    	$radResUrl = "#" ;
		    if($appointment['Appointment']['status']=='' || $appointment['Appointment']['status'] == 'Scheduled' || $appointment['Appointment']['status']=='Pending' || $appointment['Appointment']['status']=='Confirmed with Patient'|| $appointment['Appointment']['status']=='Cancelled' ||$appointment['Appointment']['status']=='No-Show'){
            ...
            /** End of Rad */
            ?></td>
            <?php /*if($referalCount[$appointment['Appointment']['patient_id']]['nCount']){
            	$denoCount=$referalCount[$appointment['Appointment']['patient_id']]['nCount'];
            	$referName=implode(',',$referSpecialist[$appointment['Appointment']['patient_id']]);
            	$referalSpeciality=implode(',',$referSpeciality[$appointment['Appointment']['patient_id']]);
				}else{
				$denoCount='0';
				}
				if($referalCount[$appointment['Appointment']['patient_id']]['yCount']){
            	$numCount=$referalCount[$appointment['Appointment']['patient_id']]['yCount'];
				}else{
				$numCount='0';
				}
          ...
          /*  echo $referal;
            ...
            if(empty($denoCount)){
				...
				echo $this->Html->link("$numCount/$denoCount",array('controller'=>'Messages','action'=>'composeCcda',$appointment['Appointment']['patient_id'],'?'=>array('referred_to'=>$referName)),array('escape'=>false));}*/?></td>-->
		<?php 
}?>
		...
		<td style="image-align: center;" class="tdLabel" id="boxSpace"><?php //debug($appointment['Diagnosis']);//initial
				if(!empty($appointment[0]['initCount'])){
						if($appointment['Appointment']['is_future_app']=='1'){
							...
							if($appointment['Appointment']['status']=='' || $appointment['Appointment']['status'] == 'Scheduled' || $appointment['Appointment']['status']=='Pending' || $appointment['Appointment']['status']=='Confirmed with Patient'|| $appointment['Appointment']['status']=='Cancelled' ||$appointment['Appointment']['status']=='No-Show'){
								echo $this->Html->image('icons/red.png',array('style'=>'cursor:not-allowed;','title'=>'Patient Not arrived yet/Not Assigned nurse/room'));
					 			...
					 			echo $this->Html->link($this->Html->image('icons/green.png',array()),array("controller" => "Diagnoses", "action" => "initialAssessment",$appointment['Appointment']['patient_id'],$appointment['Diagnosis']['id'],$appointment['Appointment']['id'],'?'=>array('type'=>$appointment['Appointment']['arrived_time']),"admin" => false),array('escape'=>false,'class'=>'initialGreen','id'=>'initial_'.$appointment['Appointment']['id'].'_'.$appointment['Appointment']['patient_id']));
							}
						}
					}else{
						if($appointment['Appointment']['status']=='' || $appointment['Appointment']['status'] == 'Scheduled' || $appointment['Appointment']['status']=='Pending' || $appointment['Appointment']['status']=='Confirmed with Patient' || $appointment['Appointment']['status']=='Cancelled' ||$appointment['Appointment']['status']=='No-Show' /* || $appointment['Appointment']['status']=='Arrived' */){
							...
							echo $this->Html->link($this->Html->image('icons/red.png',array()),'javascript:void(0)',array('escape'=>false,'class'=>'initial','id'=>'initial_'.$appointment['Appointment']['id'].'_'.$appointment['Appointment']['patient_id']));
						}
					} 
				//}?>
		</td> 
		...
		if($billData[$appointment['Appointment']['patient_id']]['total']!=''){
			$manTotal=$billData[$appointment['Appointment']['patient_id']]['total']-($billData[$appointment['Appointment']['patient_id']]['paid']+$billData[$appointment['Appointment']['patient_id']]['discount']);
		}else{
			$manTotal=1;// if no record in Billing for mandatory services payment category
		}
 
		
		if(($appointment['Person']['vip_chk']!='1' || $appointment['Patient']['tariff_standard_id']!=$privateId) && empty($billData[$appointment['Appointment']['patient_id']]['paid']) ){
			...
			if(empty($appointment['Note']['id']) ||empty($appointment['Note']['sign_note']) ||($appointment['Note']['sign_note']==0) ){
					if(!empty($appointment['Note']['id'])){
						if($appointment['Appointment']['status']=='' || $appointment['Appointment']['status'] == 'Scheduled' || $appointment['Appointment']['status']=='Pending' || $appointment['Appointment']['status']=='Confirmed with Patient' || $appointment['Appointment']['status']=='Cancelled' ||$appointment['Appointment']['status']=='No-Show'/* || $appointment['Appointment']['status']=='Arrived'*/){
							...
							if($appointment['Appointment']['status']=='' || $appointment['Appointment']['status'] == 'Scheduled' || $appointment['Appointment']['status']=='Pending' || $appointment['Appointment']['status']=='Confirmed with Patient' || $appointment['Appointment']['status']=='Cancelled' ||$appointment['Appointment']['status']=='No-Show' /*|| $appointment['Appointment']['status']=='Arrived'*/){
                                ...
                                if(!empty($appointment['Patient']['epenImages']))
								   echo $this->Html->link($this->Html->image('icons/green.png',array()),array('controller'=>'Notes','action'=>'epen',$appointment['Appointment']['patient_id']),array(/*'id'=>$appointment['Appointment']['patient_id'],*/'escape'=>false,'title'=>'Documentation Complete','class'=>'','id'=>''/*,'class'=>'context-menu-one'*/));
                                else
                                	echo $this->Html->image('icons/red.png',array('style'=>'cursor:not-allowed;','title'=>'Patient Not arrived yet/Not Assigned nurse/room'));
						...
						if($appointment['Appointment']['is_future_app']=='1'){
                            ...
                            if(!empty($appointment['Patient']['epenImages']))
	                              echo $this->Html->link($this->Html->image('icons/green.png',array()),array('controller'=>'notes','action'=>'epen',$appointment['Appointment']['patient_id']),
			                     array('class'=>'docSigned','id'=>'docSigned_'.$appointment['Appointment']['id'].'_'.$appointment['Appointment']['patient_id'].'_'.$appointment['Note']['id'],'escape'=>false/* ,'title'=>'Documentation Complete/Right Click To view Rx ','class'=>'context-menu-one' */));
                            else
							  echo $this->Html->link($this->Html->image('icons/red.png',array()),array('controller'=>'notes','action'=>'epen',$appointment['Appointment']['patient_id']),
								array('class'=>'','id'=>'','escape'=>false ,'title'=>'Documentation Not Completed '/*,'class'=>'context-menu-one' */));


						//		echo $this->Html->link($this->Html->image('icons/red.png',array()),'javascript:void(0)',array('class'=>'doc_clicked'/* ,'title'=>'Right Click To view Rx ' */)),array('controller'=>'Notes','action'=>'clinicalNote',$appointment['Appointment']['patient_id'],$appointment['Appointment']['id'],'?'=>array('arrived_time'=>$appointment['Appointment']['arrived_time'])),array('id'=>$appointment['Appointment']['patient_id'],'escape'=>false,'title'=>'Documentation Not Completed'/*,'class'=>'context-menu-one'*/));
        		...
        		if($appointment['Appointment']['is_future_app']=='1'){
                   ...
                   if(!empty($appointment['Patient']['epenImages']))
						echo $this->Html->link($this->Html->image('icons/green.png',array()),array('controller'=>'notes','action'=>'epen',$appointment['Appointment']['patient_id']),
							 array('class'=>'','id'=>'','escape'=>false/* ,'title'=>'Documentation Complete/Right Click To view Rx ','class'=>'context-menu-one' */));
                   else
                   	echo $this->Html->image('icons/red.png',array('style'=>'cursor:not-allowed;','title'=>'Patient Not arrived yet'));

						//echo $this->Html->link($this->Html->image('icons/green.png',array()),array('controller'=>'PatientForms','action'=>'power_note',$appointment['Note']['id'],$appointment['Appointment']['patient_id'],'?'=>array('appointmentId'=>$appointment['Appointment']['id'])),
						//	 array('class'=>'docSigned','id'=>'docSigned_'.$appointment['Appointment']['id'].'_'.$appointment['Appointment']['patient_id'],'escape'=>false/* ,'title'=>'Documentation Complete/Right Click To view Rx ','class'=>'context-menu-one' */));
						}
						//echo $this->Html->link($this->Html->image('icons/red.png',array()),'javascript:void(0)',array('escape'=>false,'class'=>'initial','id'=>'initial_'.$appointment['Appointment']['id'].'_'.$appointment['Appointment']['patient_id']));
					}
				}
			...
			if(empty($appointment['Note']['id']) ||empty($appointment['Note']['sign_note']) ||($appointment['Note']['sign_note']==0) ){
					if(!empty($appointment['Note']['id'])){
						if($appointment['Appointment']['status']=='' || $appointment['Appointment']['status'] == 'Scheduled' || $appointment['Appointment']['status']=='Pending' || $appointment['Appointment']['status']=='Confirmed with Patient' || $appointment['Appointment']['status']=='Cancelled' ||$appointment['Appointment']['status']=='No-Show'/* || $appointment['Appointment']['status']=='Arrived'*/){
							...
							if($appointment['Appointment']['status']=='' || $appointment['Appointment']['status'] == 'Scheduled' || $appointment['Appointment']['status']=='Pending' || $appointment['Appointment']['status']=='Confirmed with Patient' || $appointment['Appointment']['status']=='Cancelled' ||$appointment['Appointment']['status']=='No-Show' /*|| $appointment['Appointment']['status']=='Arrived'*/){
								...
								echo $this->Html->link($this->Html->image('icons/green.png',array()),'javascript:void(0)'/*,array('controller'=>'Notes','action'=>'clinicalNote',$appointment['Appointment']['patient_id'],$appointment['Appointment']['id'],$appointment['Note']['id'],'?'=>array('arrived_time'=>$appointment['Appointment']['arrived_time']))*/,array(/*'id'=>$appointment['Appointment']['patient_id'],*/'escape'=>false,'title'=>'Documentation Complete','class'=>'docSigned','id'=>'docSigned_'.$appointment['Appointment']['id'].'_'.$appointment['Appointment']['patient_id'].'_'.$appointment['Note']['id']/*,'class'=>'context-menu-one'*/));
							}
						}
					}else{
						if($appointment['Appointment']['is_future_app']=='1'){
							...
							echo $this->Html->link($this->Html->image('icons/red.png',array()),'javascript:void(0)',/*array('controller'=>'PatientForms','action'=>'power_note',$appointment['Note']['id'],$appointment['Appointment']['patient_id'],'?'=>array('appointmentId'=>$appointment['Appointment']['id'])),*/
								array('class'=>'docSigned doc_clicked','id'=>'docSigned_'.$appointment['Appointment']['id'].'_'.$appointment['Appointment']['patient_id'].'_'.$appointment['Note']['id'],'escape'=>false ,'title'=>'Documentation Not Completed '/*,'class'=>'context-menu-one' */));
        		...
        		if($appointment['Appointment']['is_future_app']=='1'){
					...
					if($appointment['Appointment']['status']=='' || $appointment['Appointment']['status'] == 'Scheduled' || $appointment['Appointment']['status']=='Pending' || $appointment['Appointment']['status']=='Confirmed with Patient' || $appointment['Appointment']['status']=='Cancelled' ||$appointment['Appointment']['status']=='No-Show' /*|| $appointment['Appointment']['status']=='Arrived'*/){
						...
						echo $this->Html->link($this->Html->image('icons/green.png',array()),'javascript:void(0)',/*array('controller'=>'PatientForms','action'=>'power_note',$appointment['Note']['id'],$appointment['Appointment']['patient_id'],'?'=>array('appointmentId'=>$appointment['Appointment']['id'])),*/
							 array('class'=>'docSigned','id'=>'docSigned_'.$appointment['Appointment']['id'].'_'.$appointment['Appointment']['patient_id'].'_'.$appointment['Note']['id'],'escape'=>false/* ,'title'=>'Documentation Complete/Right Click To view Rx ','class'=>'context-menu-one' */));
	...
	<?php }?>
		...
		<td style="text-align: center;" class="tdLabel" id="boxSpace"><?php //Provider
					...
					echo $this->Form->input('doctor_id',array('style'=>'width:100px;','type'=>'select','label'=>false,'disabled'=>'disabled',
						...
						if(empty($appointment['Appointment']['doctor_id'])){
							echo $this->Form->input('doctor_id',array('style'=>'width:100px;','type'=>'select','label'=>false,'empty'=>'Please Select',
								'options'=>$doctors,'value'=>$appointment['Appointment']['doctor_id'],'class'=>'currentDropdown doctor',
								'id'=>'doctor_'.$appointment['Appointment']['id'].'_'.$appointment['Appointment']['doctor_id'].'_'.$appointment['Appointment']['patient_id']));
						}
						else{
				...
				<?php //Room 
					...
					echo $this->Form->input('room_id',array('style'=>'width:100px;','type'=>'select','label'=>false,'disabled'=>'disabled',
						 ...
						 'id'=>'room_'.$appointment['Appointment']['id'],'patient_id'=>$appointment['Appointment']['patient_id'],'person_id'=>$appointment['Appointment']['person_id']));
				}else{
				    if($appointment['Appointment']['is_future_app']=='1'){
		...
		<?php //Nurse
				if($appointment[Appointment]['status']==''||$appointment[Appointment]['status']=='Scheduled'||$appointment['Appointment']['status']=='Pending'||$appointment['Appointment']['status']=='Cancelled'||$appointment['Appointment']['status']=='No-Show'){
						echo $this->Form->input('nurse_id',array('style'=>'width:100px;','type'=>'select','label'=>false,'disabled'=>'disabled',
						     'options'=>$nurses,'empty'=>"Select Nurse",'value'=>'','class'=>'currentDropdown nurse','patient_id'=>$appointment['Appointment']['patient_id'],'person_id'=>$appointment['Appointment']['person_id'],
							 'id'=>'nurse_'.$appointment['Appointment']['patient_id'].'_'.$appointment['Appointment']['id']));
					}else{
						if($appointment['Appointment']['is_future_app']=='1'){
							echo $this->Form->input('nurse_id',array('style'=>'width:100px;','type'=>'select','label'=>false,
								...
								if(empty($appointment['Appointment']['nurse_id'])){
										echo $this->Form->input('nurse_id',array('style'=>'width:100px;','type'=>'select','label'=>false,
											 'options'=>$nurses,'empty'=>"Select Nurse",'value'=>$appointment['Appointment']['nurse_id'],'class'=>'currentDropdown nurse','disabled'=>$disabled,'patient_id'=>$appointment['Appointment']['patient_id'],'person_id'=>$appointment['Appointment']['person_id'],
						                     'id'=>'nurse_'.$appointment['Appointment']['patient_id'].'_'.$appointment['Appointment']['id']));
						         }else{
							            echo $this->Form->input('nurse_id',array('style'=>'width:100px;','type'=>'select','label'=>false,
								             'options'=>$nurses,'value'=>$appointment['Appointment']['nurse_id'],'class'=>'currentDropdown nurse'/*,'disabled'=>$disabled*/,'patient_id'=>$appointment['Appointment']['patient_id'],'person_id'=>$appointment['Appointment']['person_id'],
								             'id'=>'nurse_'.$appointment['Appointment']['patient_id'].'_'.$appointment['Appointment']['id']));
								 }
							}
					 }//end of else ?>
		...
		if($isFingerPrintEnable==1)
		   $taskOptions=array('qrCode'=>'Print QR Code',/* 'packageEstimate'=>'Estimate', */'Patient Survey'=>'Patient Survey','fingerprint'=>'Capture Fingerprint','printsheet'=>'Print Sheet');
			...
			$taskOptions=array('qrCode'=>'Print QR Code',/* 'packageEstimate'=>'Estimate', */'Patient Survey'=>'Patient Survey','printsheet'=>'Print Sheet','printsheetinvoice'=>'Print sheet Invoice');
		...
		<td style="text-align: center;" class="tdLabel td_ht" id="boxSpace"><?php //Nurse
				if($appointment[Appointment]['status']==''||$appointment[Appointment]['status']=='Scheduled'||$appointment['Appointment']['status']=='Pending'||$appointment['Appointment']['status']=='Cancelled'||$appointment['Appointment']['status']=='No-Show'){
						echo $this->Form->input('nurse_id',array('style'=>'width:100px;','type'=>'select','label'=>false,'disabled'=>'disabled',
						     'options'=>$nurses,'empty'=>"Select",'value'=>'','class'=>'currentDropdown nurse','patient_id'=>$appointment['Appointment']['patient_id'],'person_id'=>$appointment['Appointment']['person_id'],
							 'id'=>'nurse_'.$appointment['Appointment']['patient_id'].'_'.$appointment['Appointment']['id']));
					}else{
						if($appointment['Appointment']['is_future_app']=='1'){
							echo $this->Form->input('nurse_id',array('style'=>'width:100px;','type'=>'select','label'=>false,
								...
								if(empty($appointment['Appointment']['nurse_id'])){
										echo $this->Form->input('nurse_id',array('style'=>'width:100px;','type'=>'select','label'=>false,
											 'options'=>$nurses,'empty'=>"Select",'value'=>$appointment['Appointment']['nurse_id'],'class'=>'currentDropdown nurse','disabled'=>$disabled,'patient_id'=>$appointment['Appointment']['patient_id'],'person_id'=>$appointment['Appointment']['person_id'],
						                     'id'=>'nurse_'.$appointment['Appointment']['patient_id'].'_'.$appointment['Appointment']['id']));
						         }else{
							            echo $this->Form->input('nurse_id',array('style'=>'width:100px;','type'=>'select','label'=>false,
								             'options'=>$nurses,'value'=>$appointment['Appointment']['nurse_id'],'class'=>'currentDropdown nurse'/*,'disabled'=>$disabled,'patient_id'=>$appointment['Appointment']['patient_id'],'person_id'=>$appointment['Appointment']['person_id'],
...
//if the staus is closed then the button will be green
						...
						if($appointment['Appointment']['status']=='' || $appointment['Appointment']['status'] == 'Scheduled' || $appointment['Appointment']['status']=='Pending'){
							...
							echo $this->Html->link($this->Html->image('icons/red.png',array('class'=>'encounter','id'=>'encounter_'.$appointment['Appointment']['person_id'].'_'.$appointment['Appointment']['id'],'title'=>'Positive ID')),'javascript:void(0)',array('escape'=>false));
						}
				}
