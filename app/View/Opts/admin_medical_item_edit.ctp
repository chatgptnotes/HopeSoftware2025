...
<div class="inner_title">
 <h3><?php echo __('Edit Medical Item', true); ?></h3>
</div>
<script>
...
<?php } ?>
<form name="optitemfrm" id="medicalItem" action="" method="post" >
        <?php 
              echo $this->Form->input('OtItem.id', array('type' => 'hidden')); 
        ?>
	...
	<tr>
	  ...
	  <td>
	   <?php 
	          echo $this->Form->input('MedicalItem.pharmacy_item', array('class' => 'validate[required,custom[name]]','id' => 'pharmacy_item', 'label'=> false, 'div' => false, 'error' => false, 'value' => $this->request->data['PharmacyItem']['name']));
	   ?><input type="hidden" name="MedicalItem[pharmacy_item_id]" id="pharmacy_item_id" value="<?php echo $this->request->data['MedicalItem']['pharmacy_item_id']?>"/>
	  </td>
	 </tr>
 	 <tr>
	  <td class="form_lables">
