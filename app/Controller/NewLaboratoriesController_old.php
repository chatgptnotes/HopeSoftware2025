...
Configure::write('debug',2);
			 ...
			 * Swapnil
			 */
			public function lab_dashboard($resetFilterFlag = 'null') {
				// $this->layout = "ajax";
				
					 $this->set('resetFilterFlag','null');
				...
				if ($this->request->query) {
					...
					//$conditions ['LaboratoryTestOrder.location_id'] = $this->Session->read ( "locationid" ); // commented by gulshan 
						...
						if ($status == "Recieved") {
							$conditions = array (
									'AND' => array (
											...
											'LaboratoryTestOrder.completed' => NULL ,
											//'LaboratoryResult.is_authenticate' => 0 
									) 
							);
						}
						if ($status == "Completed") {
							$conditions ['LaboratoryTestOrder.completed'] = 1;
							$conditions ['LaboratoryResult.is_authenticate'] = 0;
						}
				...
				$discFlag = $conditions ['Patient.is_discharge'];
					...
					//dpr($conditions);echo count($conditions);dpr($this->Session->read ( 'laboratory_dashboard_filters' ));exit;
    				if (count ( $conditions ) > 0 ) {
					
					$conditions ['Patient.is_discharge'] = ($discFlag)?$discFlag:'0';
					
					$this->Session->write ( 'laboratory_dashboard_filters', $conditions );//dpr($this->Session->read ( 'laboratory_dashboard_filters' ));exit;
				} else {
					...
					//dpr($this->Session->read ( 'laboratory_dashboard_filters' ));exit;
					if ($resetFilterFlag!=1)
						$conditions = $this->Session->read ( 'laboratory_dashboard_filters' );
					else $this->Session->write ( 'laboratory_dashboard_filters', '');
					if((($this->Session->read('role')) != Configure::read('doctor')) && $this->Session->read ('role')!=Configure::read('adminLabel')){
							
						$conditions ['LaboratoryTestOrder.completed'] = null;
					
					}else if(($this->Session->read('role')) == Configure::read('doctor')){
						$subSpclty = $this->User->find('first',array('conditions'=>array('User.id'=>$this->Session->read('userid')),'fields'=>array('sub_specialty')));
						if($subSpclty['User']['sub_specialty']){
							$conditions['Laboratory.test_group_id'] = $subSpclty['User']['sub_specialty'];
						}
						$conditions ['LaboratoryTestOrder.completed'] = 1;
						$conditions ['LaboratoryResult.is_authenticate'] = 0;
					}
		...
		/*$testOrdered = $this->LaboratoryTestOrder->find('all',array(*/
				'limit' => Configure::read ( 'number_of_rows' ),
						...
						'fields' => array (
								...
								'LaboratoryTestOrder.received_on',
								'LaboratoryTestOrder.user_id_rec',
								'Ward.name',
								'Bed.bedno',
								...
								'LaboratoryResult.id',
								'LaboratoryResult.user_id',
								'LaboratoryResult.create_time',
								'LaboratoryResult.modify_time',
								'LaboratoryResult.modified_by'
						),
						'order' => array (
				...
				$userName = $this->User->getUsersAllLocation();
				...
				$this->set (array ('testOrdered' => $testOrdered,'dataNoteDiagnosis'=>$dataNoteDiagnosis,'userName'=>$userName));
				
				$testResult = $this->LaboratoryResult->find ( 'list', array (
						'fields' => array (
								'LaboratoryResult.id',
								'LaboratoryResult.laboratory_test_order_id' 
						) 
				) );
				$this->set ( 'testResult', $testResult );
					...
					if ($this->request->data ['PatientDocument'] ['file_name'] [0] ['name']) {
						foreach ( $this->request->data ['PatientDocument'] ['file_name'] as $imgKey => $imgData ) {
							...
							if (! empty ( $imgData ['name'] )) {
								...
								$path = WWW_ROOT . 'uploads/laboratory/' . $imagename;
								move_uploaded_file ($imgData['tmp_name'], $path );
							}
							$dataVal = array ();
			...
			function printLab() {
				...
				$this->set ( 'authenticated', $authenticated );
				/*~~~~~~~~Gulshan~~~~~~*/
				 $signature = $this->Configuration->find ( 'first', array (
						'conditions' => array (
								'Configuration.name' => 'Signature',
								'Configuration.category' => 1 
						) 
				) ); 
				$signature = unserialize ( $signature ['Configuration'] ['value'] );
				$this->set ( 'unserializeSignature', $signature );
				
				
				$doctorDataForSignature = $this->DoctorProfile->find ( 'first', array (
						'fields' => array (
								'id',
								'education',
								'first_name',
						...
						'conditions' => array (
								'DoctorProfile.user_id' => $this->Session->read('userid')
						)
				) );
				$this->set ( 'doctorDataForSignature', $doctorDataForSignature );
			...
			public function AddSample() {
				...
				if (! empty ( $sample )) {
					...
					foreach ( $obj as $kay => $arr ) {
						...
						$getReqNo = $this->LaboratoryTestOrder->GenerateReqNo ( /* $patientId */$labId );
						$getSpecimenId = $this->LaboratoryTestOrder->autoGeneratedSpecimenID ( $key, $labId ); // specimenId
						$this->LaboratoryTestOrder->getLabRequsitionQR ( $getSpecimenId, $patientId ); // generating bar code using spcimen_id and patient_id
				...
				if (! empty ( $received )) {
					...
					foreach ( $rec as $kay => $arrRec ) {
						...
						$data['user_id_rec'] = $this->Session->read("userid");
						$this->LaboratoryTestOrder->save ( $data );
						$this->LaboratoryTestOrder->id = '';
					}
				}
			...
			function reTest() {
				$this->uses = array ('LaboratoryTestOrder','LaboratoryResult');
				$this->autoRender = false;
				$this->layout = "ajax";
				...
				if ($reTest) {
					...
					foreach ( $object as $key => $arr ) {
						...
						$laboratoryResultId = $arr ['laboratoryResultId'];
						/*
						 * $patientId = $arr['patientId'];
						 *
						...
						$this->LaboratoryTestOrder->id = '';
						$this->LaboratoryResult->updateAll (array('LaboratoryResult.is_authenticate' => 0), array ('LaboratoryResult.id' => $laboratoryResultId));
						$this->LaboratoryResult->id = '';
					}
				}
				$this->lab_dashboard ($resetFilterFlag = '1');
			}
