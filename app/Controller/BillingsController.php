...
class BillingsController extends AppController {
	...
	public function beforeFilter(){
		parent::beforeFilter();
    $this->loadModel('ChatGptLog','NewSurgeryData');
	}
...
// 	public function p2_form($patient_id=null){
...
// 	}
	...
	public function p2_form($patient_id=null){
        ...
        // Example: loop through and save each entry
        foreach ($data['procedure'] as $index => $procedureName) {
            $entry = [
                'srno' => $data['srno'][$index],
                // 'patient_id' => $patient_id,
                'procedure_name' => $procedureName,
                'cghs_code' => $data['cghs_code'][$index],
                'other_code' => $data['other_code'][$index],
                'rate' => $data['rate'][$index],
                'amount_claimed' => $data['amount_claimed'][$index],
                'amount_admitted' => $data['amount_admitted'][$index],
                'remarks' => $data['remarks'][$index],
                'patient_id' => $patientId
            ];
            $this->YourModel->create();
            $this->YourModel->save($entry); // Replace with your model
        }
...
// debug($responseData);exit;
	...
	function discharge_summary($id=null,$printInvestigation=null){
		...
		$this->uses = array('DiagnosisDrug','Person','Consultant','User','Patient','Diagnosis','SuggestedDrug','Note','OptAppointment','DoctorProfile'
				,'DischargeSummary','DischargeDrug','DischargeSurgery','FinalBilling', 'OperativeNote','PharmacySalesBillDetail','NewSurgeryData');
...
// 	$dineshTawade = $this->NewSurgeryData->find('all');
// 			debug($dineshTawade);exit;
		...
		if($labGroup){
			...
			$laboratoryTestOrderData= $this->LaboratoryTestOrder->find('all',array(
					...
					LaboratoryTestOrder.create_time,Laboratory.name,LaboratoryTestOrder.discount,
							LaboratoryToken.ac_id,LaboratoryToken.sp_id,TariffAmount.nabh_charges,TariffAmount.non_nabh_charges','TariffList.cghs_code','ServiceProvider.name'),
...
public function generateDeathSummary($patient_id = null) {
    ...
    $query = "Generate a detailed medical death summary for the following patient:\n\n"
		...
		// debug($query);
    // OpenAI API Call
    $apiKey = "YOUR_OPENAI_API_KEY_HERE"; // Replace with a valid API key
    $bodyText = [
        "model" => "gpt-4",
	...
	public function billReportLab($patientId=null){
		...
		if($this->request->query['flag']=='Lab'){
			...
			if(!empty($this->request->query['recID'])){//for billing record vise receipt
				...
				// $labData = $this->LaboratoryTestOrder->find('all',array('fields'=>array('LaboratoryTestOrder.*','ServiceProvider.name','Laboratory.name','TariffAmount.nabh_charges,TariffAmount.non_nabh_charges'),
				// 		'conditions' =>array('LaboratoryTestOrder.patient_id'=>$patientId,'LaboratoryTestOrder.is_deleted'=>'0',
				// 				'LaboratoryTestOrder.id'=>$labToPrint),/*'group'=>array('Laboratory.id')*/)); commented by pratik manusmare(27.06.2025)
				
				// Created by Pratik Manusmare on 27-06-2025 to print the list of laboratory services in ascending order by date.

				$labData = $this->LaboratoryTestOrder->find('all',array(
                'fields'=>array('LaboratoryTestOrder.*','ServiceProvider.name','Laboratory.name','TariffAmount.nabh_charges,TariffAmount.non_nabh_charges'),
                'conditions' =>array('LaboratoryTestOrder.patient_id'=>$patientId,'LaboratoryTestOrder.is_deleted'=>'0',
                        'LaboratoryTestOrder.id'=>$labToPrint),
                'order'=>array('LaboratoryTestOrder.start_date' => 'ASC', 'LaboratoryTestOrder.create_time' => 'ASC')
                /*'group'=>array('Laboratory.id')*/));
			}
...
//same day ward shift and price check 
	...
	public function printServiceInvoice($patientID=null,$tariffStandardId=null,$billNumber=null){
		...
		if($this->params->query['group']=='laboratory'){
		    // Sort lab data by start_date
				usort($labRadRec['labRate'], function($a, $b) {
					return strtotime($a['LaboratoryTestOrder']['start_date']) - strtotime($b['LaboratoryTestOrder']['start_date']);
				});
		    ...
		    // ADD THIS if needed
			usort($labRadRec['radRate'], function($a, $b) {
				return strtotime($a['RadiologyTestOrder']['start_date']) - strtotime($b['RadiologyTestOrder']['start_date']);
			});
...
public function p2_form_submit($patientId = null)
{
    $this->autoRender = false;
    ...
    // // Make connections using your model
    // $hopeDB->makeConnection($this->ProcedureEntry);
    // $ayushmanDB->makeConnection($this->ProcedureEntry);
    ...
    if ($this->request->is('post')) {
        ...
        // Load model
        $this->loadModel('ProcedureEntry');

        // Loop through and save each entry
        foreach ($data['procedure'] as $index => $procedureName) {
            $entry = [
                'srno' => $data['srno'][$index],
                'procedure_name' => $procedureName,
            ...
            $this->ProcedureEntry->create();
            $this->ProcedureEntry->save($entry);
        }
...
public function save_additional_procedures($patientId = null)
{
    $this->uses = array('AdditionalProcedures');
...
debug($hopeDB);
    // Make connections using your model
    $hopeDB->makeConnection($this->AdditionalProcedures);
    // $ayushmanDB->makeConnection($this->AdditionalProcedures);

    if ($this->request->is('post')) {
        $this->loadModel('AdditionalProcedures');
        debug($this->loadModel('AdditionalProcedures'));
        $data = $this->request->data('procedure');
      debug($this->request->data('procedure'));
        ...
        foreach ($data as $index => $item) {
            $entry = [
                ...
                'rate' => $item['rate'],
                'amount_claimed' => $item['claimed'],
                'amount_admitted' => $item['admitted'],
                'remarks' => $item['remarks']
            ];

            $this->AdditionalProcedures->create(); // CakePHP 2.x
            $this->AdditionalProcedures->save($entry);
              debug($data);exit;
        }
