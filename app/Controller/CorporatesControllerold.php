...
class CorporatesController extends BillingsController {
    ...
    public $components = array('RequestHandler', 'Email', 'General', 'Number', 'PhpExcel');
...
// function for amount paid for surgeon in report by swati
    ...
    public function getAnaesthesistamt($id, $amount) {
        ...
        if (!empty($id)) {
            //$id holds the patient's id 
            $this->request->data['OptAppointment']['id'] = $id;
            $this->request->data['OptAppointment']['anaesthesist_amt'] = $amount;
            $this->OptAppointment->save($this->request->data);  //update the extension_status of patient
        }
    }
    ...
    public function admin_mahindra_report($type = NULL) {
        ...
        $fields = array('Patient.id', 'Patient.lookup_name', 'Person.relation_to_employee', 'Patient.form_received_on', 'Patient.discharge_date', 'FinalBilling.cmp_paid_date',
    ...
    public function admin_rgjay_report($type = NULL) {
        ...
        $this->paginate = array(
            'fields' => array('Patient.id', 'Patient.lookup_name', 'Patient.discharge_date', 'Patient.is_discharge',
                ...
                'Person.district', 'FinalBilling.id', 'FinalBilling.bill_uploading_date', 'FinalBilling.dr_claim_date',
    ...
    public function admin_rgjay_payment_received_report($type = NULL) {
        ...
        $result = $this->Patient->find('all', array('fields' => array('Patient.id', 'Patient.case_no', 'Patient.hospital_no', 'Patient.lookup_name', 'Patient.form_received_on',
                'FinalBilling.package_amount', 'FinalBilling.amount_paid'), 'conditions' => array('Patient.tariff_standard_id' => '25', 'Patient.is_deleted' => 0, 'Patient.location_id' => $this->Session->read('locationid'))));
...
//debug($result);exit;
    ...
    public function admin_mpkay_report($type = NULL) {
        ...
        $fields = array('Patient.id', 'Patient.lookup_name', 'Patient.card_no', 'Patient.sanction_no', 'Person.id', 'Patient.form_received_on', 'FinalBilling.amount_paid', 'FinalBilling.id', 'FinalBilling.other_deduction_modified',
            'Person.relation_to_employee', 'FinalBilling.bill_number', 'FinalBilling.cmp_amt_paid', 'FinalBilling.cmp_paid_date', 'FinalBilling.package_amount',
    ...
    public function admin_wcl_report($type = NULL) {
        $this->uses = array('Patient', 'TariffStandard', 'FinalBilling', 'Billing');
        $this->layout = 'advance';
        $tariffID = $this->TariffStandard->getTariffStandardID("WCL"); //debug($tariffID);exit;
        $this->Patient->unBindModel(array(
            'hasMany' => array('PharmacySalesBill', 'InventoryPharmacySalesReturn')));

        $this->Patient->bindModel(array('belongsTo' => array(
                'PatientDocument' => array('foreignKey' => false, 'conditions' => array('PatientDocument.patient_id=Patient.id')),
                'FinalBilling' => array('type' => 'INNER', 'foreignKey' => false, 'conditions' => array('FinalBilling.Patient_id=Patient.id'),
                    'fields' => array('FinalBilling.id', 'FinalBilling.other_deduction', 'FinalBilling.paid_to_patient', 'FinalBilling.discount', 'FinalBilling.bill_uploading_date', 'FinalBilling.amount_paid', 'FinalBilling.bill_number', 'FinalBilling.tds', 'FinalBilling.package_amount')))), false);
        ...
        $conditions1 = array('Patient.tariff_standard_id' => 8, 'Patient.admission_type' => 'IPD', 'Patient.is_discharge' => '1', 'Patient.is_deleted' => 0, 'Patient.is_hidden_from_report' => 0, 'Patient.location_id' => $this->Session->read('locationid'));
        if (!empty($this->request->query)) {
            unset($conditions1['Patient.is_hidden_from_report']);
            if (!empty($this->request->query['lookup_name'])) {
                $conditions1['Patient.lookup_name'] = $this->request->query['lookup_name'];
        ...
        $fields = array('Patient.id', 'Patient.lookup_name', 'Patient.admission_type', 'Patient.form_received_on', 'FinalBilling.bill_uploading_date', 'FinalBilling.other_deduction',
            ...
            'Patient.is_hidden_from_report', 'FinalBilling.id', 'Patient.remark', 'FinalBilling.package_amount', 'PatientDocument.id', 'PatientDocument.filename');
    ...
    public function getRemark($id, $val) {
        ...
        if ($this->request->query['remark'] != NULL) {
            ...
            $this->request->data['Patient']['remark'] = "Package amount Rs." . $this->request->query['remark'] . "/-";
            $this->Patient->save($this->request->data);
        }
    }
    ...
    public function getUpdateStatus($id, $status_update, $tariffStandard = null) {
        $this->uses = array('Patient', 'Billing', 'ServiceBill', 'VoucherEntry', 'FinalBilling');
        //$this->autoRender = false;
        //$this->layout = 'false';
        ...
        if ($tariffStandard != "Private" && $tariffStandard != Configure::read('RGJAY') && $status_update == $configStatus['File Submitted']) {
            $finalData = $this->FinalBilling->find('first',array(
                'fields'=>array('FinalBilling.hospital_invoice_amount'),
                'conditions'=>array('FinalBilling.patient_id'=>$id)));
            if(!isset($finalData['FinalBilling']['hospital_invoice_amount']) || $finalData['FinalBilling']['hospital_invoice_amount'] < 0){
                echo json_encode('2');
                exit;
            } 
        }
        ...
        if ($tariffStandard == Configure::read('RGJAY') && $status_update == $configStatus['Claim Doctor Approved']) {
            $this->Billing->deleteRevokeJV($id);
            $this->ServiceBill->JvRgjayData($id);
            $this->Billing->addFinalVoucherLogJV($billArrayData, $id);
        } elseif ($tariffStandard != "Private" && $tariffStandard != Configure::read('RGJAY') && $status_update == $configStatus['File Submitted']) {
            $this->Billing->deleteRevokeJV($id);
            //$this->Billing->finalDischargeJV($id);
            //$this->Billing->addFinalVoucherLogJV($billArrayData,$id);
            //=======================================BOF for final discharge jv by amit jain===============================//
            $finalBillData = $this->FinalBilling->find('first', array('fields' => array('FinalBilling.hospital_invoice_amount'),
                'conditions' => array('FinalBilling.patient_id' => $id)));
            $this->VoucherEntry->corporatePatientFinalJv($id, $finalBillData['FinalBilling']['hospital_invoice_amount']);
            //=============================================EOF for final dischage Jv=======================================//
        }
        echo json_encode('1');
        exit;
        //EOF for RGJAY
    ...
    public function admin_other_outstanding_report($typeId = NULL) { //debug($this->request->query);exit;
        $this->layout = 'advance';
        ...
        $conditions1 = array('Patient.tariff_standard_id' => $typeId, 'Patient.is_deleted' => 0, 'Patient.is_discharge' => 1, 'Patient.is_hidden_from_report' => 0,
            'Patient.location_id' => $this->Session->read('locationid'));
        ...
        $this->paginate = array(
            ...
            'fields' => array("SUM(CASE WHEN Billing.payment_category != 'corporateAdvance' AND Billing.payment_category != 'TDS' THEN Billing.amount ELSE 0 END) as patientPaid",
                ...
                'SUM(Billing.discount_amount) as total_discount', 'Billing.payment_category', 'Patient.patient_id',
                'Patient.admission_id', 'Patient.discharge_date', 'Patient.id', 'Patient.is_discharge', 'Patient.lookup_name', 'Patient.form_received_on', 'Patient.person_id',
                'FinalBilling.amount_pending', 'FinalBilling.bill_uploading_date', 'FinalBilling.total_amount', 'PatientDocument.id', 'PatientDocument.filename'),
            'conditions' => $conditions1,
            'group' => array('Patient.id'),
        ...
        $tariffData = $this->TariffStandard->find('list', array('fields' => array('id', 'name'), 'conditions' => array('TariffStandard.is_deleted' => 0, /* 'TariffStandard.code_name IS NOT NULL', */'TariffStandard.location_id' => $this->Session->read('locationid'), 'TariffStandard.name NOT' => 'Private')));
        $this->set('tariffData', $tariffData);
        $this->set('tariffStandardID', $typeId);
        if ($this->request->isAjax()) {
            $this->layout = 'ajax';
            $this->render('ajax_other_outstanding_report');
        }
        if ($this->params->pass[1] == 'excel') {
            $this->autoRender = false;
            $this->layout = false;
    ...
    public function updatePackageAmount($patient_id, $tariff_standard_id) {
        ...
        if ($this->request->data['corporateClaims']) {
            ...
            if ($this->request->data['corporateClaims']['patient_id'] && !empty($this->request->data['corporateClaims']['cmp_amt_paid'])) { //update actual amount received from company
                $this->Billing->save($billingData = array(
            ...
            $this->redirect(array('controller' => 'corporates', 'action' => 'other_outstanding_report', $tariff_standard_id, 'admin' => true));
        }
        //exit;
    }
...
<<<<<<< .mine
		...
		function corporate_advance_payment($tariff_standard_id=null,$patient_id=null){
			...
			$advanceData = $this->Patient->find('all',array('conditions'=>array('Patient.id'=>$patient_id,
					...
					'fields'=>array('FinalBilling.total_amount','Billing.id','Billing.amount','Billing.date','Billing.discount','Billing.payment_category','Billing.bill_number','Billing.remark'),
			...
			foreach($advanceData as $key =>$value){
				...
				$totalAmount  = $value['FinalBilling']['total_amount'] ;
			} 
			...
			if(!empty($this->request->query)){
				...
				$this->paginate = array(
						...
						'fields'=>array("SUM(CASE WHEN Billing.payment_category != 'corporateAdvance' AND Billing.payment_category != 'TDS' THEN Billing.amount ELSE 0 END) as patientPaid",
								...
								'Patient.tariff_standard_id','Patient.admission_id','Patient.discharge_date','Patient.id','Patient.is_discharge','Patient.lookup_name','Patient.form_received_on',
...
<<<<<<< .mine
		...
		function updateCorporatePaymentReceived(){
			...
			if(!empty($this->request->data)){
				if(!empty($this->request->data['CorporateSuperBill']['corporate_super_bill_id'])){
					...
					$this->CorporateSuperBill->updateAll(array('received_amount'=>"'$recAmt'"),
							array('CorporateSuperBill.id'=>$corporate['CorporateSuperBill']['id']));
		...
		public function getExcel($billId=null,$tariffId=null){
			$this->autoRender = false ;
			...
			$patientId = $billData['CorporateSuperBill']['patient_id'];
			...
			switch ($tariffName){
				case "WCL": 
					$this->getWclExcel($billId,$tariffId);
				break; 
				case "BSNL":
					$this->getBSNLExcel($billId,$tariffId);
					break;
				case "BHEL":
						$this->getBHELExcel($billId,$tariffId);
					break;
				case "MPKAY":
						$this->getMPKAYExcel($billId,$tariffId);
				    break;
				case "MML":
						$this->getMMLExcel($billId,$tariffId);
					break;
				case "ECHS":
					$this->getECHSExcel($billId,$tariffId);
				default: 				
			}
			...
			if($data['surgeryArray']['from']){
				...
				foreach($data['surgeryArray'] as $surKey =>$surVal){
					...
					foreach($surVal as $key=>$surgery){
						...
						}else if($data['superbillData']['CorporateSuperBill']['patient_type']=='Semi-Special'||$data['superbillData']['CorporateSuperBill']['patient_type']=='Semi-Private'){
							if($key==1){
								$this->PhpExcel->addTableRow(array($count,$serialCharArray[$key-1]." ".$surgery['name'],$surgery['cghs'],$surgery['amount'],"",""));
							}else{
								$this->PhpExcel->addTableRow(array("",$serialCharArray[$key-1]." ".$surgery['name'],$surgery['cghs'],$surgery['amount'],"",""));
						...
						}else if($data['superbillData']['CorporateSuperBill']['patient_type']=='Special'||$data['superbillData']['CorporateSuperBill']['patient_type']=='Private'){
							if($key==1){
								$this->PhpExcel->addTableRow(array($count,$serialCharArray[$key-1]." ".$surgery['name'],$surgery['cghs'],$surgery['amount'],"",""));
								$dedAmt=$surgery['amount']+$surgery['tenPer'];
								$this->PhpExcel->getActiveSheet()->getStyle("C".$this->PhpExcel->_row)->getFont()->setBold(true);
								$this->PhpExcel->addTableRow(array("",""," 15% more as per WCL Guideline",$surgery['tenPer'],"1",$dedAmt));
							}else{
								$this->PhpExcel->addTableRow(array("",$serialCharArray[$key-1]." ".$surgery['name'],$surgery['cghs'],$surgery['amount'],"",""));
								$this->PhpExcel->getActiveSheet()->getStyle("C".$this->PhpExcel->_row)->getFont()->setBold(true);
								$this->PhpExcel->addTableRow(array("",""," 50% Less as per CGHS Guideline",$surgery['fiftyPer'],"",""));
...
>>>>>>> .r9948
    ...
    public function getWclExcel($billId = null, $tariffId = null) {
        ...
        $this->PhpExcel->addTableRow(array("", "UNIT NAME / MINE ", $admisionDate, "", "", ""));
        $this->PhpExcel->addTableRow(array("", "NEIS CODE NUMBER	", $admisionDate, "", "", ""));
        $this->PhpExcel->addTableRow(array("", "AEIS CODE NUMBER", $admisionDate, "", "", ""));
        $this->PhpExcel->addTableRow(array("", "EMPLOYEE ADDRESS", $data['tariffStdData']['Patient']['address1'], "", "", ""));
        $this->PhpExcel->addTableRow(array("", "DESIGNATION", $data['tariffStdData']['Patient']['designation'], "", "", ""));
        $this->PhpExcel->addTableRow(array("", "CONTACT NO.", $data['tariffStdData']['Patient']['mobile_phone'], "", "", ""));
        $this->PhpExcel->addTableRow(array("", "DIAGNOSIS", $data['tariffStdData']['Patient']['diagnosis_txt'], "", "", ""));
        ...
        $productsHead = array(
            array('label' => __(' Sr.no '), 'filter' => true),
            array('label' => __(' ITEM '), 'filter' => true, 'width' => 50, 'wrap' => true),
            array('label' => __(' WCL NABH CODE No. ')),
            array('label' => __(' WCL NABH RATE ')),
            array('label' => __(' QTY ')),
            array('label' => __(' AMOUNT ')),
            array('label' => __(' Total AMOUNT '))
        );
        ...
        if ($data['wardArray']) {
            ...
            foreach ($data['wardArray'] as $wardKey => $wardVal) {
                if ($wardVal['name']) {
                    $this->PhpExcel->addTableRow(array(" ", "$romanArray[$r] )  Accomodation For " . $data['wardAmt']['wardName'] . " Ward", "", "", "", ""));
                    foreach ($data['conservative'] as $conKey => $conVal) {
                        $this->PhpExcel->getActiveSheet()->getStyle('B' . $this->PhpExcel->_row)->getAlignment()->applyFromArray(
                                array(
                                    'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_RIGHT,
        ...
        if ($data['total']['Pharmacy']) {
            ...
            $count = $count + 1;
            $mediceneTotal = 0;
            $mediceneTotal +=$data['total']['Pharmacy'];
        }
        $this->PhpExcel->getActiveSheet()->getStyle("D" . $this->PhpExcel->_row)->getFont()->setBold(true);
        $this->PhpExcel->getActiveSheet()->getStyle("D" . $this->PhpExcel->_row)->getFont()->setSize(14);
        $this->PhpExcel->addTableRow(array("", "", "", "Sub Total:-", "", "", $mediceneTotal));
        // EOF Medicene
                ...
                foreach ($surVal as $key => $surgery) {
                    if ($data['superbillData']['CorporateSuperBill']['patient_type'] == 'General') {
                        ...
                        if ($key == 1) {
                            ...
                            $this->PhpExcel->getActiveSheet()->getStyle("C" . $this->PhpExcel->_row)->getFont()->setBold(true);
                            $dedAmt = $surgery['amount'] - $surgery['fiftyPer'] + $surgery['tenPer'];
                            $this->PhpExcel->addTableRow(array("", "", " 15% Less as per WCL Guideline", $surgery['tenPer'], "1", $dedAmt));
                        }
                    }
                    $surgeryTotal +=$dedAmt;
                }
        ...
        $this->PhpExcel->getActiveSheet()->getStyle($totalTextBlock)->getFont()->setBold(true);
        $this->PhpExcel->getActiveSheet()->getStyle("D" . $this->PhpExcel->_row)->getFont()->setBold(true);
        $this->PhpExcel->getActiveSheet()->getStyle()->getFont("G" . $this->PhpExcel->_row)->setBold(true);
        ...
        $this->PhpExcel->getActiveSheet()->getStyle()->getFont("D" . $this->PhpExcel->_row)->setBold(true);
        $this->PhpExcel->getActiveSheet()->getStyle()->getFont("G" . $this->PhpExcel->_row)->setBold(true);
        ...
        //	$this->PhpExcel->getActiveSheet()->setCellValue($totalCellBlock,$wholeTotal );
        // close table and output
    ...
    public function getMPKAYExcel($billId = null, $tariffId = null) {
        ...
        // Create a first sheet, representing Product data
        $this->PhpExcel->setActiveSheetIndex(0);
        $this->PhpExcel->getActiveSheet()->setTitle('MPKAY BILL'); //to set the worksheet title
        //$patientId='1';
        ...
        $this->PhpExcel->addTableRow(array("", "AGE", ':' . $data['tariffStdData']['Patient']['age'], "", "", ""));
        $this->PhpExcel->addTableRow(array("", "SEX", ':' . strtoupper($data['tariffStdData']['Patient']['sex']), "", "", ""));
        $this->PhpExcel->addTableRow(array("", "NAME OF THE MAIN MEMBER", ':', "", "", ""));
        //$this->PhpExcel->addTableRow(array("","NAME OF MPKAY BENEFICIARY",$data['tariffStdData']['Patient']['name_of_ip'],"","",""));
        $this->PhpExcel->addTableRow(array("", "DESIGNATION", ':' . $data['tariffStdData']['Patient']['designation'], "", "", ""));
        $this->PhpExcel->addTableRow(array("", "RELATION WITH MPKAY EMPLOYEE", ':' . $data['tariffStdData']['Patient']['relation_to_employee'], "", "", ""));
        $this->PhpExcel->addTableRow(array("", "CONTACT NO.", ':' . $data['tariffStdData']['Patient']['mobile_phone'], "", "", ""));
        //$this->PhpExcel->addTableRow(array("","EMPLOYEE ADDRESS",':'.$data['tariffStdData']['Patient']['address1'],"","",""));
        //$this->PhpExcel->addTableRow(array("","DESIGNATION",':'.$data['tariffStdData']['Patient']['designation'],"","",""));
        $this->PhpExcel->addTableRow(array("", "GPF NUMBER / EMPLOYEE CODE", ':', "", "", ""));
        $this->PhpExcel->addTableRow(array("", "MAIN MEMBER UHID NO.", ':', "", "", ""));
        ...
        $this->PhpExcel->addTableRow(array("", "DATE OF ADMISSION ", ':' . $admisionDate, "", "", ""));
        $this->PhpExcel->addTableRow(array("", "DATE OF DISCHARGE", ':' . $dischargeDate, "", "", ""));
        $this->PhpExcel->addTableRow(array("", "CATEGORY", ':', "", "", ""));
        $this->PhpExcel->addTableRow(array("", "DIAGNOSIS", ':' . $data['tariffStdData']['Patient']['diagnosis_txt'], "", "", ""));
        $this->PhpExcel->addTableRow(array("", "", "", "", "", ""));
        ...
        $productsHead = array(
            array('label' => __(' Sr.no '), 'filter' => true),
            array('label' => __(' ITEM '), 'filter' => true, 'width' => 50, 'wrap' => true),
            array('label' => __(' MPKAY NABH CODE No. ')),
            array('label' => __(' MPKAY NABH RATE ')),
            array('label' => __(' QTY ')),
            array('label' => __(' AMOUNT '))
        );
        ...
        // for align header text center
        $this->PhpExcel->getActiveSheet()->getStyle('A24:F24')->getAlignment()->applyFromArray(
                array(
                    'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,
                    'vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER,
                    'rotation' => 0,
        ...
        $this->PhpExcel->getActiveSheet()->getStyle("A" . $this->PhpExcel->_row)->getFont()->setBold(true);
        $this->PhpExcel->getActiveSheet()->getStyle("B" . $this->PhpExcel->_row)->getFont()->setBold(true);
        $this->PhpExcel->addTableRow(array("", "Consulation Charges", "", "", "", ""));
        ...
        if ($data['surgeryArray']['from']) {
            ...
            $this->PhpExcel->getActiveSheet()->getStyle("B" . $this->PhpExcel->_row)->getFont()->setBold(true);
            $this->PhpExcel->addTableRow(array("Surgical Treatment", "", "", "", "", ""));
            //$this->PhpExcel->addTableRow(array(""," Surgical Treatment( ". $data['surgeryArray']['from']." To ".$data['surgeryArray'][to] .")","","","",""));
            $serialCharArray = array('a)', 'b)', 'c)', 'd)', 'e)', 'f)', 'g)', 'h)', 'i)');
            foreach ($data['surgeryArray'] as $surKey => $surVal) {
    ...
    public function getBHELExcel($billId = null, $tariffId = null) {
        ...
        $productsHead = array(
            array('label' => __(' Sr.no '), 'filter' => true),
            array('label' => __(' ITEM '), 'filter' => true, 'width' => 50, 'wrap' => true),
            array('label' => __(' BHEL NABH CODE No. ')),
            array('label' => __(' BHEL NABH RATE ')),
            array('label' => __(' QTY ')),
            array('label' => __(' AMOUNT '))
        );
        // add heading with different font and bold text
        ...
        if ($data['radArray']) {
            foreach ($data['radArray'] as $radKey => $radVal) {
                ...
                $this->PhpExcel->addTableRow(array($count, $radVal['name'], $radVal['Bhel_code'], $radVal['amount'], $radVal['no_of_times'], $radVal['amount'] * $radVal['no_of_times']));
                $count++;
            }
        }
        // EOF Radiology
    ...
    public function getECHSExcel($billId = null, $tariffId = null) {
        ...
        $dischargeDate = $this->DateFormat->formatDate2Local($data['tariffStdData']['Patient']['discharge_date'], Configure::read('date_format'), false);
        $this->PhpExcel->getActiveSheet()->getStyle("C" . $this->PhpExcel->_row)->getFont()->setBold(true);
        $this->PhpExcel->addTableRow(array("", "", "ECHS BILL", "", "", ""));
        ...
        $this->PhpExcel->addTableRow(array("", "AGE", ':' . $data['tariffStdData']['Patient']['age'], "", "", ""));
        $this->PhpExcel->addTableRow(array("", "SEX", ':' . strtoupper($data['tariffStdData']['Patient']['sex']), "", "", ""));
        $this->PhpExcel->addTableRow(array("", "NAME OF ECHS BENEFICIARY", ':', "", "", ""));
        $this->PhpExcel->addTableRow(array("", "RANK", ':', "", "", ""));
        $this->PhpExcel->addTableRow(array("", "RELATION WITH ECHS BENEFICIARY", ':' . $data['tariffStdData']['Patient']['relation_to_employee'], "", "", ""));
        $this->PhpExcel->addTableRow(array("", "ECHS MEMBERSHIP NUMBER", ':', "", "", ""));
        //$this->PhpExcel->addTableRow(array("","CONTACT NO.",$data['tariffStdData']['Patient']['mobile_phone'],"","",""));
        $this->PhpExcel->addTableRow(array("", "OPD REGISTRATION NUMBER", ':', "", "", ""));
        $this->PhpExcel->addTableRow(array("", "DATE OF ISSUE OF REFERAL", ':', "", "", ""));
        $this->PhpExcel->addTableRow(array("", "DATE OF ADMISSION", ':' . $admisionDate, "", "", ""));
        $this->PhpExcel->addTableRow(array("", "DATE OF DISCHARGE", ':' . $dischargeDate, "", "", ""));
        $this->PhpExcel->addTableRow(array("", "", "", "", "", ""));
        ...
        if ($data['surgeryArray']['from']) {
            ...
            $this->PhpExcel->addTableRow(array("Surgical Treatment", "", "", "", ""));
            $this->PhpExcel->addTableRow(array("( " . $data['surgeryArray']['from'] . " To " . $data['surgeryArray'][to] . ")", "", "", "", ""));
            $this->PhpExcel->getActiveSheet()->getStyle("C" . $this->PhpExcel->_row)->getFont()->setBold(true);
            $this->PhpExcel->getActiveSheet()->getStyle("C" . $this->PhpExcel->_row)->getFont()->setSize(14);

            //$this->PhpExcel->addTableRow(array($count,""," Surgical Treatment( ". $data['surgeryArray']['from']." To ".$data['surgeryArray'][to] .")","","","",""));
            $this->PhpExcel->addTableRow(array($count, "", "Surgical Treatment:", "", ""));
            $serialCharArray = array('a)', 'b)', 'c)', 'd)', 'e)', 'f)', 'g)', 'h)', 'i)');
            foreach ($data['surgeryArray'] as $surKey => $surVal) {
                foreach ($surVal as $key => $surgery) {
                    if ($data['superbillData']['CorporateSuperBill']['patient_type'] == 'General') {
                        ...
                        if ($key == 1) {
                            ...
                            $this->PhpExcel->getActiveSheet()->getStyle("C" . $this->PhpExcel->_row)->getFont()->setBold(true);
                            $dedAmt = $surgery['amount'] - $surgery['fiftyPer'] + $surgery['tenPer'];
                            $this->PhpExcel->addTableRow(array("", "", "", " 15% Less as per ECHS Guideline", $surgery['tenPer'], "1", $dedAmt));
                            //debug($dedAmt);exit;	
                        }
                    }/* else  //for private
                      {
        ...
        /* $countRow= $this->PhpExcel->_row+1;
          //debug($countRow); exit; */
        $totalCellBlock = "G" . $countRow;
        $totalTextBlock = "C" . $countRow;
        $totalTextBlock = "D" . $countRow;
        ...
        //$this->PhpExcel->getActiveSheet()->getStyle()->getFont("C".$this->PhpExcel->_row)->setBold(true);
        $this->PhpExcel->getActiveSheet()->getStyle()->getFont("G" . $this->PhpExcel->_row)->setBold(true);
        $this->PhpExcel->getActiveSheet()->getStyle()->getFont("C" . $this->PhpExcel->_row)->setBold(true);
        $this->PhpExcel->addTableRow(array("", "", "TOTAL BILL AMOUNT", "", "", "", "$totalAmount"));
    ...
    public function getMMLExcel($billId = null, $tariffId = null) {
        ...
        // Create a first sheet, representing Product data
        $this->PhpExcel->setActiveSheetIndex(0);
        $this->PhpExcel->getActiveSheet()->setTitle('MML BILL'); //to set the worksheet title
        //$patientId='1';
        $data = $this->companyExcelReport($billId, $tariffId);
        //debug($data);exit;
        ...
        $dischargeDate = $this->DateFormat->formatDate2Local($data['tariffStdData']['Patient']['discharge_date'], Configure::read('date_format'), false);
        $this->PhpExcel->getActiveSheet()->getStyle("C" . $this->PhpExcel->_row)->getFont()->setBold(true);
        $this->PhpExcel->addTableRow(array("", "", "Mahindra & Mahindra Ltd BILL", "", "", ""));
        ...
        $this->PhpExcel->addTableRow(array("", "RELATION WITH MAHINDRA AND MAHINDRA Ltd BENEFICIARY", $data['tariffStdData']['Patient']['relation_to_employee'], "", "", ""));
        $this->PhpExcel->addTableRow(array("", "EMPLOYEE ADDRESS", $data['tariffStdData']['Patient']['address1'], "", "", ""));
        $this->PhpExcel->addTableRow(array("", "CONTACT NO.", $data['tariffStdData']['Patient']['mobile_phone'], "", "", ""));
        $this->PhpExcel->addTableRow(array("", "DESIGNATION", $data['tariffStdData']['Patient']['designation'], "", "", ""));
        $this->PhpExcel->addTableRow(array("", "DIAGNOSIS", $data['tariffStdData']['Patient']['diagnosis_txt'], "", "", ""));
        ...
        $productsHead = array(
            array('label' => __('Sr.no'), 'filter' => true),
            array('label' => __('ITEM'), 'filter' => true, 'width' => 50, 'wrap' => true),
            array('label' => __('Mahindra & Mahindra Ltd NABH CODE No.')),
            array('label' => __('Mahindra & Mahindra Ltd NABH RATE')),
            array('label' => __('QTY')),
            array('label' => __('AMOUNT'))
        );
                ...
                foreach ($surVal as $key => $surgery) {
                    if ($data['superbillData']['CorporateSuperBill']['patient_type'] == 'General') {
                        ...
                        if ($key == 1) {
                            ...
                            $this->PhpExcel->getActiveSheet()->getStyle("C" . $this->PhpExcel->_row)->getFont()->setBold(true);
                            $dedAmt = $surgery['amount'] - $surgery['fiftyPer'];
                            $this->PhpExcel->addTableRow(array("", "", " 50% Less as per Mahindra & Mahindra Ltd Guideline", $surgery['fiftyPer'], "1", $dedAmt));
                        }
                    } else if ($data['superbillData']['CorporateSuperBill']['patient_type'] == 'Special' || $data['superbillData']['CorporateSuperBill']['patient_type'] == 'Private') {
                        if ($key == 1) {
                            $this->PhpExcel->addTableRow(array("", $serialCharArray[$key - 1] . " " . $surgery['name'], $surgery['cghs'], $surgery['amount'], "", ""));
                            $dedAmt = $surgery['amount'] + $surgery['tenPer'];
                            $this->PhpExcel->getActiveSheet()->getStyle("C" . $this->PhpExcel->_row)->getFont()->setBold(true);
                            $this->PhpExcel->addTableRow(array("", "", " 15% more as per Mahindra & Mahindra Ltd Guideline", $surgery['tenPer'], "1", $dedAmt));
                        } else {
                            $this->PhpExcel->addTableRow(array("", $serialCharArray[$key - 1] . " " . $surgery['name'], $surgery['cghs'], $surgery['amount'], "", ""));
                            $this->PhpExcel->getActiveSheet()->getStyle("C" . $this->PhpExcel->_row)->getFont()->setBold(true);
                            $this->PhpExcel->addTableRow(array("", "", " 50% Less as per RBI Guideline", $surgery['fiftyPer'], "", ""));
                            $this->PhpExcel->getActiveSheet()->getStyle("C" . $this->PhpExcel->_row)->getFont()->setBold(true);
                            $dedAmt = $surgery['amount'] - $surgery['fiftyPer'] + $surgery['tenPer'];
                            $this->PhpExcel->addTableRow(array("", "", " 15% Less as per Mahindra & Mahindra Ltd Guideline", $surgery['tenPer'], "1", $dedAmt));
                            //debug($dedAmt);exit;
                        }
                    }/* else  //for private
                      {
    ...
    public function getBSNLExcel($billId = null, $tariffId = null) {
        ...
        $productsHead = array(
            array('label' => __(' Sr.no '), 'filter' => true),
            array('label' => __(' ITEM '), 'filter' => true, 'width' => 50, 'wrap' => true),
            array('label' => __(' BSNL NABH CODE No. ')),
            array('label' => __(' BSNL NABH RATE ')),
            array('label' => __(' QTY ')),
            array('label' => __(' AMOUNT '))
        );
        // add heading with different font and bold text
