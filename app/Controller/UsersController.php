...
public function update_admission_id() {
    $this->autoRender = false;
    ...
    if ($this->request->is('ajax')) {
        ...
        $newAdmissionId = trim($this->request->data['new_admission_id']);
        
        // Simple validation
        if (empty($newAdmissionId)) {
            echo json_encode(['success' => false, 'message' => 'Admission ID cannot be empty']);
            return;
        }
        
        // Update without checking duplicates (if that's what you want)
        $this->Patient->id = $patientId;
        if ($this->Patient->saveField('admission_id', $newAdmissionId)) {
            echo json_encode(['success' => true, 'message' => 'Updated successfully', 'new_id' => $newAdmissionId]);
        } else {
