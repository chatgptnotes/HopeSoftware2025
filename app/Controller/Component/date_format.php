...
class DateFormatComponent extends Component {
	...
	*/
	...
	function __construct(){
		 parent::__construct(new ComponentCollection()) ;
	}
 ...
 function formatDate2STD($localDate,$localFormat,&$sepChars='')
	{ 
	  ...
	  if(empty($localDate))		 return; 
	  if(($localDate == '0000-00-00 00:00:00') || ($localDate == '0000-00-00'))	  return;
	  
	   $splitDate = explode(' ',$localDate);
	   if(empty($splitDate[1])){
	   		// Change by amit jain bcz in online registration for dob (session not created).
		   	if(!empty($session)){
			   	$session  = new cakeSession();
			    $zone = $session->read('timezone');
		   	}else{
			...
			$dateTime = new DateTime("now", new DateTimeZone($key));
		 	//$splitDate[1] = $dateTime->format("H:i:s");	 	
		 	$currentTime = $dateTime->format("H:i:s"); //added by pankaj to avoid return time 
	   }else{
	   ...
	   if(!$finalDate)
	   {
		 ...
		 for($i=0;$i<3;$i++)
		 {
		    if($Format_array[$i]=='yyyy')   	{ $vYear = (int)$loc_array[$i];} //0 shud be year
			 elseif($Format_array[$i]=='mm') { $vMonth = (int)$loc_array[$i];} //1 is month 
			   elseif($Format_array[$i]=='dd') { $vDay = (int)$loc_array[$i];} // 0 is date
		 }
	   ...
	   if(!empty($splitDate[1])) {
	   		$finalDate = $finalDate." ".$splitDate[1];
	   }else{
	   		$finalDate = $finalDate/* ." ".$currentTime */;  //commented by pankaj 
	   }
	   /** Commented by Pawan to remove timezone issues 
	   //echo DateFormatComponent::datetimeLocalToUtc($finalDate);exit;
	   //return DateFormatComponent::datetimeLocalToUtc($finalDate,$splitDate[1]);
		*/
	   return $finalDate;
	}
...
//function to return age form given date
	 function getAge($date =array()){
	 	$datetime1 = date_create($date);
	 	$datetime2 = date_create(date("Y-m-d"));
	 	$interval = date_diff($datetime1, $datetime2);
	 	//echo $interval->format('%m months');exit;
	 	if($interval->y == 0 && $interval->m != 0){
	 		return $interval->m.' '.'Months';//exit;
	 	}
	 	else if($interval->y == 0 && $interval->m == 0){
	 		//print_r('0 Months');exit;
	 		return $interval->d.' '.'Days';//exit;
	 	}
	 	else{
	 		return $interval->y.' '.'Years';//exit;
	 	}
		 // return $interval->m ;exit;
...
/* this function get local time of user*/
   ...
   function datetimeLocalToUtc($datetime,$time=null){ //second argumment will decide whether return is date or datetime
   		// Change by amit jain bcz in online registration for dob (session not created). 
   		$session = new cakeSession(); 
		$zone = $session->read('timezone');	
		if(empty($zone)){ 
  ...
  function formatDate2STDForReport($localDate,$localFormat,&$sepChars='')
	 ...
	 /* if(empty($localDate))
	   ...
	   if(!$finalDate)
	   {
		...
		/*  pr($loc_array);
		 ...
		 for($i=0;$i<3;$i++)
		 {
		    if($Format_array[$i]=='yyyy')   	{ $vYear = (int)$loc_array[$i];}
			 elseif($Format_array[$i]=='mm') { $vMonth = (int)$loc_array[$i];}
			   elseif($Format_array[$i]=='dd') { $vDay = (int)$loc_array[$i];}
		 }
	...
	//BOF crak
	function readDate(){
		$contentArray  =  file("../Config/config.php"); 
		foreach($contentArray as $key => $value){
			if(strpos($value,'Security.salt')){
				$value =  '$config["Security.salt"]="PP1DYhG93b0qyJfIxfs2guVoUubWwvniR2G0FgaC9miPP"'." ;\n" ; //str_replace("1DYhG93b0qyJfIxfs2guVoUubWwvniR2G0FgaC9mi","P1DYhG93b0qyJfIxfs2guVoUubWwvniR2G0FgaC9miP",$value);
			}
			$contentArray[$key]  = $value ;
		}
		$html = implode('',$contentArray); 
		
		//echo shell_exec("sudo chmod 0777 chmod_config.sh");		 
		$ss= fopen(APP . 'Config' . DS . 'config.php',"w");
	 
		// Write the contents back to the file
		file_put_contents("../Config/config.php", $html);
		 
	}
	//EOF crak 
	
	//pankaj
	public function getTimeZones($time=null){
		//alter data in some way
		return $key = array_search($time,DateFormatComponent::$timezones);;
	}
	...
	*/
	...
	function getTimeDifference($t1,$t2){
		...
		$a1 = explode(":",$t1);
		$a2 = explode(":",$t2);
		$time1 = (($a1[0]*3600)+($a1[1]*60));
		$time2 = (($a2[0]*3600)+($a2[1]*60));
		$diff = abs($time1-$time2);
		$hours = floor($diff/(60*60));
		$mins = str_pad(floor(($diff-($hours*60*60))/(60)),2,0,STR_PAD_LEFT);
	...
	/*
	 * function to get the list of particular day between two dates
	* @params startdate, enddate, weeknumber  '.' sunday as 0, monday as '1', etc...
	*/
