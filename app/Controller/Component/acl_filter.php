 ...
 */
    public function auth() {
    	...
    	// $this->controller->Auth->ajaxLogin = 'ajaxlogin';
    	...
    	if($this->controller->action != "common" && $this->controller->action != "login" && $this->controller->action != "logout" && 
    	$this->controller->action != "autocomplete" && $this->controller->name != "Landings"){
    		//Pawan+Pankaj (Landing action is added as patient login goes to infinite loop)
    		//BOF pankaj for module permissions    		 
    		$modulePermissions = $this->Session->read('module_permissions') ;
    		 
    		if(!in_array($this->controller->name,$modulePermissions)){
    			$this->Session->setFlash(__('You don\'t have permission for this module.'));
    			$this->controller->redirect(array('controller'=> 'users','action' => 'common','admin'=>false,'superadmin'=>false));
    		}
    		 
    		//EOF pankaj for module permissions
    		$aro = $this->controller->Acl->Aro->find('first', array(
    				...
    				$aroUser = $this->controller->Acl->Aro->find('first', array(
    						...
    						if($current_action_acosid['Aco']['is_permission_need'] == "0"){
    							return true;
    						}
    						/*****************************************************************************************/
