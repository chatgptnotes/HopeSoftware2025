...
public function parseORU(){
		...
		foreach($paragonMessages as $message){
			...
			foreach ($xml->children() as $child){
				array_push($this->InterfaceEngine->LABORUNODES,$child->getName());
			}
			//echo $xmlString;
			//pr($xml);exit;
			$orc = $xml->ORC;
			$pid = $xml->PID;
			$this->InterfaceEngine->NTESEG = $xml->NTE;
			...
			$this->InterfaceEngine->obxSegments = $xml->OBX;
			$cnt = 0;
			$labChilds = $laboratoryTestOrderModel->find('all',array('fields'=>array('id'),'conditions'=>array('LaboratoryTestOrder.patient_id'=>$patientId,'LaboratoryTestOrder.parent_id'=>$laboratoryTestOrderId)));
			
			foreach($xml->OBR as $key=>$value){
				if($cnt > 0){
					$laboratoryTestOrderId = $labChilds[$cnt - 1]['LaboratoryTestOrder']['id'];
				}
				$laboratoryResultId = $this->InterfaceEngine->parseOBR($value,$patientId,$laboratoryTestOrderId,$xml->ORC,$xml->MSH);
				$laboratoryHL7Result = $this->InterfaceEngine->parseOBX($this->InterfaceEngine->obxSegments,$laboratoryResultId,$laboratoryTestOrderId,$patientId,$this->InterfaceEngine->NTESEG);
				$cnt++;
				//$this->nteCounter = 1;
			}
	...
	public function createORMMessage($patientId,$labId){
		...
		$labdata = $laboratoryTestOrderModel->find('all',array('fields'=>array('id','patient_id'),'conditions'=>array('LaboratoryTestOrder.is_processed'=>'0','LaboratoryTestOrder.parent_id'=>'0')));
